// CONFIG
// Funziona sia con server locale che con GitHub Codespaces
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const data = { action: 'login', username: document.getElementById('l-username').value, password: document.getElementById('l-password').value };
...
function renderDashboard() {
    ...
    const el = document.getElementById('overall-circle');
    el.style.setProperty('--pct', `${ov}%`);

    // Stats bars
    const stats = ['tiro','velocita','dribbling','fisico','mentalita'];
    stats.forEach(s => {
...
function renderGame() {
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        dot.className = 'month-dot' + (i < p.mese_corrente ? ' done' : i === parseInt(p.mese_corrente) ? ' current' : '');
        dotsEl.appendChild(dot);
    }
    document.getElementById('current-month-text').textContent = `${getMeseName(p.mese_corrente)} Anno ${p.anno_corrente} · Età ${p.age}`;
...
function updatePlayBtn() {
    ...
    btn.textContent = selectedActions.length > 0 ? `⚽ Gioca Mese (${selectedActions.length} azioni)` : '⚽ Seleziona azioni';
}
...
async function loadCareer() {
    ...
    el.innerHTML = res.map(s => {
        let badge = '';
        if (s.pallone_doro_pos === 1) badge = '<span class="badge-gold">🏆 1°</span>';
...
async function loadTransfer() {
    ...
    el.innerHTML = teams.map(t => {
        ...
        return `<div class="team-card ${isCurrent ? 'current' : ''}">
            ...
            ${isCurrent 
                ? '<div style="text-align:center;color:var(--green);font-weight:700">✅ Squadra attuale</div>'
                : `<button class="btn-transfer" ${!canJoin ? 'disabled title="Overall insufficiente"' : ''} onclick="transferTo(${t.id}, '${t.nome}')">
                    ${canJoin ? '🔄 Trasferisciti' : `🔒 Richiede OVR ${minOv}`}
                  </button>`
            }
        </div>`;
    }).join('');
...
async function transferTo(teamId, teamName) {
    if (!confirm(`Vuoi trasferti a ${teamName}?`)) return;
