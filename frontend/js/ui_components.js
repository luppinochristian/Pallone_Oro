/**
 * Golden Striker — UI Components Library
 * Reusable, animated UI building blocks for the entire game
 */
...
const GS_UI = (() => {
    ...
    function confirm(message, onConfirm, onCancel, opts = {}) {
        ...
        `;
        const confirmLabel = opts.confirmLabel || (lang === 'en' ? 'Confirm' : 'Conferma');
        const cancelLabel  = opts.cancelLabel  || (lang === 'en' ? 'Cancel'  : 'Annulla');
    ...
    function buildStatBar(label, value, max = 125, color = '#FFD700') {
        ...
        div.innerHTML = `
            ...
            <div style="height:5px;background:var(--bg3);border-radius:3px;overflow:hidden">
                <div class="gs-ui-bar-fill" style="height:100%;width:0%;background:linear-gradient(90deg,${color},${color}88);border-radius:3px;transition:width 0.7s cubic-bezier(0.34,1.56,0.64,1)"></div>
            </div>`;
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
        ...
        div.innerHTML = `
            ...
            <div style="flex:1;min-width:0">
                ...
                <div style="font-size:0.72rem;color:var(--text-dim);margin-top:1px">
                    ${lang === 'en' ? 'Age' : 'Età'}: ${player.age || '—'} · ${player.lega_nome || '—'}
                </div>
            </div>
    ...
    function copyToClipboard(text, successMsg) {
        navigator.clipboard?.writeText(text).then(() => {
            const lang = localStorage.getItem('gs_lang') || 'it';
            toast(successMsg || (lang === 'en' ? 'Copied!' : 'Copiato!'), 'success', 2000);
        });
    }
    ...
    return {
        toast, showProgressModal, confirm, showTip, hideTip,
        makeCollapsible, formatNumber, formatMoney, renderStars,
        buildStatBar, buildCard, buildProfileHeader, buildEmptyState,
        typeText, buildProgressRing, copyToClipboard,
    };
})();
