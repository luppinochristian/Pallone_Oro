...
const GS_Charts = (() => {
    ...
    function radarChart(container, stats, opts = {}) {
        ...
        const W = size, H = size;
        const MARGIN = 36; // margin for labels
        const cx = W / 2, cy = H / 2;
        ...
        // Labels — adaptive font size and padding for long names
        ctx.save();
        const labelPad = MARGIN * 0.85; // fixed padding matching margin
        for (let i = 0; i < n; i++) {
            const angle = (2 * Math.PI * i) / n - Math.PI / 2;
            const cosA = Math.cos(angle);
            const sinA = Math.sin(angle);
            const lx = cx + cosA * (r + labelPad);
            const ly = cy + sinA * (r + labelPad * 0.8);

            // Adaptive font: shorter for long labels
            const label = keys[i];
