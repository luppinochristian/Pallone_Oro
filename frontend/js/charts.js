...
const GS_Charts = (() => {
    ...
    function getCanvas(id, w, h) {
        ...
        c.width  = w || c.parentElement?.offsetWidth || 400;
        c.height = h || 200;
        c.style.width  = '100%';
    ...
    function barChart(container, data, opts = {}) {
        const canvas = getCanvas(opts.id || 'gs-bar-chart', container.offsetWidth, opts.height || 220);
    ...
    function lineChart(container, datasets, opts = {}) {
        const canvas = getCanvas(opts.id || 'gs-line-chart', container.offsetWidth, opts.height || 220);
        container.appendChild(canvas);
        const ctx = canvas.getContext('2d');
        const W = canvas.width, H = canvas.height;
