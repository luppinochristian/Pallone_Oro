...
async function renderChampions(player) {
        ...
        gruppiKeys.forEach(g => {
            ...
            html += `<div class="champ-group-card" data-gruppo="${g}">
                ...
                <table class="champ-group-table">
                    <thead><tr>
                        <th>Squadra</th>
                        <th title="Vittorie">V</th><th title="Pareggi">P</th><th title="Sconfitte">S</th>
                        <th title="Gol Fatti">GF</th><th title="Gol Subiti">GS</th>
                        <th title="Differenza Reti">DR</th><th title="Punti">Pt</th>
                    </tr></thead><tbody>`;
            teams.forEach((t, i) => {
    ...
    requestAnimationFrame(() => {
        ...
        let bd = delay + 60;
        wrap.querySelectorAll('.bracket-team').forEach(el => {
...
function _renderBracket(bracket, player) {
    ...
    const teamCard = (t) => {
        if (!t) return `<div class="ucl-team ucl-team-tbd"><span class="ucl-name" style="color:var(--text-dim);font-style:italic">TBD</span></div>`;
        const isMe    = t.team_id == player.team_id;
        const elim    = t.eliminato == 1 || t.eliminato === '1';
        const winner  = t.fase === 'vincitore';
        const flag    = _buildFlag(t.nazione_nome || '');
        return `<div class="ucl-team ${isMe ? 'ucl-me' : ''} ${elim ? 'ucl-elim' : 'ucl-active'} ${winner ? 'ucl-winner' : ''}">
            <span class="ucl-flag">${flag}</span>
            <span class="ucl-name">${t.team_nome}</span>
            <span class="ucl-ovr">${t.ovr}</span>
            ${isMe  ? '<span class="my-team-badge" style="font-size:0.55rem;padding:1px 4px">TU</span>' : ''}
            ${elim  ? '<span class="ucl-x">✕</span>' : ''}
            ${winner ? '<span class="ucl-crown">🏆</span>' : ''}
        </div>`;
    };
    ...
    // Fix Bug 6: filtra solo per posizione esatta, senza fallback
    ...
    const getByGruppo = (arr, gruppo) => arr.find(t => t.gruppo === gruppo) || null;

    // ── PLAYOFF: A2 vs C3, B2 vs D3, C2 vs A3, D2 vs B3 ──
    const playoffMatches = [
        [getByGruppo(playoff2,'A'), getByGruppo(playoff3,'C')],
        [getByGruppo(playoff2,'B'), getByGruppo(playoff3,'D')],
        [getByGruppo(playoff2,'C'), getByGruppo(playoff3,'A')],
        [getByGruppo(playoff2,'D'), getByGruppo(playoff3,'B')],
    ].filter(([a,b]) => a || b); // mostra solo se almeno una squadra esiste

    // ── OTTAVI: ogni 1ª vs TBD (il vincitore del playoff arriverà) ──
    const ottaviByGruppo = {};
    ottavi.forEach(t => { if (t.gruppo) ottaviByGruppo[t.gruppo] = t; });

    // Abbina 1e ai vincitori playoff cross-gruppo:
    // A1 vs vin(B2/D3), B1 vs vin(A2/C3), C1 vs vin(D2/B3), D1 vs vin(C2/A3)
    // I vincitori playoff hanno già cambiato fase in 'ottavi', cerchiamoli per gruppo sorgente
    const playoffWinners = (byFase['ottavi'] || []).filter(t => !t.gruppo || parseInt(t.posizione_gruppo) > 1);
    const getPlayoffWinner = (gruppoProvenienza3, gruppoProvenienza2) => {
        // Cerca in ottavi la squadra che era playoff/playoff_3 di quei gruppi
        return ottavi.find(t =>
            (t.gruppo === gruppoProvenienza2 && parseInt(t.posizione_gruppo) === 2) ||
            (t.gruppo === gruppoProvenienza3 && parseInt(t.posizione_gruppo) === 3)
        ) || null;
    };

    const ottaviMatches = [
        [ottaviByGruppo['A'], getPlayoffWinner('D','B')],
        [ottaviByGruppo['B'], getPlayoffWinner('C','A')],
        [ottaviByGruppo['C'], getPlayoffWinner('B','D')],
        [ottaviByGruppo['D'], getPlayoffWinner('A','C')],
    ].filter(([a,b]) => a || b);

    // ── QUARTI, SEMI ──
    const quartiMatches = chunkPairs(quarti);
    const semiMatches   = chunkPairs(semi);

    // ── FINALE + VINCITORE ──
    // Fix Bug 4: separali — il vincitore è il risultato della finale
    const finaleMatches = chunkPairs(finale);
    const winnerMatch   = vincitore.length ? [[vincitore[0], null]] : [];

    // Colonne del bracket
    const col = (title, content, cls = '') =>
        `<div class="ucl-col ${cls}">
            <div class="ucl-col-title">${title}</div>
            <div class="ucl-col-body">${content || '<div class="ucl-tbd">In attesa...</div>'}</div>
        </div>`;

    const cols = [];

    if (playoffMatches.length)
        cols.push(col('⚡ Playoff',
            playoffMatches.map(([a,b]) => matchBox(a, b)).join('')));

    if (ottaviMatches.length)
        cols.push(col('🔵 Ottavi di Finale',
            ottaviMatches.map(([a,b]) => matchBox(a, b, a ? `1° Gr.${a.gruppo}` : '')).join('')));

    if (quartiMatches.length)
        cols.push(col('⚽ Quarti di Finale',
            quartiMatches.map(([a,b]) => matchBox(a, b)).join('')));

    if (semiMatches.length)
        cols.push(col('🥈 Semifinale',
            semiMatches.map(([a,b]) => matchBox(a, b)).join('')));

    if (finaleMatches.length || winnerMatch.length) {
        const finContent = [
            ...finaleMatches.map(([a,b]) => matchBox(a, b)),
            ...winnerMatch.map(([a]) => `<div class="ucl-winner-banner">${teamCard(a)}</div>`)
        ].join('');
        cols.push(col('🏆 Finale', finContent, 'ucl-col-finale'));
    }

    return `<div class="ucl-bracket">${cols.join('<div class="ucl-connector">▶</div>')}</div>`;
}
