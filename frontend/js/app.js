...
async function loadTransfer() {
    ...
    el.innerHTML = '<div style="text-align:center;padding:48px;color:var(--text-dim)">Caricamento squadre...</div>';

    // Fetch leghe e player in parallelo
    let leghe, p;
    try {
        [leghe, p] = await Promise.all([
            api('player.php', { action: 'leghe' }, 'GET'),
            api('player.php', { action: 'get', career_id: currentCareerId }, 'GET')
        ]);
    } catch(e) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Errore di rete.</div>';
        return;
    }

    if (!p || p.error) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Errore giocatore: ' + (p?.error || '?') + '</div>';
        return;
    }
    if (!Array.isArray(leghe) || leghe.length === 0) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Nessuna lega trovata.</div>';
        return;
    }
    ...
    const nazioniMap = {};
    leghe.forEach(l => {
        const nid = l.nazione_id;
        if (!nazioniMap[nid]) nazioniMap[nid] = { id: nid, nome: l.nazione_nome, bandiera: l.bandiera || '🌍', leghe: [] };
        nazioniMap[nid].leghe.push(l);
    });
    const nazioni = Object.values(nazioniMap);

    // Determina nazione corrente del giocatore
    const myLega = leghe.find(l => l.id == p.lega_id);
    const myNazioneId = myLega ? myLega.nazione_id : nazioni[0].id;
    if (!selectedNazioneId) selectedNazioneId = myNazioneId;

    // Assicura che selectedNazioneId esista
    if (!nazioniMap[selectedNazioneId]) selectedNazioneId = myNazioneId;

    const nazCorrente = nazioniMap[selectedNazioneId];
    const legheNazione = nazCorrente.leghe;

    // Determina lega da mostrare
    const myLegaInNaz = legheNazione.find(l => l.id == p.lega_id);
    if (!selectedLegaId || !legheNazione.find(l => l.id == selectedLegaId)) {
        selectedLegaId = myLegaInNaz ? myLegaInNaz.id : legheNazione[0].id;
    }

    // Fetch squadre della lega selezionata
    const teams = await api('player.php', { action: 'teams', lega_id: selectedLegaId }, 'GET');
    if (!Array.isArray(teams)) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Errore squadre.</div>';
        return;
    }
    ...
    nazioni.forEach(n => {
        const isActive = n.id == selectedNazioneId;
        const isMine = n.id == myNazioneId;
        html += `<button class="nazione-tab ${isActive ? 'active' : ''} ${isMine ? 'current-naz' : ''}"
            onclick="selectNazione(${n.id})">${n.bandiera} ${n.nome}${isMine ? ' ★' : ''}</button>`;
    });
    html += '</div>';

    html += '<div class="lega-subtabs">';
    legheNazione.forEach(l => {
        const isActive = l.id == selectedLegaId;
        const isMine = l.id == p.lega_id;
        html += `<button class="lega-subtab ${isActive ? 'active' : ''} ${isMine ? 'current-lega' : ''}"
            onclick="filterLega(${l.id})">${l.livello == 1 ? '🥇' : '🥈'} ${l.nome}${isMine ? ' (tua lega)' : ''}</button>`;
    });
    html += '</div>';
