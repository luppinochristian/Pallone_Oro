...
const TRANSLATIONS = {
    it: {
        ...
        create_start_btn:'🌟 Inizia questa Carriera!',
        ...
        ai_describe_sub:"Scrivi le caratteristiche fisiche in italiano — capelli, occhi, carnagione. L'AI genererà un ritratto.",
        ai_generate_btn:'🎨 Genera Immagine', ai_regenerate_btn:'🔄 Rigenera',
        ai_powered:'Powered by Pollinations.ai · Gratuito',
        ...
        match_pts:'pt',
        energy_click:'Energia a zero! Riposati prima di allenarti.',
        ...
        forgot_pw_hint:'Inserisci la tua email e ti manderemo le istruzioni per reimpostare la password.',
        ...
        dash_activities_title:'📋 Ultime Attività',
        ...
        career_best_goals:'MIGLIOR PARTITA PER GOL',
        career_best_assists:'MIGLIOR PARTITA PER ASSIST',
        ...
        champions_not_started:"🏆 La Champions Cup non è ancora iniziata. Le squadre si qualificano a settembre.",
    ...
    en: {
        ...
        ai_describe_sub:'Write physical characteristics — hair, eyes, skin tone. AI will generate a portrait.',
        ai_generate_btn:'🎨 Generate Image', ai_regenerate_btn:'🔄 Regenerate',
        ai_powered:'Powered by Pollinations.ai · Free',
        ...
        dash_objectives:'🎯 Season Objectives',
        ...
        match_pts:'pts',
        energy_click:'Energy at zero! Rest before training.',
        ...
        forgot_pw_hint:'Enter your email and we will send you instructions to reset your password.',
        ai_describe_sub_html:'Describe physical features — hair, eyes, skin tone, expression. The AI (Pollinations) will generate a portrait.',
        dash_activities_title:'📋 Recent Activity',
        ...
        career_best_goals:'BEST MATCH (GOALS)',
        career_best_assists:'BEST MATCH (ASSISTS)',
        ...
        champions_not_started:'🏆 Champions Cup not started yet. Teams qualify in September.',
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    _buildActionsGrid();
    // Initialize crowd animation canvas
    if (window.GS_Crowd) {
        const crowdCanvas = document.getElementById('gs-crowd-canvas');
        if (crowdCanvas) { GS_Crowd.init(crowdCanvas); GS_Crowd.start(); }
    }
    // AI avatar prompt
...
function renderDashboard() {
    ...
    if (window.GS_Locale) {
        const m = GS_Locale.getMilestone(parseInt(p.overall));
    ...
    if (statCard && !document.getElementById('player-radar-chart')) {
        ...
        radarWrap.style.cssText = 'margin-top:16px;padding:8px;height:200px';
    ...
    if (currentPlayer && window.GS_Charts) {
        ...
        if (radarContainer) {
            ...
            GS_Charts.radarChart(radarContainer, {
                'Tiro': p.tiro, 'Vel': p.velocita, 'Drib': p.dribbling,
                'Fis': p.fisico, 'Men': p.mentalita
            }, { id: 'gs-radar', size: 180 });
        }
    }
}
...
function renderGame() {
    ...
    // Inject stadium background on first render
    const stadContainer = document.getElementById('gs-stadium-container');
    if (stadContainer && !stadContainer.innerHTML && window.GS_Animations) {
        stadContainer.innerHTML = GS_Animations.createStadiumSVG();
    }
    const p = currentPlayer;
...
// Actions grid built by _buildActionsGrid() inside DOMContentLoaded
    ...
    if (window.GS_Charts && seasons?.length > 1) {
        ...
        if (p) {
            ...
            GS_Charts.radarChart(radarBox, {
                'Tiro': parseInt(p.tiro || 0),
                'Vel': parseInt(p.velocita || 0),
                'Drib': parseInt(p.dribbling || 0),
                'Fis': parseInt(p.fisico || 0),
                'Men': parseInt(p.mentalita || 0),
            }, { id: 'career-radar', size: 200 });
...
function getStrutturaName(lvl) {
    const nomi_it = ['Nessuna','Campetto Base','Spogliatoio Attrezzato','Palestra e Campo','Centro Sportivo','Centro High-Tech','Centro d\'Elite','Academy Personale'];
    const nomi_en = ['None','Basic Pitch','Equipped Dressing Room','Gym & Field','Sports Centre','High-Tech Centre','Elite Centre','Personal Academy'];
    const nomi = currentLang === 'en' ? nomi_en : nomi_it;
    return nomi[parseInt(lvl)] || (currentLang === 'en' ? 'None' : 'Nessuna');
}
