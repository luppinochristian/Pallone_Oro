...
async function renderChampions(player) {
    ...
    if (gruppiKeys.length) {
        ...
        gruppiKeys.forEach(g => {
            ...
            teams.forEach((t, i) => {
                ...
                // Usa abbreviazione lega invece della bandiera (emoji non supportate su Windows)
                const legaTag = t.lega_nome ? `<span class="champ-lega-tag">${t.lega_nome.substring(0,3).toUpperCase()}</span>` : '';
                html += `<tr class="${qualClass} ${isMe ? 'champ-g-me' : ''}">
                    <td><span class="champ-team-name">${legaTag} ${t.team_nome}</span>${isMe ? ' <span class="my-team-badge">TU</span>' : ''}</td>
    ...
    // Stagger: righe gironi e team bracket entrano in sequenza
    requestAnimationFrame(() => {
        // Righe tabelle gironi: tutte le tr di ogni gruppo, delay progressivo globale
        let delay = 40;
        wrap.querySelectorAll('.champ-group-table tbody tr').forEach(tr => {
            setTimeout(() => tr.classList.add('champ-row-visible'), delay);
            delay += 50;
        });
        // Team del bracket: stagger più rapido
        let bracketDelay = delay + 60;
        wrap.querySelectorAll('.champions-team').forEach(el => {
            setTimeout(() => el.classList.add('champ-row-visible'), bracketDelay);
            bracketDelay += 40;
        });
    });
}
