...
function buildHairColorPicker() {
    ...
    requestAnimationFrame(() => {
        ...
        const gc = document.getElementById('cp-hair-grad');
        if (gc) updateGradCursor('hair', (s/100)*gc.width, (1-v/100)*gc.height);
        const hc = document.getElementById('cp-hair-hue');
        if (hc) updateHueCursor('hair', (h/360)*hc.width);
    });
}
...
function buildEyePicker() {
    ...
    requestAnimationFrame(() => {
        ...
        const gc = document.getElementById('cp-eye-grad');
        if (gc) updateGradCursor('eye', (s/100)*gc.width, (1-v/100)*gc.height);
        const hc = document.getElementById('cp-eye-hue');
        if (hc) updateHueCursor('eye', (h/360)*hc.width);
    });
}
...
function initColorPickerCanvas(type, h, s, v, hex) {
    ...
    if (gradCanvas) {
        ...
        updateGradCursor(type, (s/100)*W, (1-v/100)*H);
    ...
    if (hueCanvas) {
        updateHueCursor(type, (h/360)*hueCanvas.width);
        hueCanvas.onmousedown = (e) => {
            const move = (ev) => {
                ...
                if (cur && gradCanvas) {
                    ...
                    const ns=Math.round((cx2/gradCanvas.width)*100);
                    const nv=Math.round((1-(cy2/gradCanvas.height))*100);
...
function onPickerHexInput(type, val) {
    ...
    if (val.length === 6) {
        ...
        const gc = document.getElementById(`cp-${type}-grad`);
        if (gc) updateGradCursor(type, (s/100)*gc.width, (1-v/100)*gc.height);
        const hc = document.getElementById(`cp-${type}-hue`);
        if (hc) updateHueCursor(type, (h/360)*hc.width);
...
function showResults(res) {
    ...
    if (altriFiltered.length) {
        html += `<div class="res-section"><div class="res-section-title">📋 Altro</div>
            <div class="res-row-wrap">${altriFiltered.map(r=>`<div class="res-chip">${r}</div>`).join('')}</div></div>`;
    }
...
async function loadCareer() {
    ...
    el.innerHTML = res.map(s => {
        ...
        return `<tr>
            ...
            <td>${s.gol}</td><td>${s.assist}</td><td>${s.partite}</td>
            <td>${s.media_voto}</td>
...
function renderTransferUI(teams, p, leghe) {
    ...
    const agScontoOvr = p.agent_ovr_sconto || 0;
    const ovrEffettivo = Math.max(0, ovr - agScontoOvr);
    ...
    if (teamsFiltrati.length === 0) {
        ...
        sorted.forEach(t => {
            ...
            const canTransfer = !isCurrent && ovrEffettivo >= teamOvr - 5;
            const stipendio = Math.round(parseInt(t.moltiplicatore_stipendio || 100) * (ovr * 500) / 100);
            cardsHtml += `<div class="team-card ${isCurrent?'current':''}">
                ...
                <div class="team-stats">
                    ...
                    <div class="team-stat"><div class="ts-label">💰 Stipendio</div><div class="ts-val">${formatMoney(stipendio)}/mese</div></div>
                    <div class="team-stat"><div class="ts-label">🏟️ Popolarità</div><div class="ts-val">${t.popolarita}</div></div>
                    <div class="team-stat"><div class="ts-label">🎯 OVR richiesto</div><div class="ts-val">${teamOvr - 5}${agScontoOvr>0?` <small style="color:var(--green)">(-${agScontoOvr}%)</small>`:''}</div></div>
                </div>
                ${isCurrent
                    ...
                    : `<button class="btn-transfer" ${canTransfer?'':'disabled'}
                        ...
                        ${canTransfer ? '✈️ Trasferisciti' : `🔒 OVR ${teamOvr - 5} richiesto`}
                      </button>`
                }
            </div>`;
        });
    ...
    el.innerHTML = `
        ...
        ${agScontoOvr > 0 ? `<div style="background:rgba(0,200,100,0.1);border:1px solid var(--green);border-radius:8px;padding:8px 14px;margin-bottom:14px;font-size:0.82rem;color:var(--green)">🤝 Agente attivo: -${agScontoOvr} OVR richiesto per il trasferimento</div>` : ''}
        ${cardsHtml}`;
}

function selectTrNaz(nazId) {
    _trNaz = nazId;
    _trLiv = 1;
    // Ricarica con i dati già in memoria se possibili
    api('player.php', { action: 'teams' }, 'GET').then(teams => {
        api('player.php', { action: 'get', career_id: currentCareerId }, 'GET').then(p => {
            api('player.php', { action: 'leghe' }, 'GET').then(leghe => {
...
async function renderChampions(player) {
    ...
    data.forEach(t => { if (fasi[t.fase]) fasi[t.fase].push(t); });
