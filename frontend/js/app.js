...
function showPage(page) {
    ...
    // Pulsante Esci: visibile su tutto tranne auth puro (login/register)
    ...
    // "Menu Principale" solo se si è dentro una carriera (non nella schermata selezione carriere)
...
function showCareerSelect(careers, email) {
    ...
    // Mostra Esci in alto a destra, ma senza "Menu Principale"
    const exitWrap = document.getElementById('exit-dropdown-wrap');
    const exitOptHome = document.getElementById('exit-opt-home');
    const exitDivHome = document.getElementById('exit-divider-home');
...
async function doCreateCareer() {
    ...
    // Piede forte = quello scelto, debole = l'altro. Livelli fissi: forte 3/5, debole 2/5
...
function renderDashboard() {
    ...
    // Piede forte / debole / livello skill
...
function showResults(res) {
    ...
    // Categorizza i risultati testuali
    const allenamento = [];
    const infortuni   = [];
    const altri       = [];
    const champions   = [];
    ...
    (res.risultati || []).forEach(r => {
        if (r.includes('INFORTUN') || r.includes('infortun')) infortuni.push(r);
    ...
    if (res.pallone_doro?.pos > 0)
        html += `<div class="res-banner gold-banner">🏆 ${res.pallone_doro.msg}</div>`;
    ...
    if (allenamento.length) {
        html += `<div class="res-section">
            <div class="res-section-title">🏋️ Allenamento</div>
            <div class="res-row-wrap">
                ${allenamento.map(r => `<div class="res-chip">${r}</div>`).join('')}
            </div>
        </div>`;
    }
    ...
    // ── INFORTUNI ──
    if (infortuni.length) {
        html += `<div class="res-section infortuni-section">
            <div class="res-section-title">🚑 Infortuni</div>
            ${infortuni.map(r => `<div class="res-chip danger">${r}</div>`).join('')}
        </div>`;
    }
    ...
    if (legaMsgs.length) {
        ...
        html += `<div class="res-section">
            <div class="res-section-title">⚽ Partite del mese</div>
            <div class="res-matches">`;
        legaMsgs.forEach((m, i) => {
            const color = esitoColor[m.esito] || '#aaa';
            const label = esitoLabel[m.esito] || m.esito;
            html += `<div class="res-match-card" style="border-top:3px solid ${color}">
                ...
                <div class="res-match-pts">+${m.punti} pt</div>
                <div class="res-match-stats">
                    <span>⚽ ${match.gol ?? 0} gol</span>
                    <span>🎯 ${match.assist ?? 0} assist</span>
                    <span>📊 ${match.voto ?? '-'}</span>
                </div>
            </div>`;
        });
        html += `</div></div>`;
    ...
    if (champions.length) {
        html += `<div class="res-section">
            <div class="res-section-title">🏆 Champions Cup</div>
            ${champions.map(r => `<div class="res-chip">${r}</div>`).join('')}
        </div>`;
    }

    // ── ALTRI (stipendio, premi, eventi, fine stagione) ──
    const altriFiltered = altri.filter(r => !r.startsWith('⚽ G'));
    if (altriFiltered.length) {
        html += `<div class="res-section">
            <div class="res-section-title">📋 Altro</div>
            <div class="res-row-wrap">
                ${altriFiltered.map(r => `<div class="res-chip">${r}</div>`).join('')}
            </div>
        </div>`;
    }
...
async function loadTransfer() {
    ...
    const [teamsRaw, p, leghe] = await Promise.all([
        api('player.php', { action: 'teams' }, 'GET'),
        api('player.php', { action: 'get', career_id: currentCareerId }, 'GET'),
        api('player.php', { action: 'leghe' }, 'GET')
    ]);

    if (!p || p.error || !Array.isArray(teamsRaw) || teamsRaw.length === 0) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Errore caricamento squadre. Ricarica la pagina.</div>';
        return;
    }
    ...
    const legheMap = {};
    if (Array.isArray(leghe)) leghe.forEach(l => { legheMap[l.id] = l; });

    const myOvr = parseInt(p.overall || 50);
    const stelle = ['', '⭐', '⭐⭐', '⭐⭐⭐', '⭐⭐⭐⭐', '⭐⭐⭐⭐⭐'];
    const stelleReq = { 1: 55, 2: 75, 3: 90, 4: 105, 5: 120 };

    // Raggruppa per lega
    const byLega = {};
    teamsRaw.forEach(t => {
        const lid = t.lega_id;
        if (!byLega[lid]) byLega[lid] = { lega: legheMap[lid] || { nome: 'Lega ' + lid, livello: 1, nazione_nome: '' }, squadre: [] };
        byLega[lid].squadre.push(t);
    });

    let html = '<div style="display:flex;flex-direction:column;gap:28px;">';

    Object.values(byLega).forEach(({ lega, squadre }) => {
        const isCurrent = squadre.some(t => t.id == p.team_id);
        html += `<div>
            <div style="font-size:0.78rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--border)">
                ${lega.livello == 1 ? '🥇' : '🥈'} ${lega.nome}
                <span style="opacity:.5;margin-left:6px;font-weight:400">${lega.nazione_nome || ''}</span>
                ${isCurrent ? '<span style="color:var(--gold);margin-left:8px">← tua lega</span>' : ''}
            </div>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:10px;">`;

        squadre.forEach(t => {
            const isMine = t.id == p.team_id;
            const minOvr = stelleReq[t.stelle] || 55;
            const canJoin = myOvr >= minOvr;
            const borderColor = isMine ? 'var(--gold)' : canJoin ? 'var(--border)' : 'rgba(255,255,255,0.05)';
            const safeName = (t.nome || '').replace(/'/g, "\\'");
            html += `<div style="background:var(--bg3);border:1px solid ${borderColor};border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:5px;${canJoin ? '' : 'opacity:0.45'}">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:4px">
                    <div style="font-weight:700;font-size:0.9rem;color:${isMine ? 'var(--gold)' : 'var(--text)'};">${t.nome}</div>
                    <div style="font-size:0.75rem;white-space:nowrap">${stelle[t.stelle] || ''}</div>
                </div>
                <div style="font-size:0.72rem;color:var(--text-dim)">Budget: <strong>${formatMoney(t.budget)}</strong></div>
                <div style="font-size:0.72rem;color:var(--text-dim)">OVR min: <strong style="color:${canJoin ? '#4ade80' : '#f87171'}">${minOvr}</strong> &nbsp;(tuo: ${myOvr})</div>
                <div style="font-size:0.72rem;color:var(--text-dim);margin-bottom:4px">🎯 ${t.obiettivo || '—'}</div>
                ${isMine
                    ? '<div style="text-align:center;padding:7px;background:rgba(255,200,0,.1);border-radius:8px;color:var(--gold);font-size:0.8rem;font-weight:700">✅ Squadra attuale</div>'
                    : `<button onclick="transferTo(${t.id},'${safeName}',${canJoin ? 'true' : 'false'})"
                         style="padding:8px;border-radius:8px;border:1px solid ${canJoin ? 'var(--gold)' : 'transparent'};background:${canJoin ? 'rgba(255,200,0,.1)' : 'rgba(255,255,255,0.03)'};color:${canJoin ? 'var(--gold)' : 'var(--text-dim)'};font-size:0.82rem;cursor:${canJoin ? 'pointer' : 'not-allowed'};font-weight:600;width:100%">
                         ${canJoin ? '🔄 Trasferisciti' : '🔒 OVR troppo basso'}
                       </button>`
                }
            </div>`;
        });
...
async function transferTo(teamId, teamName, canJoin) {
    if (!canJoin) { toast('OVR troppo basso per questa squadra', 'error'); return; }
    if (!confirm('Trasferirsi a ' + teamName + '?')) return;
    const res = await api('game.php', { action: 'change_team', team_id: teamId });
    if (res.error) { toast(res.error, 'error'); return; }
    toast(res.msg || 'Trasferito!', 'gold');
    await loadPlayer();
    loadTransfer();
    renderDashboard();
}
...
async function loadSkillTree() {
    ...
    // Update piede forte/debole/livello skill panel
