...
function _renderBracket(bracket, player) {
    ...
    const matchCard = (a, b, matchPhaseRank) => {
        ...
        // Vince questo match = ha avanzato OLTRE il rank di questa fase
        const aWon  = rankA > matchPhaseRank;
        const bWon  = rankB > matchPhaseRank;

        // Ha perso IN QUESTA fase = rank uguale alla fase E eliminato
    ...
    const poWinners = ottAll.filter(t => {
        const pos = parseInt(t.posizione_gruppo);
        return pos === 2 || pos === 3;
    });
    // Per ogni ottavo: trova il vincitore del playoff cross-gruppo
    // Accoppiamenti: A2-C3, B2-D3, C2-A3, D2-B3 → ottavi: A1 vs vincitore(B2/D3), ecc.
    const poW = (g2, g3) =>
        poWinners.find(t => t.gruppo === g2) ||
        poWinners.find(t => t.gruppo === g3) || null;
    ...
    const winnerOf = (a, b) => {
        ...
        if (rA > rB) return a;
        if (rB > rA) return b;
        // Stesso rank: chi non è eliminato ha avanzato
        if (isElim(a) && !isElim(b)) return b;
        if (isElim(b) && !isElim(a)) return a;
        return a; // entrambi ancora in gioco → TBD
    };
