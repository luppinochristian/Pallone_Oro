...
function showResults(res) {
    ...
    (res.risultati || []).forEach(r => {
        if (r.includes('INFORTUN') || r.includes('infortun')) infortuni.push(r);
                 ...
                 r.startsWith('🔥'))       allenamento.push(r);
        // Salta il riepilogo ⚽ del match (gol/assist/voto) — già mostrato nelle card partite
        else if (r.startsWith('⚽ ')) { /* skip: ridondante con le card partite */ }
...
function fixNewsBoxHeight() {
    ...
    box.style.maxHeight = (window.innerHeight - topOffset - 24) + 'px';
}
window.addEventListener('resize', fixNewsBoxHeight);
document.addEventListener('DOMContentLoaded', fixNewsBoxHeight);
// Chiamato anche ogni volta che si naviga alla dashboard
const _origShowPage = showPage;
function showPage(page) {
    _origShowPage(page);
    if (page === 'dashboard') setTimeout(fixNewsBoxHeight, 50);
}
