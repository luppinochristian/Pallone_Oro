...
function renderCareerList(careers) {
    ...
    el.innerHTML = careers.map(c => `
        <div class="career-card" onclick="selectCareer(${c.id})">
            <div class="career-avatar">${renderAvatarSVG(c.skin_color||'medium',c.skin_hair||'short_black',c.eye_color||'brown',c.gender||'male',52,c.hair_color||'#1a1a1a')}</div>
...
function renderGame() {
    ...
    // Mesi percorsi nella stagione
    const percorsi = currentIdx > -1 ? currentIdx : 0;
    document.getElementById('current-month-text').textContent =
        `${MONTH_ABBR[meseCorrente]} · Stagione ${annoStagione} (${percorsi}/10) · Età ${p.age}`;
...
function _renderBracket(bracket, player) {
    ...
    const teamRowMatch = (t, winner, lostHere) => {
        ...
        return `<div class="${cls}">
            ...
            <span class="bk-right">
                ...
                ${champ     ? '<span class="bk-trophy-ico">🏆</span>' : ''}
                ${winner && !champ ? '<span class="bk-adv">›</span>' : ''}
            </span>
        </div>`;
    };
    ...
    const matchCard = (a, b, matchPhaseRank) => {
        ...
        if      (rankA > rankB)        { aWon = true; }         // A avanzato oltre
            ...
            // Se rank > mr, sono squadre che si sono incontrate in una fase successiva
            // (wrong pairing): nessun vincitore determinabile → ⏳.
            if (rankA === matchPhaseRank && rankB === matchPhaseRank) {
                const ovrA = parseInt(a?.ovr || 0);
                const ovrB = parseInt(b?.ovr || 0);
                if      (ovrA > ovrB) { bWon = true; }
                else if (ovrB > ovrA) { aWon = true; }
            }
            // Se rank > mr per almeno uno: ⏳ (non si sono incontrate qui)
        }

        // ❌ solo se:
        // 1. L'avversario ha vinto (bWon/aWon)
        // 2. Il team è eliminato (aElim/bElim)
        // 3. Il team è stato eliminato IN QUESTA fase o prima (rankA <= matchPhaseRank)
        //    Se rank > matchPhaseRank → ha vinto questa fase, non deve mostrare ❌ qui.
    ...
    if (ottPairs.length)
        phases.push({ icon:'🔵', label:'Ottavi', sub:`${ottPairs.length} match`, pairs: ottPairs, rank: 1 });

    // ─── Quarti/Semi/Finale: accoppiamenti derivati dagli ottavi ────────
    // Ricostruisce il bracket completo dai vincitori delle fasi precedenti.
    // winnerOf(a,b): restituisce chi ha avanzato oltre il loro match comune.
    // winnerOf: chi ha vinto il match tra a e b?
    // Chi ha rank STRETTAMENTE più alto ha avanzato oltre (independentemente dall'eliminato).
    // Stesso rank: chi è eliminato ha perso, chi non lo è avanza.
    // Entrambi stessa condizione → None (match non ancora giocato o pairing sbagliato).
    const winnerOf = (a, b) => {
        if (!a) return b; if (!b) return a;
        const rA = phaseRank(a), rB = phaseRank(b);
        if (rA > rB) return a;          // A ha rank più alto = ha avanzato oltre
        if (rB > rA) return b;          // B ha rank più alto = ha avanzato oltre
        const eA = isElim(a), eB = isElim(b);
        if (eA && !eB) return b;        // A eliminato, B avanza
        if (eB && !eA) return a;        // B eliminato, A avanza
        return null;                    // indeterminabile: ⏳ (match non ancora giocato)
    };

    // ottPairs ha sempre 4 match nell'ordine:
    // [0] A1 vs poW(B2/D3)  [1] B1 vs poW(A2/C3)  [2] C1 vs poW(D2/B3)  [3] D1 vs poW(C2/A3)
    // Quarti: winner[0]vsWinner[2], winner[1]vsWinner[3]  (metà alta vs metà bassa)
    // Semifinale: winner[QUA0]vsWinner[QUA1]
    // Finale: winner[SEMI0] vs winner[SEMI1] (sempre 1 match)

    // Espandi ottPairs a sempre 4 slot (null per quelli mancanti)
    // così w[0..3] sono sempre accessibili senza undefined
    const ottPairsFull = [
        ottPairs[0] || [null, null],
        ottPairs[1] || [null, null],
        ottPairs[2] || [null, null],
        ottPairs[3] || [null, null],
    ];
    const w = ottPairsFull.map(([a,b]) => winnerOf(a, b));  // sempre 4 slot
    ...
    if (semFinal.length)  phases.push({ icon:'🥈', label:'Semifinale', sub:'Ultime 4',         pairs: semFinal,  rank: 3 });
    if (finFinal.length || vicAll.length)
        phases.push({ icon:'🥇', label:'Finale', sub:'La Grande Finale', pairs: finFinal, vincitore: vicAll[0] || null, rank: 4 });
    ...
    return `<div class="bk-bracket">${html}</div>`;
}
