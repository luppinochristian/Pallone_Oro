...
function _applyTranslations() {
    ...
    if (document.getElementById('cc-nation-list')) {
        ...
        if (curNat) {
            ...
            if (tName) tName.textContent = currentLang === 'en' ? curNat.key.replace(/_/g,' ') : curNat.key_it;
        }
    }
...
function showCreateCareer() {
    ...
    setTimeout(() => {
        ...
        if (defNat) {
            ...
            if (triggerName) triggerName.textContent = currentLang === 'en' ? 'Italy' : 'Italia';
...
const CC_ALL_NATIONS = [
    {key:'Algeria',      key_it:'Algeria',          iso:'dz', color:'#006233'},
    {key:'Argentina',    key_it:'Argentina',        iso:'ar', color:'#74acdf'},
    {key:'Australia',    key_it:'Australia',        iso:'au', color:'#00008b'},
    {key:'Austria',      key_it:'Austria',          iso:'at', color:'#ed2939'},
    {key:'Belgium',      key_it:'Belgio',           iso:'be', color:'#ef3340'},
    {key:'Bolivia',      key_it:'Bolivia',          iso:'bo', color:'#d52b1e'},
    {key:'Brazil',       key_it:'Brasile',          iso:'br', color:'#009c3b'},
    {key:'Cameroon',     key_it:'Camerun',          iso:'cm', color:'#007a5e'},
    {key:'Canada',       key_it:'Canada',           iso:'ca', color:'#ff0000'},
    {key:'Chile',        key_it:'Cile',             iso:'cl', color:'#d52b1e'},
    {key:'Colombia',     key_it:'Colombia',         iso:'co', color:'#fcd116'},
    {key:'CostaRica',    key_it:'Costa Rica',       iso:'cr', color:'#002b7f'},
    {key:'Croatia',      key_it:'Croazia',          iso:'hr', color:'#e31837'},
    {key:'CzechRepublic',key_it:'Rep. Ceca',        iso:'cz', color:'#d7141a'},
    {key:'Denmark',      key_it:'Danimarca',        iso:'dk', color:'#c60c30'},
    {key:'Ecuador',      key_it:'Ecuador',          iso:'ec', color:'#ffd100'},
    {key:'Egypt',        key_it:'Egitto',           iso:'eg', color:'#ce1126'},
    {key:'England',      key_it:'Inghilterra',      iso:'gb-eng', color:'#cf142b'},
    {key:'Finland',      key_it:'Finlandia',        iso:'fi', color:'#003580'},
    {key:'France',       key_it:'Francia',          iso:'fr', color:'#002395'},
    {key:'Germany',      key_it:'Germania',         iso:'de', color:'#555'},
    {key:'Ghana',        key_it:'Ghana',            iso:'gh', color:'#006b3f'},
    {key:'Greece',       key_it:'Grecia',           iso:'gr', color:'#0d5eaf'},
    {key:'Hungary',      key_it:'Ungheria',         iso:'hu', color:'#ce2939'},
    {key:'Iran',         key_it:'Iran',             iso:'ir', color:'#239f40'},
    {key:'Ireland',      key_it:'Irlanda',          iso:'ie', color:'#169b62'},
    {key:'Italy',        key_it:'Italia',           iso:'it', color:'#009246'},
    {key:'Ivory_Coast',  key_it:"Costa d'Avorio",   iso:'ci', color:'#f77f00'},
    {key:'Jamaica',      key_it:'Giamaica',         iso:'jm', color:'#000000'},
    {key:'Japan',        key_it:'Giappone',         iso:'jp', color:'#bc002d'},
    {key:'Mexico',       key_it:'Messico',          iso:'mx', color:'#006847'},
    {key:'Morocco',      key_it:'Marocco',          iso:'ma', color:'#c1272d'},
    {key:'Netherlands',  key_it:'Olanda',           iso:'nl', color:'#ae1c28'},
    {key:'Nigeria',      key_it:'Nigeria',          iso:'ng', color:'#008751'},
    {key:'Norway',       key_it:'Norvegia',         iso:'no', color:'#ef2b2d'},
    {key:'Paraguay',     key_it:'Paraguay',         iso:'py', color:'#d52b1e'},
    {key:'Peru',         key_it:'Perù',             iso:'pe', color:'#d91023'},
    {key:'Poland',       key_it:'Polonia',          iso:'pl', color:'#dc143c'},
    {key:'Portugal',     key_it:'Portogallo',       iso:'pt', color:'#006600'},
    {key:'Romania',      key_it:'Romania',          iso:'ro', color:'#002b7f'},
    {key:'SaudiArabia',  key_it:'Arabia Saudita',   iso:'sa', color:'#006c35'},
    {key:'Scotland',     key_it:'Scozia',           iso:'gb-sct', color:'#003078'},
    {key:'Senegal',      key_it:'Senegal',          iso:'sn', color:'#00853f'},
    {key:'Serbia',       key_it:'Serbia',           iso:'rs', color:'#c6363c'},
    {key:'SouthKorea',   key_it:'Corea del Sud',    iso:'kr', color:'#cd2e3a'},
    {key:'Spain',        key_it:'Spagna',           iso:'es', color:'#c60b1e'},
    {key:'Sweden',       key_it:'Svezia',           iso:'se', color:'#006aa7'},
    {key:'Switzerland',  key_it:'Svizzera',         iso:'ch', color:'#ff0000'},
    {key:'Tunisia',      key_it:'Tunisia',          iso:'tn', color:'#e70013'},
    {key:'Turkey',       key_it:'Turchia',          iso:'tr', color:'#e30a17'},
    {key:'Ukraine',      key_it:'Ucraina',          iso:'ua', color:'#005bbb'},
    {key:'Uruguay',      key_it:'Uruguay',          iso:'uy', color:'#5aaaa8'},
    {key:'USA',          key_it:'USA',              iso:'us', color:'#3c3b6e'},
    {key:'Venezuela',    key_it:'Venezuela',        iso:'ve', color:'#cf142b'},
    {key:'Wales',        key_it:'Galles',           iso:'gb-wls', color:'#00ab39'},
];
...
function ccBuildNationList(filter = '') {
    ...
    // Ordina alfabeticamente per la lingua corrente
    const nameKey = currentLang === 'en' ? 'key' : 'key_it';
    let sorted = [...CC_ALL_NATIONS].sort((a, b) =>
        a[nameKey].localeCompare(b[nameKey], currentLang === 'en' ? 'en' : 'it')
    );
    ...
    list.innerHTML = sorted.map(n => {
        const name = currentLang === 'en' ? n.key.replace(/_/g,' ') : n.key_it;
...
function ccBuildSummary() {
    ...
    document.getElementById('cc-summary-tags').innerHTML = [
        `<span class="cc-tag" style="border: 1.5px solid ${nation?.color||'var(--border)'};">${currentLang==='en'?natKey.replace(/_/g,' '):(nation?.key_it||natKey)}</span>`,
...
async function generateAIAvatar() {
    ...
    try {
        ...
        if (prev) prev.innerHTML = '<div style="color:var(--text-dim);font-size:0.75rem;text-align:center;padding:16px;line-height:1.5">' + _t('no_preview') + '</div>';
    }
}
