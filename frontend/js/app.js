...
// ========================
// NOTIFICHE NOTIZIE (badge nav)
// ========================
async function loadNotificheNotizie() {
    const res = await api('extra.php', { action: 'notizie' }, 'GET');
    const btn = document.getElementById('nav-notizie');
    if (!btn || !res) return;
    const unread = res.unread || 0;
    btn.innerHTML = unread > 0
        ? `📰 Notizie <span class="news-badge">${unread}</span>`
        : '📰 Notizie';
}
...
async function loadAgente() {
    const res = await api('agente.php', { action: 'get' }, 'GET');
            ...
            <div class="agente-attuale">
                ...
                <div class="agente-info">
                    ...
                    <div class="agente-bonus">
                        ...
                        <span class="ag-bonus-item">📋 +${info.bonus_offerte} offerte/mese</span>
                    </div>
                    <div class="agente-descr">${info.descr}</div>
                </div>
            </div>`;
    } else {
        cur.innerHTML = `<div class="agente-vuoto">👤 Non hai ancora un agente. Assumine uno per massimizzare i tuoi guadagni!</div>`;
    }

    // Lista agenti disponibili
    let html = '<div class="agenti-grid">';
    Object.entries(agenti).forEach(([lv, info]) => {
        lv = parseInt(lv);
        const isOwned   = livello >= lv;
        const isNext    = livello === lv - 1;
        const isUpgrade = livello === lv - 1 && livello > 0;
        const costo     = isUpgrade ? agenti[livello]?.costo_up : info.costo;

        html += `<div class="agente-card ${isOwned ? 'owned' : ''} ${isNext ? 'next' : ''}">
            ...
            <div class="ag-stats">
                ...
                <span>📋 +${info.bonus_offerte} offerte</span>
            </div>
            <div class="ag-costo">€${parseInt(info.costo).toLocaleString()}</div>
            ${isOwned
                ? '<div class="ag-badge-ok">Hai questo agente</div>'
                : isNext
                    ? `<button class="btn-primary" style="width:100%;margin-top:8px" onclick="assumiAgente(${lv})">${livello > 0 ? '⬆️ Upgrade: €'+parseInt(agenti[livello]?.costo_up||info.costo).toLocaleString() : '🤝 Assumi: €'+parseInt(info.costo).toLocaleString()}</button>`
                    : `<button class="btn-primary" style="width:100%;margin-top:8px;opacity:0.4" disabled>🔒 Prima il Lv.${lv-1}</button>`
            }
        </div>`;
    });
    html += '</div>';
