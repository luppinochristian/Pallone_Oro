...
function _renderBracket(bracket, player) {
    ...
    const isWin  = t => t && t.fase === 'vincitore';

    /* ── riga squadra ── */
    const row = (t) => {
        if (!t) return `<div class="br-row br-empty"><span class="br-name" style="color:var(--text-dim);font-style:italic;font-size:0.74rem">Da definire</span></div>`;
        const me   = t.team_id == myId;
        const out  = isElim(t);
        const won  = isWin(t);
        let cls = 'br-row' + (won ? ' br-won' : me ? ' br-me' : out ? ' br-out' : '');
        return `<div class="${cls}">
            ...
            <span class="br-right">
                ...
                ${out ? '<span class="br-x">✕</span>' : ''}
                ${won ? '<span class="br-trophy">🏆</span>' : ''}
            </span>
        </div>`;
    };
    ...
    const match = (a, b, note) => `
        <div class="br-match">
            ${note ? `<div class="br-note">${note}</div>` : ''}
            ${row(a)}
            <div class="br-vs"></div>
            ${row(b)}
        </div>`;
    ...
    const win = byFase['vincitore']  ||[];

    const primi = {};
    ott.filter(t=>parseInt(t.posizione_gruppo)===1).forEach(t=>{primi[t.gruppo]=t;});
    const ottNoPrimi = ott.filter(t=>parseInt(t.posizione_gruppo)!==1);

    const poWinner = (g2,g3) =>
        ottNoPrimi.find(t=>(t.gruppo===g2&&parseInt(t.posizione_gruppo)===2)||(t.gruppo===g3&&parseInt(t.posizione_gruppo)===3))
    ...
    if (poPairs.length)
        cols.push({ key:'playoff', icon:'⚡', label:'Playoff',
            body: poPairs.map(([a,b])=>match(a,b)).join('') });

    /* OTTAVI */
    const ottPairs = [
        [primi['A'], poWinner('B','D')],
        [primi['B'], poWinner('A','C')],
        [primi['C'], poWinner('D','B')],
        [primi['D'], poWinner('C','A')],
    ].filter(([a,b])=>a||b);
    if (ottPairs.length)
        cols.push({ key:'ottavi', icon:'🔵', label:'Ottavi',
            body: ottPairs.map(([a,b])=>match(a,b)).join('') });

    /* QUARTI */
    if (qua.length)
        cols.push({ key:'quarti', icon:'⚔️', label:'Quarti',
            body: pairs(qua).map(([a,b])=>match(a,b)).join('') });

    /* SEMI */
    if (sem.length)
        cols.push({ key:'semi', icon:'🥈', label:'Semifinale',
            body: pairs(sem).map(([a,b])=>match(a,b)).join('') });

    /* FINALE */
    if (fin.length || win.length) {
        let body = pairs(fin).map(([a,b])=>match(a,b)).join('');
        if (win.length)
            body += `<div class="br-champion">${row(win[0])}<div class="br-champ-label">🏆 Campione</div></div>`;
        cols.push({ key:'finale', icon:'🥇', label:'Finale', body });
    }

    if (!cols.length) return `<p style="color:var(--text-dim);padding:16px;text-align:center">Nessun dato disponibile.</p>`;

    return `<div class="br-wrap">
        ${cols.map((c,i) => `
            <div class="br-col">
                <div class="br-head"><span class="br-head-icon">${c.icon}</span>${c.label}</div>
                <div class="br-body">${c.body}</div>
            </div>
            ${i < cols.length-1 ? '<div class="br-arr">›</div>' : ''}
        `).join('')}
    </div>`;
}
