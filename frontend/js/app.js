    ...
    switch (style) {
        ...
        case 'long':
            return `<path d="M${s*0.26} ${s*0.42}
                ...
                Q${s*0.30} ${s*0.32} ${s*0.26} ${s*0.42}Z" fill="${hc}"/>
                <path d="M${s*0.20} ${s*0.40} C${s*0.14} ${s*0.52} ${s*0.12} ${s*0.64} ${s*0.15} ${s*0.76} C${s*0.17} ${s*0.86} ${s*0.22} ${s*0.92} ${s*0.28} ${s*0.94} C${s*0.32} ${s*0.96} ${s*0.34} ${s*0.90} ${s*0.32} ${s*0.84} C${s*0.28} ${s*0.74} ${s*0.26} ${s*0.62} ${s*0.26} ${s*0.50} Q${s*0.24} ${s*0.44} ${s*0.20} ${s*0.40}Z" fill="${hc}"/>
                <path d="M${s*0.80} ${s*0.40} C${s*0.86} ${s*0.52} ${s*0.88} ${s*0.64} ${s*0.85} ${s*0.76} C${s*0.83} ${s*0.86} ${s*0.78} ${s*0.92} ${s*0.72} ${s*0.94} C${s*0.68} ${s*0.96} ${s*0.66} ${s*0.90} ${s*0.68} ${s*0.84} C${s*0.72} ${s*0.74} ${s*0.74} ${s*0.62} ${s*0.74} ${s*0.50} Q${s*0.76} ${s*0.44} ${s*0.80} ${s*0.40}Z" fill="${hc}"/>`;
        case 'curly':
            return `<path d="M${s*0.26} ${s*0.40}
                Q${s*0.20} ${s*0.28} ${s*0.26} ${s*0.16}
                Q${cx} ${s*0.04} ${s*0.74} ${s*0.16}
                Q${s*0.80} ${s*0.28} ${s*0.74} ${s*0.40}
                Q${s*0.68} ${s*0.30} ${cx} ${s*0.28}
                Q${s*0.32} ${s*0.30} ${s*0.26} ${s*0.40}Z" fill="${hc}"/>
        ...
        case 'afro':
            return `<ellipse cx="${cx}"     cy="${s*0.17}" rx="${s*0.32}" ry="${s*0.16}" fill="${hc}"/>
        ...
        case 'bun':
            return `<path d="M${s*0.26} ${s*0.38}
                ...
                Q${s*0.68} ${s*0.30} ${s*0.62} ${s*0.32}
                Q${cx} ${s*0.28} ${s*0.38} ${s*0.32}
                Q${s*0.32} ${s*0.30} ${s*0.26} ${s*0.38}Z" fill="${hc}"/>
...
function toggleExitMenu(e) {
    e.stopPropagation();
    document.getElementById('exit-dropdown')?.classList.toggle('open');
}
...
function renderDashboard() {
    ...
    if (avatarWrap) {
        avatarWrap.innerHTML = renderAvatarSVG(p.skin_color||'medium', p.skin_hair||'short_black', p.eye_color||'brown', p.gender||'male', 72);
...
function renderTransferUI(teams, p, leghe) {
    ...
    if (nazioneSelezionata) {
        nazioneSelezionata.leghe.forEach(l => {
            ...
            const tierLabel = l.livello === 1 ? 'Prima Divisione' : `Divisione ${l.livello}`;
...
async function loadAgente() {
    ...
    const myMoltStip = parseInt(player?.moltiplicatore_stipendio) || 100;
...
async function renderClassificaTable(legaId, player) {
    ...
    let html = `<table class="season-table classifica-table">
        <thead>
          <tr class="champ-thead-labels">
            <th>#</th><th>Squadra</th><th title="Overall squadra">OVR</th>
          ...
          <tr class="champ-thead-desc">
            <th></th><th></th><th>Overall</th>
            <th>Giocate</th><th>Vittorie</th><th>Pareggi</th><th>Sconfitte</th>
            <th>Gol F.</th><th>Gol S.</th><th>Diff.</th><th>Punti</th>
          </tr>
        </thead><tbody>`;
    data.forEach((row, i) => {
...
async function renderChampions(player) {
    ...
    if (gruppiKeys.length) {
        ...
        gruppiKeys.forEach(g => {
            ...
            html += `<div class="champ-group-card" data-gruppo="${g}">
                ...
                <div class="champ-group-table-wrap"><table class="champ-group-table">
                    <thead>
                        <tr class="champ-thead-labels">
                            <th>Squadra</th>
                            <th title="Overall squadra">OVR</th>
            ...
            teams.forEach((t, i) => {
                ...
                const qualClass = i === 0 ? 'champ-g-primo' : i === 1 ? 'champ-g-secondo' : i === 2 ? 'champ-g-terzo' : t.eliminato ? 'champ-g-elim' : '';
                ...
                html += `<tr class="${qualClass} ${isMe ? 'champ-g-me' : ''}">
                    ...
                    <td><span class="ovr-badge">${t.ovr ?? '-'}</span></td>
    ...
    requestAnimationFrame(() => {
        ...
        let bd = delay + 60;
        wrap.querySelectorAll('.ucl-team').forEach(el => {
            setTimeout(() => el.classList.add('champ-row-visible'), bd);
            bd += 25;
        });
    });
}
...
function _renderBracket(bracket, player) {
    ...
    const teamCard = (t) => {
        if (!t) return `<div class="ucl-team ucl-tbd-row"><span class="ucl-name ucl-tbd-txt">TBD</span></div>`;
        const isMe   = t.team_id == player.team_id;
        const elim   = t.eliminato == 1 || t.eliminato === '1';
        const winner = t.fase === 'vincitore';
        const flag   = _buildFlag(t.nazione_nome || '');
        let cls = 'ucl-team ';
        if (winner)    cls += 'ucl-winner ';
        else if (isMe) cls += 'ucl-me ';
        else if (elim) cls += 'ucl-elim ';
        else           cls += 'ucl-active ';
        return `<div class="${cls}">
            <span class="ucl-flag">${flag}</span>
            <span class="ucl-name">${t.team_nome}</span>
            <span class="ucl-ovr">${t.ovr}</span>
            ${isMe   ? '<span class="ucl-badge-me">TU</span>' : ''}
            ${elim   ? '<span class="ucl-badge-elim">✕</span>' : ''}
            ${winner ? '<span class="ucl-badge-win">🏆</span>' : ''}
        </div>`;
    };

    const matchBox = (t1, t2, label = '') => `
        <div class="ucl-match">
            ${label ? `<div class="ucl-match-label">${label}</div>` : ''}
            ${teamCard(t1)}
            <div class="ucl-vs">—</div>
            ${teamCard(t2)}
        </div>`;
    ...
    // Cerca vincitore playoff: prima in ottavi (già risolto), poi in playoff/playoff_3 (ancora da giocare)
    const allPlayoff2 = [...(byFase['playoff']   || []), ...(byFase['ottavi'] || []).filter(t => parseInt(t.posizione_gruppo) === 2)];
    const allPlayoff3 = [...(byFase['playoff_3'] || []), ...(byFase['ottavi'] || []).filter(t => parseInt(t.posizione_gruppo) === 3)];

    // playoff2/playoff3 visibili nel tab playoff = quelli non ancora risolti
    const playoff2  = (byFase['playoff']   || []).filter(t => parseInt(t.posizione_gruppo) === 2);
    const playoff3  = (byFase['playoff_3'] || []).filter(t => parseInt(t.posizione_gruppo) === 3);
    const ottavi    = byFase['ottavi']     || [];
    const quarti    = byFase['quarti']     || [];
    const semi      = byFase['semifinale'] || [];
    const finale    = byFase['finale']     || [];
    const vincitore = byFase['vincitore']  || [];

    const byGruppo = (arr, g) => arr.find(t => t.gruppo === g) || null;

    // Il vincitore del playoff è SOLO chi ha già fase='ottavi' con posizione 2 o 3
    // Se il playoff non è ancora stato giocato → null (TBD negli ottavi)
    const playoffWinner = (g2, g3) =>
        ottavi.find(t => t.gruppo === g2 && parseInt(t.posizione_gruppo) === 2) ||
        ottavi.find(t => t.gruppo === g3 && parseInt(t.posizione_gruppo) === 3) ||
        null; // TBD finché il playoff non è risolto

    const ottaviByGruppo = {};
    ottavi.forEach(t => {
        if (t.gruppo && parseInt(t.posizione_gruppo) === 1)
            ottaviByGruppo[t.gruppo] = t;
    });

    const cols = [];

    const playoffPairs = [
        [byGruppo(playoff2,'A'), byGruppo(playoff3,'C'), 'A2 vs C3'],
        [byGruppo(playoff2,'B'), byGruppo(playoff3,'D'), 'B2 vs D3'],
        [byGruppo(playoff2,'C'), byGruppo(playoff3,'A'), 'C2 vs A3'],
        [byGruppo(playoff2,'D'), byGruppo(playoff3,'B'), 'D2 vs B3'],
    ].filter(([a,b]) => a || b);
    if (playoffPairs.length)
        cols.push({ title: '⚡ Playoff', matches: playoffPairs.map(([a,b,l]) => matchBox(a,b,l)) });

    const ottaviPairs = [
        [ottaviByGruppo['A'], playoffWinner('B','D'), '1° Gr.A vs Vin.Playoff'],
        [ottaviByGruppo['B'], playoffWinner('A','C'), '1° Gr.B vs Vin.Playoff'],
        [ottaviByGruppo['C'], playoffWinner('D','B'), '1° Gr.C vs Vin.Playoff'],
        [ottaviByGruppo['D'], playoffWinner('C','A'), '1° Gr.D vs Vin.Playoff'],
    ].filter(([a]) => a); // mostra solo se c'è la prima classificata
    if (ottaviPairs.length)
        cols.push({ title: '🔵 Ottavi di Finale', matches: ottaviPairs.map(([a,b,l]) => matchBox(a,b,l)) });

    const quartiP = chunkPairs(quarti);
    const semiP   = chunkPairs(semi);
    if (quartiP.length) cols.push({ title: '⚔️ Quarti', matches: quartiP.map(([a,b]) => matchBox(a,b)) });
    if (semiP.length)   cols.push({ title: '🥈 Semi', matches: semiP.map(([a,b]) => matchBox(a,b)) });

    const finaleP = chunkPairs(finale);
    if (finaleP.length || vincitore.length) {
        const m = finaleP.map(([a,b]) => matchBox(a,b));
        if (vincitore.length) m.push(`<div class="ucl-winner-card">${teamCard(vincitore[0])}<div class="ucl-winner-label">🏆 Campione</div></div>`);
        cols.push({ title: '🥇 Finale', matches: m, cls: 'ucl-col-finale' });
    }

    if (!cols.length) return '<p style="color:var(--text-dim);padding:12px">Nessun dato disponibile.</p>';

    return `<div class="ucl-bracket">${cols.map((c,i) =>
        `<div class="ucl-col ${c.cls||''}">
            <div class="ucl-col-title">${c.title}</div>
            <div class="ucl-col-body">${c.matches.join('')}</div>
        </div>${i < cols.length-1 ? '<div class="ucl-arrow">›</div>' : ''}`
    ).join('')}</div>`;
}
