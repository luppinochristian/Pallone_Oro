...
const TRANSLATIONS = {
    it: {
        ...
        ai_describe_sub: "Scrivi le caratteristiche fisiche in italiano — capelli, occhi, carnagione, espressione. L'AI (Pollinations) genererà un ritratto.",
        ...
        action_tiro: '🎯 Tiro', action_velocita: '⚡ Velocità', action_dribbling: '🏃 Dribbling',
        action_fisico: '💪 Fisico', action_mentalita: '🧠 Mentalità',
        action_riposo: '😴 Riposo', action_social: '📱 Social', action_speciale: '🔥 Speciale',
        // Dashboard labels
        dash_period: 'Periodo', dash_money: 'Soldi', dash_goals: '⚽ Gol Stagione',
        dash_assists: '🎯 Assist Stagione', dash_facility: 'Struttura',
        dash_tech_stats: '📊 Statistiche Tecniche',
        ...
        transfer_title: 'Trasferimento', transfer_once: 'Un solo trasferimento per stagione.',
        strutture_title: 'Strutture',
        dash_season: 'Stag.', dash_season_full: 'Stagione',
        // Auth extended
        ...
        // AI panel
        ai_avatar_placeholder: 'es: capelli ricci neri, occhi verdi, carnagione scura, sorridente',
        // Game actions
        action_label_allenamento_tiro: '🎯 Allenamento Tiro',
        action_label_allenamento_velocita: '⚡ Velocità',
        action_label_dribbling: '🏃 Dribbling',
        action_label_allenamento_fisico: '💪 Fisico',
        action_label_allenamento_mentalita: '🧠 Mentalità',
        action_label_riposo: '😴 Riposo',
        action_label_social: '📱 Social',
        action_label_allenamento_speciale: '🔥 Speciale',
    },
    en: {
        ...
        ai_describe_sub: 'Write physical characteristics — hair, eyes, skin tone, expression. AI (Pollinations) will generate a portrait.',
        ...
        action_tiro: '🎯 Shooting', action_velocita: '⚡ Speed', action_dribbling: '🏃 Dribbling',
        action_fisico: '💪 Physical', action_mentalita: '🧠 Mental',
        action_riposo: '😴 Rest', action_social: '📱 Social', action_speciale: '🔥 Special',
        // Dashboard labels
        dash_period: 'Period', dash_money: 'Budget', dash_goals: '⚽ Season Goals',
        dash_assists: '🎯 Season Assists', dash_facility: 'Facility',
        dash_tech_stats: '📊 Technical Stats',
        ...
        transfer_title: 'Transfer', transfer_once: 'One transfer per season only.',
        strutture_title: 'Facilities',
        dash_season: 'Season', dash_season_full: 'Season',
        // Auth extended
        ...
        // AI panel
        ai_avatar_placeholder: 'e.g: curly black hair, green eyes, dark skin, smiling',
        // Game actions
        action_label_allenamento_tiro: '🎯 Shooting Training',
        action_label_allenamento_velocita: '⚡ Speed',
        action_label_dribbling: '🏃 Dribbling',
        action_label_allenamento_fisico: '💪 Physical',
        action_label_allenamento_mentalita: '🧠 Mental',
        action_label_riposo: '😴 Rest',
        action_label_social: '📱 Social',
        action_label_allenamento_speciale: '🔥 Special',
    }
}
...
function setLanguage(lang) {
    ...
    // Rebuild actions grid with new language
    _buildActionsGrid();
    selectedActions = [];
    if (document.getElementById('play-month-btn')) updatePlayBtn();
    // Update auth tab labels
    const loginTab = document.querySelector('.auth-tab[data-tab="login"]');
    const regTab   = document.querySelector('.auth-tab[data-tab="register"]');
    if (loginTab) loginTab.textContent = _t('auth_login');
    if (regTab)   regTab.textContent   = _t('auth_register');
    ...
    if (loginBtn) loginBtn.textContent = _t('auth_login_btn');
    if (regBtn)   regBtn.textContent   = _t('auth_register_btn');
}
...
function _applyTranslations() {
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        const txt = _t(key);
        if (!txt) return;
        // For buttons/labels that might have child elements, use textContent only if no children
        // For inputs/selects use placeholder or value
        const tag = el.tagName.toLowerCase();
        if (tag === 'input') {
            el.placeholder = txt;
        } else if (tag === 'option') {
            el.textContent = txt;
        } else if (tag === 'button' || tag === 'label' || tag === 'h2' || tag === 'div' || tag === 'p' || tag === 'span') {
            el.textContent = txt;
        } else {
            el.textContent = txt;
        }
    });
    // Update AI panel description separately (has dynamic content)
    const aiSub = document.querySelector('#avatar-ai-panel p');
    if (aiSub) aiSub.textContent = _t('ai_describe_sub');
    const aiPowered = document.querySelector('#avatar-ai-panel p:last-of-type');
    if (aiPowered) aiPowered.textContent = _t('ai_powered');
    // Update play button
    const playBtn = document.getElementById('play-month-btn');
    if (playBtn && playBtn.disabled === false) {
        const hasActions = typeof selectedActions !== 'undefined' && selectedActions.length > 0;
        playBtn.textContent = hasActions ? _t('play_btn_ready') : _t('play_btn');
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Build actions grid (must be inside DOMContentLoaded so DOM exists)
    _buildActionsGrid();
...
function _showAIPreview(url, statusEl, btn) {
    ...
    img.onerror = () => {
        // Pollinations not reachable — show local SVG fallback using prompt keywords
        if (statusEl) statusEl.textContent = '⚠️ Server AI non raggiungibile. Avatar generato localmente.';
        if (btn) { btn.disabled = false; btn.textContent = _t('ai_generate_btn') || '🎨 Genera Immagine'; }
        // Generate local SVG from prompt keywords
        const prompt = _aiPromptSaved || '';
        const skinGuess = /scur|nera|neri|africa|oliv/i.test(prompt) ? 'dark'
            : /chiar|palli|bianca/i.test(prompt) ? 'light'
            : /media|ambra/i.test(prompt) ? 'medium' : 'medium';
        const hairGuess = /biondi?|bionda/i.test(prompt) ? '#F5D060'
            : /ross[io]/i.test(prompt) ? '#C0392B'
            : /bianchi?/i.test(prompt) ? '#DDD'
            : /grigi?/i.test(prompt) ? '#888' : '#1a1a1a';
        const eyeGuess = /verd[ei]/i.test(prompt) ? '#27AE60'
            : /azzu[rr]/i.test(prompt) || /blu/i.test(prompt) ? '#3A9BD5'
            : /grigi?/i.test(prompt) ? '#717D7E' : '#5C3317';
        const hairStyle = /ricc[io]/i.test(prompt) ? 'curly'
            : /afro/i.test(prompt) ? 'afro'
            : /lunghi?/i.test(prompt) ? 'long'
            : /rast[ae]/i.test(prompt) ? 'bun'
            : /mohawk|cresta/i.test(prompt) ? 'mohawk'
            : /calvo|rasato/i.test(prompt) ? 'bald' : 'short';
        const svg = renderAvatarSVG(skinGuess, hairStyle, eyeGuess, 'male', 120, hairGuess);
        if (prev) prev.innerHTML = svg;
        // Save a marker so we know it's a local fallback (don't save prompt)
        _aiPromptSaved = null;
    };
    img.src = url;
    img.alt = 'AI Avatar';
...
function renderDashboard() {
    ...
    document.getElementById('info-mese').textContent = `${_t('dash_season') || 'Stag.'} ${p.anno_corrente} — ${_meseNome}`;
...
function renderGame() {
    ...
    document.getElementById('current-month-text').textContent =
        `${MONTH_ABBR[meseCorrente]} · ${_t('dash_season_full') || 'Stagione'} ${annoStagione} · ${_t('dash_age') || 'Età'} ${p.age}`;
...
function getActions() {
    ...
    return ACTIONS_DEF.map(a => ({
        id: a.id, icon: a.icon,
        name: lang === 'en' ? a.name_en : a.name_it,
        desc: lang === 'en' ? a.desc_en : a.desc_it,
    }));
}
