...
function _renderBracket(bracket, player) {
    ...
    // ─── Match card ──────────────────────────────────────────────────────
    // Evidenzia il vincitore (chi NON è eliminato quando uno dei due lo è).
    const matchCard = (a, b) => {
        const aOut    = a && isElim(a);
        const bOut    = b && isElim(b);
        const decided = (aOut || bOut) && !(aOut && bOut);
        return `<div class="bk-match">
            ${teamRow(a, decided && !aOut)}
            <div class="bk-div"></div>
            ${teamRow(b, decided && !bOut)}
        </div>`;
    };
