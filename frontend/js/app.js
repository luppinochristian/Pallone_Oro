...
function showCreateCareer() {
    ...
    buildEyePicker();
    updateAvatar();
    // Reset piede selection
    selectedPiedeForte = 'dx';
    selectedPiedeDebole = 'sx';
    _updatePiedeButtons();
}

// Stato selezione piede nella creazione carriera
let selectedPiedeForte = 'dx';  // default destro forte
let selectedPiedeDebole = 'sx'; // default sinistro debole

function setPiede(tipo, lato) {
    if (tipo === 'forte') {
        selectedPiedeForte = lato;
        // aggiorna debole se coincide
        if (selectedPiedeDebole === lato) {
            selectedPiedeDebole = lato === 'dx' ? 'sx' : 'dx';
        }
    } else {
        selectedPiedeDebole = lato;
        if (selectedPiedeForte === lato) {
            selectedPiedeForte = lato === 'dx' ? 'sx' : 'dx';
        }
    }
    _updatePiedeButtons();
}

function _updatePiedeButtons() {
    const fDx = document.getElementById('btn-forte-dx');
    const fSx = document.getElementById('btn-forte-sx');
    const dSx = document.getElementById('btn-debole-sx');
    const dDx = document.getElementById('btn-debole-dx');
    const gold = 'var(--gold)', goldBg = 'var(--gold)', goldFg = '#111';
    const def  = 'var(--border)', defBg = 'transparent', defFg = 'var(--text-dim)';

    if (fDx) { const a = selectedPiedeForte==='dx'; fDx.style.borderColor=a?gold:def; fDx.style.background=a?goldBg:defBg; fDx.style.color=a?goldFg:defFg; }
    if (fSx) { const a = selectedPiedeForte==='sx'; fSx.style.borderColor=a?gold:def; fSx.style.background=a?goldBg:defBg; fSx.style.color=a?goldFg:defFg; }
    const blu = '#3a8fe8', bluBg = '#3a8fe8', bluFg = '#fff';
    if (dSx) { const a = selectedPiedeDebole==='sx'; dSx.style.borderColor=a?blu:def; dSx.style.background=a?bluBg:defBg; dSx.style.color=a?bluFg:defFg; }
    if (dDx) { const a = selectedPiedeDebole==='dx'; dDx.style.borderColor=a?blu:def; dDx.style.background=a?bluBg:defBg; dDx.style.color=a?bluFg:defFg; }
}
...
async function doCreateCareer() {
    ...
    // Validazione piede
    if (selectedPiedeForte === selectedPiedeDebole) {
        if (errEl) errEl.textContent = 'Il piede forte e debole devono essere diversi!';
        return;
    }
    ...
    const res = await api('auth.php', {
        ...
        piede_forte: 3,  // livello fisso
        piede_debole: 2, // livello fisso
        piede_forte_lato: selectedPiedeForte,
        piede_debole_lato: selectedPiedeDebole
    }, 'POST');
    if (res.error) { if(errEl) errEl.textContent = res.error; return; }
