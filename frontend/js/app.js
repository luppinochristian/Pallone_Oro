...
function _renderBracket(bracket, player) {
    ...
    // Vincitore playoff: cerca in ottavi chi viene dal playoff di g2(pos2) vs g3(pos3)
    // Se il playoff non è ancora giocato, mostra la squadra in attesa (senza TBD)
    const playoffWinner = (g2, g3) => {
        // Già risolto: cerca in ottavi la squadra proveniente da quel playoff
        // (mantiene il gruppo originale e la posizione_gruppo)
        const resolved = ottavi.find(t =>
            (t.gruppo === g2 && parseInt(t.posizione_gruppo) === 2) ||
            (t.gruppo === g3 && parseInt(t.posizione_gruppo) === 3)
        );
        if (resolved) return resolved;
        // Playoff non ancora giocato: mostra la 2ª del gruppo g2 o la 3ª del g3 come "sfidante atteso"
