...
const TRANSLATIONS = {
    ...
    de: {
        ...
        nav_notizie:'📰 News', nav_skilltree:'🌟 Fähigkeiten', nav_achievements:'🏅 Erfolge',
        settings_title:'Einstellungen', settings_language:'🌐 Sprache / Language',
        settings_navigate:'🗺️ Navigation', settings_main_menu:'Hauptmenü',
        settings_main_menu_sub:'Karriere wechseln oder neue erstellen',
        settings_feedback:'Update anfragen', settings_feedback_sub:'Sende Vorschläge an die Entwickler',
        settings_account:'👤 Konto', settings_logout:'Vollständiger Logout',
        settings_logout_sub:'Abmelden und zurück zum Login',
        auth_login:'Anmelden', auth_register:'Registrieren',
        auth_login_btn:'⚽ Anmelden', auth_register_btn:'🌟 Konto erstellen',
        auth_username:'Benutzername', auth_confirm_pw:'Passwort bestätigen',
        ...
        dash_no_news:'Spiele den ersten Monat, um News zu erhalten!',
    },
    es: {
        nav_dashboard:'🏠 Panel', nav_game:'⚽ Jugar', nav_career:'📊 Carrera',
        nav_transfer:'🔄 Transfer', nav_strutture:'🏗️ Instalaciones',
        nav_classifica:'🏟️ Clasificación', nav_agente:'🤝 Agente',
        nav_notizie:'📰 Noticias', nav_skilltree:'🌟 Habilidades', nav_achievements:'🏅 Logros',
        settings_title:'Ajustes', settings_language:'🌐 Idioma / Language',
        settings_navigate:'🗺️ Navegación', settings_main_menu:'Menú principal',
        settings_main_menu_sub:'Cambia de carrera o crea una nueva',
        settings_feedback:'Pedir una actualización', settings_feedback_sub:'Envía sugerencias a los desarrolladores',
        settings_account:'👤 Cuenta', settings_logout:'Cerrar sesión (total)',
        settings_logout_sub:'Cierra sesión y vuelve al login',
        auth_login:'Entrar', auth_register:'Registrarse',
        auth_login_btn:'⚽ Entrar', auth_register_btn:'🌟 Crear cuenta',
        auth_username:'Nombre de usuario', auth_confirm_pw:'Confirmar contraseña',
        auth_forgot_link:'¿Olvidaste la contraseña?', auth_forgot_btn:'📧 Enviar instrucciones',
...
// ── NEWS: no automatic translation ──
let _rawNewsData = null; // last fetched raw news from server

async function _translateNewsItems(newsArray, targetLang) {
    // News are authored in Italian; we intentionally avoid machine translation here.
    // UI language switching remains supported without altering news content.
    return newsArray;
}
...
function setLanguage(lang) {
    ...
    document.getElementById('html-root')?.setAttribute('lang', lang || 'it');
...
function openSettings() {
    ...
    document.getElementById('lang-btn-es')?.classList.toggle('active', currentLang === 'es');
...
async function renderChampions(player) {
    ...
    // ── GIRONI ───────────────────────────────────────────────────────────────
    if (gruppiKeys.length) {
        html += `<div class="zone-label" style="margin:20px 0 14px">📋 Fase a Gironi</div>`;
    ...
    // ── BRACKET ───────────────────────────────────────────────────────────────
    if (bracket.length) {
        html += `<div class="zone-label" style="margin:28px 0 14px">⚔️ Fase a Eliminazione Diretta</div>`;
