...
function _renderBracket(bracket, player) {
    ...
    if (quaAll.length) phases.push({ icon:'⚔️', label:'Quarti',    sub:'Quarti di finale', pairs: pairUp(quaAll), rank: 2 });
    if (semAll.length) phases.push({ icon:'🥈', label:'Semifinale', sub:'Ultime 4',         pairs: pairUp(semAll), rank: 3 });

    const finPairs = pairUp(finAll);
    if (finPairs.length || vicAll.length)
        phases.push({ icon:'🥇', label:'Finale', sub:'La Grande Finale', pairs: finPairs, vincitore: vicAll[0] || null, rank: 4 });
