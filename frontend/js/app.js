...
const TRANSLATIONS = {
    ...
    de: {
        ...
        nav_achievements:'🏅 Auszeichnungen',
        view_card:'Karte',
        ...
        reg_err_userreg_err_email:'Bitte gib eine gültige E-Mail ein',
        ...
        tab_lega:'📋 Liga', tab_champions:'🏆 Champions Cup', tab_calendario:'📅 Kalender',
        ...
        conn_tooltip_retooltip_delete:'Löschen',
        ...
        res_section_champions:'🏆 Champions Cup',
        res_section_matches:'⚽ Monats-Spiele',
        ...
        end_career_banner:'🏁 KARRIERE BEENDET!',
    ...
    es: {
        ...
        reg_err_userreg_err_email:'Introduce un email válido',
        ...
        tab_lega:'📋 Liga', tab_champions:'🏆 Copa de Campeones', tab_calendario:'📅 Calendario',
        ...
        conn_tooltip_retooltip_delete:'Eliminar',
        ...
        res_section_matches:'⚽ Partidos del mes',
        ...
        end_career_banner:'🏁 ¡FIN DE CARRERA!',
...
function openColorPicker(type) {
    ...
    if (otherPopup) { otherPopup.style.display = 'none'; otherPopup.style.position = ''; }
    ...
    // ── Anchor to button with position:fixed so it escapes any overflow container ──
    ...
    if (btn) {
        ...
        // Append to body so it's never clipped by any ancestor
        ...
        requestAnimationFrame(() => {
            ...
            if (pr.bottom > window.innerHeight - 8)
                popup.style.top = Math.max(8, r.top - pr.height - 8) + 'px';
        });
    } else {
        popup.style.display = 'block';
    }
...
function showCreateCareer() {
    ...
    setTimeout(() => {
        ...
        // Outside-click for nation dropdown is handled globally by _ccNationOutside (added in ccToggleNationDropdown)
    }, 50);
}
...
function ccToggleNationDropdown() {
    ...
    if (isOpen) {
        ...
        // ── Anchor with position:fixed so it escapes .cc-wizard overflow ──
        if (trigger) {
            const r = trigger.getBoundingClientRect();
            // Move panel to body so no ancestor can clip it
            if (panel.parentElement !== document.body) {
            ...
            requestAnimationFrame(() => {
                ...
                if (pr.bottom > window.innerHeight - 8)
                    panel.style.top = Math.max(8, r.top - pr.height) + 'px';
            });
        }
        ...
        setTimeout(() => {
            document.addEventListener('click', _ccNationOutside, { once: true, capture: true });
        }, 50);
    }
}

function _ccNationOutside(e) {
    const panel   = document.getElementById('cc-nation-panel');
    const trigger = document.getElementById('cc-nation-trigger');
    if (panel && !panel.contains(e.target) && !trigger?.contains(e.target)) {
        ...
        panel.style.zIndex = '';
        const dropdown = document.getElementById('cc-nation-dropdown');
        if (panel.parentElement === document.body && dropdown) dropdown.appendChild(panel);
    }
}
...
function ccSelectNation(key) {
    ...
    // Close dropdown and restore from fixed mode if needed
    const _panel = document.getElementById('cc-nation-panel');
    const _dd    = document.getElementById('cc-nation-dropdown');
    if (_panel) {
        ...
        _panel.style.width = '';
        _panel.style.zIndex = '';
        if (_panel.parentElement === document.body && _dd) _dd.appendChild(_panel);
    }
...
function renderGame() {
    ...
    // Null-safe update: element may not be in DOM if game page isn't active
    const _setText = (id, val) => { const e = document.getElementById(id); if (e) e.textContent = val; };
    _setText('current-month-text', `${MONTH_ABBR[meseCorrente]} · ${_t('dash_season_full')} ${annoStagione} · ${_t('dash_age')} ${p.age}`);
    _setText('g-overall', p.overall);
    _setText('g-energia', p.energia);
    _setText('g-morale',  p.morale);
    _setText('g-soldi',   formatMoney(p.soldi));
    selectedActions = [];
    updatePlayBtn();
...
document.getElementById('play-month-btn')?.addEventListener('click', async () => {
    ...
    // ── Close fixed-positioned dropdowns on scroll or resize ──────────────
    // If the user scrolls/resizes while a custom dropdown is open (position:fixed,
    // anchored to a button), the coords go stale. Close them to avoid misalignment.
    function _closeFixedDropdowns() {
        // Nation panel
        const np = document.getElementById('cc-nation-panel');
        if (np?.classList.contains('open')) {
            np.classList.remove('open');
            document.getElementById('cc-nation-dropdown')?.classList.remove('open');
            np.style.cssText = '';
            const dd = document.getElementById('cc-nation-dropdown');
            if (np.parentElement === document.body && dd) dd.appendChild(np);
        }
        // Color pickers
        ['hair','eye'].forEach(t => {
            const pp = document.getElementById(`cp-${t}-popup`);
            if (pp && pp.style.display !== 'none') {
                pp.style.display = 'none';
                pp.style.position = '';
                if (t === 'eye') cpEyeOpen = false; else cpHairOpen = false;
            }
        });
    }
    window.addEventListener('resize', _closeFixedDropdowns, { passive: true });
    window.addEventListener('scroll', _closeFixedDropdowns, { passive: true, capture: true });
});
...
function stGetSaveKey() {
    // Never use 'null' as a key — different careers would share the same slot
    if (!currentCareerId) return null;
    return `gs_skilltree_${currentCareerId}`;
}

function stLoad() {
    const key = stGetSaveKey();
...
function stSave() {
    const key = stGetSaveKey();
    if (key) localStorage.setItem(key, JSON.stringify(stState));
}
