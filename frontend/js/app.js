...
function _renderBracket(bracket, player) {
    ...
    const allWhoReached = (targetRank) =>
        bracket.filter(t => {
            const r = phaseRank(t);
            if (isElim(t)) return r === targetRank;  // eliminati esattamente qui
            return r >= targetRank;                   // avanzati oltre o ancora qui
        });
    ...
    // ─── Match card ──────────────────────────────────────────────────────
    // Determina il vincitore usando il rank della fase (robusto) + flag eliminato.
    // Il rank è il metodo primario: chi ha rank più alto ha avanzato oltre questa fase.
    // Il flag eliminato è il metodo secondario: se i rank sono uguali, chi ha eliminato=1 ha perso.
    const matchCard = (a, b) => {
        ...
        if (rankA > rankB && !aElim)       aWon = true;   // A ha rank più alto → A avanzato
        else if (rankB > rankA && !bElim)  bWon = true;   // B ha rank più alto → B avanzato
        else if (bElim && !aElim)          aWon = true;   // B eliminato, A no → A vince
        else if (aElim && !bElim)          bWon = true;   // A eliminato, B no → B vince
        // Se entrambi eliminati o nessuno: match non ancora deciso, decided=false

        const decided = aWon || bWon;
        return `<div class="bk-match">
            ${teamRow(a, decided && aWon)}
            <div class="bk-div"></div>
            ${teamRow(b, decided && bWon)}
        </div>`;
    };
    ...
    if (poPairs.length)
        phases.push({ icon:'⚡', label:'Playoff', sub:`${poPairs.length} match`, pairs: poPairs });
    ...
    if (ottPairs.length)
        phases.push({ icon:'🔵', label:'Ottavi', sub:`${ottPairs.length} match`, pairs: ottPairs });

    if (quaAll.length) phases.push({ icon:'⚔️', label:'Quarti',    sub:'Quarti di finale', pairs: pairUp(quaAll) });
    if (semAll.length) phases.push({ icon:'🥈', label:'Semifinale', sub:'Ultime 4',         pairs: pairUp(semAll) });

    const finPairs = pairUp(finAll);
    if (finPairs.length || vicAll.length)
        phases.push({ icon:'🥇', label:'Finale', sub:'La Grande Finale', pairs: finPairs, vincitore: vicAll[0] || null });
    ...
    const html = phases.map((phase, phIdx) => {
        ...
        const matches = phase.pairs.map(([a,b]) => matchCard(a,b)).join('');
