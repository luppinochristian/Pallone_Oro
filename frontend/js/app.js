...
function ccBuildNationList(filter = '') {
    ...
    list.innerHTML = filtered.map(n => `
        <div class="cc-nation-item ${n.key === current ? 'active' : ''}"
             style="--nat-color: ${n.color || '#888'}; border-color: ${n.color || 'var(--border)'};"
...
async function generateAIAvatar() {
    ...
    try {
        // Usa Pollinations.ai - gratuito, no API key, genera direttamente dal browser
        const fullPrompt = encodeURIComponent(
            'cartoon portrait of a football soccer player, ' + userPrompt + ', ' +
        ...
        const imgUrl = 'https://image.pollinations.ai/prompt/' + fullPrompt + '?width=512&height=512&nologo=true&seed=' + seed + '&model=flux';

        _aiPromptSaved = userPrompt;
        _aiAvatarUrl   = imgUrl;

        _showAIPreview(imgUrl, statusEl, btn);
    } catch(e) {
