...
function _renderBracket(bracket, player) {
    ...
    // playoff2/playoff3 = squadre ancora da giocare il playoff
    ...
    // Vincitore playoff: dopo il playoff la squadra passa a fase='ottavi' mantenendo
    // posizione_gruppo=2 o =3. Le prime classificate hanno posizione_gruppo=1.
    // Cerca tra ottavi solo chi NON è primo (posizione 2 o 3 = vince il playoff).
    // Se il playoff non è ancora giocato mostra le squadre ancora in fase playoff.
    const ottaviNonPrimi = ottavi.filter(t => parseInt(t.posizione_gruppo) !== 1);
    const playoffWinner = (g2, g3) => {
        // Playoff già risolto: cerca il vincitore (pos 2 dal gruppo g2, o pos 3 dal gruppo g3)
        const resolved = ottaviNonPrimi.find(t =>
            (t.gruppo === g2 && parseInt(t.posizione_gruppo) === 2) ||
            (t.gruppo === g3 && parseInt(t.posizione_gruppo) === 3)
        );
        if (resolved) return resolved;
        // Playoff non ancora giocato: mostra entrambe le squadre che si devono ancora scontrare
        // (il matchBox mostrerà l'accoppiamento con entrambe visibili)
        return byGruppo(playoff2, g2) || byGruppo(playoff3, g3) || null;
    };
    ...
    // Negli ottavi: mostra sempre entrambe le squadre.
    // Se il playoff non è ancora risolto, playoffWinner mostra la candidata (non TBD).
    // Il frontend mostra badge ✕ sulle squadre già eliminate.
    const ottaviPairs = [
        [ottaviByGruppo['A'], playoffWinner('B','D'), '1° A vs B2/D3'],
        [ottaviByGruppo['B'], playoffWinner('A','C'), '1° B vs A2/C3'],
        [ottaviByGruppo['C'], playoffWinner('D','B'), '1° C vs D2/B3'],
        [ottaviByGruppo['D'], playoffWinner('C','A'), '1° D vs C2/A3'],
    ].filter(([a]) => a);
