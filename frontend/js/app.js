...
async function loadTransfer() {
    ...
    if (!p || p.error || !Array.isArray(teams) || !Array.isArray(leghe)) {
        el.innerHTML = '<div style="text-align:center;padding:48px;color:#f87171">Errore caricamento.</div>';
        return;
    }
    currentPlayer = p;
    window._trD = { teams, p, leghe };
    if (!window._trNaz) {
        const ml = leghe.find(l => l.id == p.lega_id);
        window._trNaz = ml ? ml.nazione_id : leghe[0].nazione_id;
        window._trLiv = ml ? ml.livello : 1;
    }
