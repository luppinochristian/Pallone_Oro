...
async function generateAIAvatar() {
    ...
    try {
        // Il fetch viene fatto lato server PHP per evitare blocchi CORS
        const res = await api('ai_avatar.php', { action: 'generate', prompt: userPrompt }, 'POST');
        if (res.error) throw new Error(res.error);
        _aiAvatarUrl = res.data_url;
        _showAIPreview(_aiAvatarUrl);
        if (statusEl) statusEl.textContent = '✅ Immagine generata!';
        if (btn) { btn.disabled = false; btn.textContent = '🎨 Rigenera'; }
    } catch(e) {
        if (statusEl) statusEl.textContent = `❌ ${e.message || 'Errore generazione. Riprova.'}`;
