...
function buildHairSVG(style, hc, s) {
    ...
    switch (style) {
        ...
        case 'afro':
            // Afro: grande ellisse soffice intorno alla testa, alzata per non coprire gli occhi
            return `
                <ellipse cx="${cx}" cy="${s*0.26}" rx="${s*0.34}" ry="${s*0.22}" fill="${hc}"/>
                <ellipse cx="${s*0.20}" cy="${s*0.32}" rx="${s*0.09}" ry="${s*0.10}" fill="${hc}"/>
                <ellipse cx="${s*0.80}" cy="${s*0.32}" rx="${s*0.09}" ry="${s*0.10}" fill="${hc}"/>
                <ellipse cx="${s*0.26}" cy="${s*0.40}" rx="${s*0.07}" ry="${s*0.07}" fill="${hc}"/>
                <ellipse cx="${s*0.74}" cy="${s*0.40}" rx="${s*0.07}" ry="${s*0.07}" fill="${hc}"/>
                <ellipse cx="${s*0.30}" cy="${s*0.44}" rx="${s*0.06}" ry="${s*0.05}" fill="${hc}"/>
                <ellipse cx="${s*0.70}" cy="${s*0.44}" rx="${s*0.06}" ry="${s*0.05}" fill="${hc}"/>`;
    ...
    el.innerHTML = `
        ...
        <div class="eye-wheel-wrap">
            <button class="eye-custom-btn" onclick="toggleEyeWheel()" id="eye-custom-btn"
                style="background:${selectedEye}; border: 2px dashed ${selectedEye === '#5C3317' ? 'var(--gold)' : selectedEye}">
                🎨
            </button>
            <span class="eye-custom-label">Colore personalizzato</span>
