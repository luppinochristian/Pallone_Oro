...
async function renderChampions(player) {
    ...
    requestAnimationFrame(() => {
        ...
        // Stagger bracket rows
        wrap.querySelectorAll('.bkt-row').forEach((el, idx) => {
...
function _renderBracket(bracket, player) {
    ...
    const isWinner = t => t && t.fase === 'vincitore';
    const isMe = t => t && t.team_id == player.team_id;

    // Riga squadra singola dentro un match
    const teamRow = (t) => {
        if (!t) return `<div class="bkt-row bkt-tbd"><span class="bkt-name">?</span></div>`;
        let cls = 'bkt-row';
        if (isWinner(t))   cls += ' bkt-won';
        else if (isMe(t))  cls += ' bkt-me';
        else if (isElim(t)) cls += ' bkt-out';
        return `<div class="${cls}">
            <span class="bkt-flag">${flag(t)}</span>
            <span class="bkt-name">${t.team_nome}${isMe(t) ? ' <span class="bkt-you">TU</span>' : ''}</span>
            <span class="bkt-ovr">${t.ovr}</span>
            ${isElim(t)  ? '<span class="bkt-mark bkt-mark-out">✕</span>' : ''}
            ${isWinner(t)? '<span class="bkt-mark bkt-mark-win">🏆</span>' : ''}
        </div>`;
    };

    // Box match con due squadre
    const matchBox = (a, b) => `
        <div class="bkt-match">
            ${teamRow(a)}
            ${teamRow(b)}
    ...
    // Prime classificate (vanno dritto agli ottavi)
    const primi = {};
    ottavi.filter(t => parseInt(t.posizione_gruppo) === 1).forEach(t => { primi[t.gruppo] = t; });

    // Vincitore playoff: già in ottavi (pos 2 o 3) o ancora in fase playoff
    const ottaviNonPrimi = ottavi.filter(t => parseInt(t.posizione_gruppo) !== 1);
    const playoffWinner = (g2, g3) =>
        ottaviNonPrimi.find(t => (t.gruppo === g2 && parseInt(t.posizione_gruppo) === 2) ||
                                  (t.gruppo === g3 && parseInt(t.posizione_gruppo) === 3)) ||
        byGruppo(playoff2, g2) || byGruppo(playoff3, g3) || null;

    const sections = [];

    // PLAYOFF
    const poPairs = [
        [byGruppo(playoff2,'A'), byGruppo(playoff3,'C')],
        [byGruppo(playoff2,'B'), byGruppo(playoff3,'D')],
        [byGruppo(playoff2,'C'), byGruppo(playoff3,'A')],
        [byGruppo(playoff2,'D'), byGruppo(playoff3,'B')],
    ].filter(([a,b]) => a || b);
    if (poPairs.length)
        sections.push({ key:'playoff', label:'Playoff', icon:'⚡',
            html: poPairs.map(([a,b]) => matchBox(a,b)).join('') });

    // OTTAVI
    const ottPairs = [
        [primi['A'], playoffWinner('B','D')],
        [primi['B'], playoffWinner('A','C')],
        [primi['C'], playoffWinner('D','B')],
        [primi['D'], playoffWinner('C','A')],
    ].filter(([a,b]) => a || b);
    if (ottPairs.length)
        sections.push({ key:'ottavi', label:'Ottavi', icon:'🔵',
            html: ottPairs.map(([a,b]) => matchBox(a,b)).join('') });

    // QUARTI
    const qPairs = chunkPairs(quarti);
    if (qPairs.length)
        sections.push({ key:'quarti', label:'Quarti', icon:'⚔️',
            html: qPairs.map(([a,b]) => matchBox(a,b)).join('') });

    // SEMIFINALE
    const sPairs = chunkPairs(semi);
    if (sPairs.length)
        sections.push({ key:'semi', label:'Semifinale', icon:'🥈',
            html: sPairs.map(([a,b]) => matchBox(a,b)).join('') });

    // FINALE
    const fPairs = chunkPairs(finale);
    if (fPairs.length || vincitore.length) {
        let fHtml = fPairs.map(([a,b]) => matchBox(a,b)).join('');
        if (vincitore.length) fHtml += `<div class="bkt-champion">${teamRow(vincitore[0])}</div>`;
        sections.push({ key:'finale', label:'Finale', icon:'🏆', html: fHtml });
    }

    if (!sections.length) return '<p style="color:var(--text-dim);padding:16px">Nessun dato disponibile.</p>';

    return `<div class="bkt-bracket">
        ${sections.map((s, i) => `
            <div class="bkt-col" data-key="${s.key}">
                <div class="bkt-head"><span>${s.icon}</span><span>${s.label}</span></div>
                <div class="bkt-body">${s.html}</div>
            </div>
            ${i < sections.length - 1 ? '<div class="bkt-chevron">›</div>' : ''}
        `).join('')}
    </div>`;
}
