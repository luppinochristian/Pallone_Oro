 ...
 *  timeline.js, particles.js, crowd.js, charts.js, encyclopedia.js,
...
const TRANSLATIONS = {
    it: {
        ...
        verify_code_placeholder:'Inserisci il codice',
        verify_btn:'✅ Verifica',
    ...
    en: {
        ...
        verify_code_placeholder:'Enter the code',
        verify_btn:'✅ Verify',
...
// ── TRADUZIONE NOTIZIE ──────────────────────────────────────────────────────
// Traduce un array di notizie dall'italiano all'inglese tramite API Anthropic.
// Usa una cache in memoria per evitare chiamate API ripetute per le stesse notizie.
// I batch sono limitati a BATCH_SIZE notizie per volta per non superare i token disponibili.
async function _translateNewsItems(newsArray, targetLang) {
    // L'italiano è la lingua originale del gioco, nessuna traduzione necessaria
    if (targetLang === 'it') return newsArray;

    // Filtra solo le notizie non ancora presenti in cache
    const toTranslate = newsArray.filter(n => !_newsTranslationCache.en[n.id]);

    // Se tutte le notizie sono già in cache, restituisce subito i dati tradotti
    ...
    // Dimensione massima del batch: 5 notizie per evitare troncamento della risposta API
    const BATCH_SIZE = 5;

    // Funzione interna che traduce un singolo batch e popola la cache
    async function _translateBatch(batch) {
        const payload = batch.map(n => ({ id: n.id, titolo: n.titolo, testo: n.testo }));
        try {
            ...
            if (response.ok) {
                ...
                // Salva ogni voce tradotta in cache (controlla validità prima)
            ...
            console.warn('Traduzione batch fallita, mostro originale:', e);
        }
    }

    // Elabora i batch in sequenza per garantire ordine e affidabilità
    for (let i = 0; i < toTranslate.length; i += BATCH_SIZE) {
        await _translateBatch(toTranslate.slice(i, i + BATCH_SIZE));
    }

    // Restituisce l'array completo usando la cache (fallback al testo italiano)
    return newsArray.map(n => ({
        ...n,
        titolo: _newsTranslationCache.en[n.id]?.titolo ?? n.titolo,
        testo:  _newsTranslationCache.en[n.id]?.testo  ?? n.testo,
...
function setLanguage(lang) {
    ...
    // Clear news translation cache so news are re-translated in new language
    _newsTranslationCache.en = {};
    // Update lang buttons
