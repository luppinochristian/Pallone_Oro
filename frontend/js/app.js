...
// Stato locale skill tree (sincronizzato con localStorage per ora, backend opzionale)
let stState = { points: 0, unlocked: {} }; // unlocked: { skillId: livello }
...
function stLoad() {
    ...
    if (raw) {
        try { stState = JSON.parse(raw); } catch(e) { stState = { points: 0, unlocked: {} }; }
    } else {
...
function stComputeSpentPoints() {
    let spent = 0;
    Object.entries(stState.unlocked).forEach(([id, lv]) => {
        // Trova la skill
...
function stRenderTree(key) {
    ...
    const skillsHtml = Object.entries(tiers).map(([tier, skills]) => {
        const skHtml = skills.map(sk => {
            const unlockedLv = stState.unlocked[sk.id] || 0;
            const isMaxed    = unlockedLv >= sk.maxLv;
            const reqSkill   = sk.requires ? (stState.unlocked[sk.requires] || 0) : 1;
            const canUnlock  = reqSkill > 0 && !isMaxed && stState.points >= sk.cost;
            const isLocked   = reqSkill === 0;
...
function stUpgrade(treeKey, skillId) {
    if (!currentPlayer) return;
    ...
    const curLv  = stState.unlocked[skillId] || 0;
    const req    = sk.requires ? (stState.unlocked[sk.requires] || 0) : 1;

    if (req === 0)      { toast('Sblocca prima il prerequisito!', 'error'); return; }
    if (curLv >= sk.maxLv) { toast('Abilità già al massimo!', ''); return; }
    if (available < sk.cost) { toast(`Servono ${sk.cost} punti abilità (hai ${available})`, 'error'); return; }

    stState.unlocked[skillId] = curLv + 1;
    stSave();

    const newLv = stState.unlocked[skillId];
    const boost = sk.boostPerLv[newLv - 1];
    const statLabel = { tiro:'Tiro', velocita:'Velocità', dribbling:'Dribbling', fisico:'Fisico', mentalita:'Mentalità' }[sk.boostStat] || sk.boostStat;
    toast(`✨ ${sk.name} Lv${newLv} sbloccata! +${boost} ${statLabel}`, 'success');
    loadSkillTree();
}
