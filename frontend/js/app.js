...
function showPage(page) {
    ...
    // Nascondi il box notizie fixed se non siamo sulla dashboard
    const box = document.querySelector('.dash-notizie-box');
    if (box && page !== 'dashboard') box.style.visibility = 'hidden';

    if (page === 'dashboard') {
        renderDashboard();
        // Tenta più volte per aspettare che il layout sia pronto
        [100, 300, 600].forEach(ms => setTimeout(fixNewsBoxHeight, ms));
    }
...
function showResults(res) {
    ...
    (res.risultati || []).forEach(r => {
        ...
        // Salta il riepilogo ⚽ del match (gol/assist/voto) — già mostrato nelle card partite
    ...
    if (legaMsgs.length) {
        ...
        legaMsgs.forEach(m => {
            // Normalizza l'esito: trim + uppercase, e fix difensivo gf==gs→P
...
async function _reloadTransferUI() {
    const [teams, p, leghe] = await Promise.all([
        api('player.php', { action: 'teams' }, 'GET'),
        api('player.php', { action: 'get', career_id: currentCareerId }, 'GET'),
        api('player.php', { action: 'leghe' }, 'GET')
...
async function selectTrNaz(nazId) {
    _trNaz = nazId;
    _trLiv = 1;
    await _reloadTransferUI();
}

async function selectTrLiv(liv) {
    _trLiv = liv;            // NON resetta _trNaz, NON chiama selectTrNaz (che farebbe _trLiv=1)
    await _reloadTransferUI();
}
...
// ========================
// FIX ALTEZZA BOX NOTIZIE
// ========================
function fixNewsBoxHeight() {
    ...
    // Se la colonna destra non è ancora visibile/dimensionata, riprova
    if (rightRect.width < 10) {
        setTimeout(fixNewsBoxHeight, 100);
        return;
    }

    const navH     = nav ? nav.getBoundingClientRect().height : 60;
    const topOffset = navH + 16;
    const availH   = window.innerHeight - topOffset - 24;

    box.style.position      = 'fixed';
    box.style.top           = topOffset + 'px';
    box.style.left          = rightRect.left + 'px';
    box.style.width         = rightRect.width + 'px';
    ...
    const lista = document.getElementById('dash-notizie-lista');
    if (lista) {
        lista.style.flex      = '1 1 0';
        lista.style.overflowY = 'auto';
        lista.style.minHeight = '0';
    }

    // Placeholder per tenere il layout (il box è fixed, non occupa spazio)
    right.style.minHeight = availH + 'px';
}

window.addEventListener('resize', () => {
    const box = document.querySelector('.dash-notizie-box');
    if (box && box.style.visibility !== 'hidden') fixNewsBoxHeight();
});
