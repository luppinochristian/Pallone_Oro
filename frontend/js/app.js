...
function showResults(res) {
    ...
    // Decide se un risultato è "wide" (occupa tutta la riga)
    const isWide = r =>
        r.startsWith('📅') || r.startsWith('🏆') || r.startsWith('💔 Champions') ||
        r.startsWith('🏁') || r.startsWith('⭐') || r.startsWith('💰') ||
        r.startsWith('🔥') || r.includes('CAMPIONI') || r.includes('retrocessa') || r.includes('promossa');
    list.innerHTML = html + res.risultati.map(r =>
        `<li class="result-item${isWide(r) ? ' wide' : ''}">${r}</li>`
    ).join('');
...
async function loadTransfer() {
    ...
    // Aggiorna currentPlayer con i dati freschi
    if (p && !p.error) currentPlayer = p;
    const el = document.getElementById('teams-grid');

    if (!p || p.error) {
        el.innerHTML = `<div style="text-align:center;padding:48px;color:var(--text-dim)">Errore nel caricamento dati giocatore.</div>`;
        return;
    }

    // Controlla finestra mercato — usa SOLO i dati freschi di p
    const mese = parseInt(p.mese_corrente);
    const anno = parseInt(p.anno_corrente);
    const primoMese = (anno === 1 && mese === 9);
    const finestraAperta = [1, 2, 7, 8, 9].includes(mese) && !primoMese;
    const transferUsato = parseInt(p.trasferimento_anno || 0) >= 1;
    const nomiMesi = ['','Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'];

    if (!finestraAperta || primoMese || transferUsato) {
        let msg = '';
        if (primoMese) msg = '🔒 Non puoi trasferirti il primo mese di carriera.';
        else if (transferUsato) msg = '✅ Hai già effettuato un trasferimento questa stagione. Potrai trasferirti di nuovo dalla prossima stagione.';
        else msg = `🔒 Mercato chiuso. Siamo in <strong>${nomiMesi[mese]}</strong>.<br>Le finestre di mercato sono <strong>Luglio – Settembre</strong> e <strong>Gennaio – Febbraio</strong>.`;
        el.innerHTML = `<div style="text-align:center;padding:48px 24px;background:var(--card);border-radius:16px;border:1px solid var(--border)">
            <div style="font-size:3rem;margin-bottom:16px">${transferUsato ? '✅' : '🔒'}</div>
            <p style="color:var(--text-dim);font-size:1rem;line-height:1.6">${msg}</p>
        </div>`;
        return;
    }
