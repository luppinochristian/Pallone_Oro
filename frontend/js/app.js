...
function selectSkinTone(val, hex, el) {
    selectedSkin = val; // salva il val enum (es. 't5'), non l'hex
...
function updateAvatar() {
    ...
    const skinHex = SKIN_COLORS[selectedSkin] || selectedSkin || '#C68642';
    if (el) el.innerHTML = renderAvatarSVG(skinHex, selectedHairStyle, selectedEye, gender, 110, selectedHairColor);
}
...
async function selectCareer(id) {
    ...
    _stSyncDone = false; // reset sync flag per la nuova carriera
...
function showCreateCareer() {
    ...
    selectedSkin = 't5'; selectedEye = '#5C3317';
...
async function doCreateCareer() {
    ...
    // Mappa skin tone val (t1..t10) → enum backend (light/medium_light/medium/medium_dark/dark)
    const SKIN_ENUM_MAP = {
        t1:'light', t2:'light',
        t3:'medium_light', t4:'medium_light',
        t5:'medium', t6:'medium',
        t7:'medium_dark', t8:'medium_dark',
        t9:'dark', t10:'dark'
    };
    const skinEnum = SKIN_ENUM_MAP[selectedSkin] || 'medium';

    // Mappa hex occhi → enum backend (brown/blue/green/hazel/gray)
    const EYE_ENUM_MAP = {
        '#5C3317':'brown','#3d1f0a':'brown',
        '#1E90FF':'blue','#4169E1':'blue',
        '#228B22':'green','#2E8B57':'green',
        '#8B6914':'hazel','#D4A017':'hazel',
        '#708090':'gray','#536878':'gray'
    };
    const eyeEnum = EYE_ENUM_MAP[selectedEye] || 'brown';

    // Componi skin_hair come 'style_colorname' (es. 'short_black')
    // mappa il hex del colore capelli → suffisso colore
    const HAIR_COLOR_SUFFIX = (hex) => {
        const h = (hex || '#1a1a1a').toLowerCase();
        if (['#c0392b','#922b21','#ff0000','#cc0000'].includes(h)) return 'red';
        if (['#f5c842','#c4893e','#d4a017','#f5e642','#ffff00','#ffe066'].some(c => h.startsWith(c.substring(0,5)))) return 'blonde';
        const r = parseInt(h.slice(1,3),16);
        if (r > 160) return 'blonde';
        if (r > 80)  return 'brown';
        return 'black';
    };
    const hairColorSuffix = selectedHairStyle === 'bald' ? '' : '_' + HAIR_COLOR_SUFFIX(selectedHairColor);
    const skinHairValue   = selectedHairStyle + hairColorSuffix; // es. 'short_black', 'afro_brown', 'bald'

    const res = await api('auth.php', {
        action:'create_career', career_name, player_name, gender, age, nationality,
        skin_hair: skinHairValue, skin_color: skinEnum, eye_color: eyeEnum,
...
function renderDashboard() {
    ...
    // OVR circle — anima da 0% → valore reale
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
        circle.style.setProperty('--pct', targetPct + '%');
        _animateCounter(ovValEl, 0, ov, 900);
    }));
...
function _animateCounter(el, from, to, duration) {
    if (!window.requestAnimationFrame) {
        el.textContent = to; return; // fallback istantaneo
    }
...
async function toggleCareerYear(anno) {
    ...
    // Carica solo se non ancora caricato
...
async function loadDashboardNotizie() {
    ...
    api('extra.php', { action: 'leggi' }, 'POST');
}
...
async function renderChampions(player) {
    ...
    if (!data || data.error || (!Object.keys(data.gironi || {}).length && !data.bracket?.length)) {
        wrap.innerHTML = '<p style="color:var(--text-dim);padding:20px">🏆 La Champions Cup non è ancora iniziata. Le squadre si qualificano in ottobre (top 4 di ogni Prima Divisione).</p>';
        return;
    }
    ...
    if (gruppiKeys.length) {
        ...
        html += `<div class="champ-groups-grid">`;
        gruppiKeys.forEach(g => {
            const teams = gironi[g];
            html += `<div class="champ-group-card">
                <div class="champ-group-header">Gruppo ${g}</div>
                <table class="champ-group-table">
                    <thead><tr><th>Squadra</th><th>Pt</th><th>GF</th><th>GS</th></tr></thead>
                    <tbody>`;
            teams.forEach((t, i) => {
                const isMe = t.team_id == player.team_id;
                const qualClass = i === 0 ? 'champ-g-primo' : i === 1 ? 'champ-g-secondo' : t.eliminato ? 'champ-g-elim' : '';
                html += `<tr class="${qualClass} ${isMe ? 'champ-g-me' : ''}">
                    <td><span class="champ-team-name">${t.bandiera||''} ${t.team_nome}</span>${isMe ? ' <span class="my-team-badge">TU</span>' : ''}</td>
                    <td><strong>${t.punti_gruppo}</strong></td>
                    <td>${t.gol_fatti_gruppo}</td>
                    <td>${t.gol_subiti_gruppo}</td>
                </tr>`;
            });
            html += `</tbody></table>
                <div class="champ-group-legend">
                    <span class="champ-leg champ-g-primo">→ Ottavi</span>
                    <span class="champ-leg champ-g-secondo">→ Playoff</span>
                </div>
            </div>`;
        });
        html += `</div>`;
    ...
    if (bracket.length) {
        const fasi = {
            playoff:[], ottavi:[], ottavi_ritorno:[], quarti:[], semifinale:[], finale:[], vincitore:[]
        };
        bracket.forEach(t => { if (fasi[t.fase] !== undefined) fasi[t.fase].push(t); });

        const fasiLabel = [
            { key:'vincitore',      label:'🏆 Campione',           cls:'fase-vincitore' },
            { key:'finale',         label:'🥇 Finale',             cls:'fase-finale' },
            { key:'semifinale',     label:'🥈 Semifinale',         cls:'' },
            { key:'quarti',         label:'⚽ Quarti di Finale',   cls:'' },
            { key:'ottavi_ritorno', label:'🔄 Ottavi (Ritorno)',   cls:'' },
            { key:'ottavi',         label:'🔵 Ottavi di Finale',   cls:'' },
            { key:'playoff',        label:'⚡ Playoff',            cls:'' },
        ];

        html += `<div class="champ-section-title" style="margin-top:24px">🏆 Fase ad Eliminazione</div>`;
        ...
        fasiLabel.forEach(f => {
            ...
            teams.forEach(t => {
                const isMe = t.team_id == player.team_id;
                html += `<div class="champions-team ${isMe ? 'my-champions-team' : ''} ${t.eliminato ? 'elim' : ''}">
                    <span>${t.bandiera||''} ${t.team_nome}</span>
                    <span>${'⭐'.repeat(Math.min(5,parseInt(t.stelle)))} <span class="ovr-badge">${t.ovr}</span></span>
                    ${isMe ? '<span class="my-team-badge">TU</span>' : ''}
    ...
    wrap.innerHTML = html || '<p style="color:var(--text-dim);padding:20px">Nessun dato disponibile.</p>';
}
...
let _stSyncDone = false; // flag: sincronizzazione boost già eseguita in questa sessione
...
async function stUpgrade(treeKey, skillId) {
    ...
    try {
        ...
        if (res && res.success) {
            ...
            // Leggi bottone e nodo PRIMA che loadSkillTree() ricostruisca il DOM
            const btn  = document.querySelector(`button[onclick="stUpgrade('${treeKey}','${skillId}')"]`);
            const node = btn ? btn.closest('.st-node') : null;
            const btnColor = tree.color;

            // Particle burst (già presente)
            if (btn) {
                const rect = btn.getBoundingClientRect();
                _skillParticleBurst(btn, btnColor, rect);
            }

            // Animazione nodo: scale + bagliore, poi ricostruisce il DOM al termine
            if (node) {
                node.style.setProperty('--tree-color', btnColor);
                node.classList.add('unlocking');
                // Stagger sui pip già filled per effetto a cascata
                node.querySelectorAll('.st-pip.filled').forEach((pip, i) => {
                    pip.style.animationDelay = `${i * 60}ms`;
                });
                setTimeout(() => loadSkillTree(), 620);
            } else {
                loadSkillTree();
            }
        } else {
