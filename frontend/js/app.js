 ...
 *
 * Features:
 *  - Career simulation across 10 leagues in 5 countries
 *  - Dynamic news system with 400+ news templates
 *  - Champions Cup with full knockout bracket
 *  - Skill tree with 15+ upgradeable abilities
 *  - Agent & facility upgrade systems
 *  - Crowd atmosphere system (Canvas)
 *  - Particle system (Canvas)
 *  - Achievement system (40+ badges)
 *  - Live match commentary ticker
 *  - Career charts (goals, assists, rating)
 *  - Season planner with fixture difficulty
 *  - Interactive onboarding tutorial
 *  - Full IT/EN bilingual support
 *
 * @version 17.0
 * @license MIT
 */
...
const TRANSLATIONS = {
    it: {
        ...
        auth_username:'Username', auth_confirm_pw:'Conferma Password',
        auth_forgot_link:'Password dimenticata?', auth_forgot_btn:'📧 Invia Istruzioni',
        auth_back_login:'← Torna al Login',
        create_career_title:'Nuova Carriera', create_name_label:'Nome Carriera',
        ...
        ai_generate_btn:'🎨 Genera Immagine', ai_regenerate_btn:'🔄 Rigenera',
        ...
        dash_activities:'📋 Ultime Attività', dash_no_activities:'Nessuna attività ancora',
        ...
        menu_main_sub:'Cambia carriera o creane una nuova',
        menu_update:'Richiedi un Update',
        menu_update_sub:'Invia suggerimenti agli sviluppatori',
        menu_logout_full:'Logout Completo',
        menu_logout_full_sub:'Disconnetti account e torna al login',
        strutture_page_desc:'Investi i tuoi soldi per migliorare la tua struttura di allenamento e crescere più velocemente.',
        reg_err_username:'Inserisci un username',
        ...
        agente_min_chars:'Scrivi almeno 10 caratteri!',
        ...
        ai_describe_sub_html:'Scrivi le caratteristiche fisiche in italiano — capelli, occhi, carnagione, espressione. L\'AI (Pollinations) genererà un ritratto.',
        st_points_default:'0 punti abilità disponibili',
        st_skill_lvl_hint:'Sale sbloccando più abilità nell\'albero',
        ai_preview_hint:'Scrivi una descrizione<br>e premi Genera',
        color_hair_label:'Scegli colore capelli',
        color_eye_label:'Scegli colore occhi',
        conn_error:'Errore di connessione al server. Assicurati che il server PHP sia attivo (bash start.sh)',
        tooltip_rename:'Rinomina',
        tooltip_delete:'Elimina',
        btn_generate_img:'🎨 Genera Immagine',
        default_career_name:'Nuova Carriera',
        injury_banner:'🩹 Sei infortunato — <strong>{0} {1}</strong>. Puoi solo riposare, allenarti mentalmente o fare attività social.',
        ...
        knockout_stage:'🏆 Fase ad Eliminazione',
        ...
        bracket_ottavi:'Ottavi di Finale',
        bracket_quarti_sub:'Quarti di finale',
        bracket_finale_sub:'La Grande Finale',
        skill_pts:'punt{0} abilità disponibil{1}',
        skill_need_pts:'Servono {0} punti (hai {1})',
        ...
        // Transfer
        transfer_ovr_req:'🎯 OVR richiesto', transfer_btn_go:'✈️ Trasferisciti',
        transfer_btn_locked:'🔒 OVR {0} richiesto', transfer_agent_bonus:'🤝 Agente attivo: -{0}% OVR richiesto per il trasferimento',
        ...
        strutture_title:'Strutture', transfer_title:'Trasferimento',
    },
    en: {
        ...
        auth_username:'Username', auth_confirm_pw:'Confirm Password',
        auth_forgot_link:'Forgot password?', auth_forgot_btn:'📧 Send Instructions',
        auth_back_login:'← Back to Login',
        create_career_title:'New Career', create_name_label:'Career Name',
        ...
        cc_role_tuttocampista:'💪 Box-to-box', cc_role_tuttocampista_sub:'+5 Physical',
        ...
        ai_generate_btn:'🎨 Generate Image', ai_regenerate_btn:'🔄 Regenerate',
        ...
        dash_activities:'📋 Recent Activity', dash_no_activities:'No activity yet',
        ...
        menu_main:'Main Menu',
        menu_main_sub:'Change career or create a new one',
        menu_update:'Request an Update',
        menu_update_sub:'Send feedback to the developers',
        menu_logout_full:'Full Logout',
        menu_logout_full_sub:'Disconnect account and return to login',
        strutture_page_desc:'Invest your money to upgrade your training facility and grow faster.',
        reg_err_username:'Please enter a username',
        ...
        agente_min_chars:'Write at least 10 characters!',
        ...
        st_points_default:'0 skill points available',
        st_skill_lvl_hint:'Increases by unlocking more skills in the tree',
        ai_preview_hint:'Write a description<br>and press Generate',
        color_hair_label:'Choose hair colour',
        color_eye_label:'Choose eye colour',
        conn_error:'Connection error. Make sure the PHP server is running (bash start.sh)',
        tooltip_rename:'Rename',
        tooltip_delete:'Delete',
        btn_generate_img:'🎨 Generate Image',
        default_career_name:'New Career',
        injury_banner:'🩹 You are injured — <strong>{0} {1}</strong>. You can only rest, train mentally or do social activities.',
        ...
        bracket_playoff:'Playoff',
        bracket_ottavi:'Round of 16',
        bracket_quarti_sub:'Quarter-finals',
        bracket_finale_sub:'The Grand Final',
        skill_pts:'skill point{0} available',
        skill_need_pts:'Need {0} points (you have {1})',
        ...
        obj_media_voto:'Maintain an average rating ≥ 7.0',
        ...
        // Transfer
        transfer_ovr_req:'🎯 OVR required', transfer_btn_go:'✈️ Transfer',
        transfer_btn_locked:'🔒 OVR {0} required', transfer_agent_bonus:'🤝 Agent active: -{0}% OVR requirement for transfer',
        ...
        // Senza squadra
        no_team:'No team',
        strutture_title:'Facilities', transfer_title:'Transfer',
    }
}
...
// Cache: { 'en': { newsId: { titolo, testo } } }
const _newsTranslationCache = { en: {}, it: {} };
let _rawNewsData = null; // last fetched raw news from server

async function _translateNewsItems(newsArray, targetLang) {
    if (targetLang === 'it') return newsArray; // Italian is original language

    const toTranslate = newsArray.filter(n => {
        const cached = _newsTranslationCache.en[n.id];
        return !cached;
    });
    ...
    if (toTranslate.length === 0) {
        return newsArray.map(n => ({
            ...n,
            titolo: _newsTranslationCache.en[n.id]?.titolo ?? n.titolo,
            testo:  _newsTranslationCache.en[n.id]?.testo  ?? n.testo,
        }));
    }

    // Build batch payload
    const payload = toTranslate.map(n => ({ id: n.id, titolo: n.titolo, testo: n.testo }));
    ...
    try {
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 4000,
                system: 'You are a football game news translator. Translate the Italian football news to English. Keep emojis. Keep player names, team names and city names as-is. Return ONLY a JSON array with same structure: [{id, titolo, testo}]. No markdown, no explanation, no backticks.',
                messages: [{
                    role: 'user',
                    content: `Translate these football news items from Italian to English:\n${JSON.stringify(payload)}`
                }]
            })
        });
        ...
        if (response.ok) {
            ...
            translated.forEach(t => {
                _newsTranslationCache.en[t.id] = { titolo: t.titolo, testo: t.testo };
            });
        }
    } catch(e) {
        console.warn('News translation failed, showing original:', e);
    }
...
function setLanguage(lang) {
    ...
    if (currentPlayer) {
        ...
        renderGame();
        renderDashboard();
        // If notizie page is currently visible, reload with new language
...
// Auto-attach tooltips on dashboard render
const _origRenderDash = window.renderDashboard;
// attachTooltips is called after render in existing code via setTimeout
        ...
        items.forEach(a => {
            ...
            html += `
                <div class="ach-card ${a.isUnlocked ? 'unlocked' : 'locked'} ${secret ? 'secret' : ''}"
                     title="${a.isUnlocked ? (currentLang === 'en' ? 'Unlocked!' : 'Sbloccato!') : ''}">
                    ...
                    ${a.isUnlocked ? '<div style="font-size:0.68rem;color:var(--gold);margin-top:6px">✓ ' + (currentLang === 'en' ? 'Unlocked' : 'Sbloccato') + '</div>' : ''}
                </div>`;
        });
        html += '</div>';
    });
...
GS_Events.on('goal', ({ player, gol, esito }) => {
    if (window.GS_Crowd) GS_Crowd.showGoalChant?.();
    if (window.GS_Particles) GS_Particles.effects.goalCelebration();
});

GS_Events.on('win', ({ player }) => {
    if (window.GS_Crowd) GS_Crowd.showVictoryChant?.();
...
function computeExtendedStats(seasons, logData) {
    if (!seasons?.length && !logData?.length) return null;
    const lang = currentLang;
    
    const allMatches = (logData || []).filter(l => l.avv && l.avv !== '__riepilogo');
    const wins   = allMatches.filter(l => l.esito === 'V').length;
    const draws  = allMatches.filter(l => l.esito === 'P').length;
    const losses = allMatches.filter(l => l.esito === 'S').length;
    ...
    const topGolMatch = allMatches.reduce((best, l) => 
        parseInt(l.gol || 0) > parseInt(best.gol || 0) ? l : best, 
    allMatches[0] || {});
    
    const topVotoMatch = allMatches.reduce((best, l) => 
        parseFloat(l.voto || 0) > parseFloat(best.voto || 0) ? l : best, 
    allMatches[0] || {});
    
    const totalGoals   = allMatches.reduce((a, l) => a + parseInt(l.gol    || 0), 0);
    const totalAssists = allMatches.reduce((a, l) => a + parseInt(l.assist || 0), 0);
    const avgVoto = total ? (allMatches.reduce((a, l) => a + parseFloat(l.voto || 6), 0) / total).toFixed(2) : '—';
    
    return {
        total, wins, draws, losses, winPct, drawPct, lossPct,
        totalGoals, totalAssists, avgVoto,
        topGolMatch, topVotoMatch,
        goalsPerMatch: total ? (totalGoals / total).toFixed(2) : '0',
        assistsPerMatch: total ? (totalAssists / total).toFixed(2) : '0',
    };
}

// ── Render extended stats panel ───────────────────────────────────────────────
function renderExtendedStats(container, stats) {
    if (!container || !stats) return;
    const lang = currentLang;
    const W = stats.winPct, D = stats.drawPct, L = stats.lossPct;
    
    container.innerHTML = `
        <div class="career-chart-box" style="margin-bottom:16px">
            <div style="font-size:0.78rem;font-weight:700;color:var(--gold);margin-bottom:12px">
                📊 ${lang === 'en' ? 'All-time W/D/L' : 'W/P/S storico'}
            </div>
            <div style="display:flex;height:12px;border-radius:6px;overflow:hidden;margin-bottom:8px">
                <div style="width:${W}%;background:#10b981;transition:width 0.8s ease"></div>
                <div style="width:${D}%;background:#eab308;transition:width 0.8s ease"></div>
                <div style="width:${L}%;background:#ef4444;transition:width 0.8s ease"></div>
            </div>
            <div style="display:flex;gap:16px;font-size:0.72rem">
                <span style="color:#10b981">▮ ${lang === 'en' ? 'W' : 'V'} ${W}% (${stats.wins})</span>
                <span style="color:#eab308">▮ D ${D}% (${stats.draws})</span>
                <span style="color:#ef4444">▮ ${lang === 'en' ? 'L' : 'S'} ${L}% (${stats.losses})</span>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px">
                <div style="text-align:center;padding:8px;background:var(--bg3);border-radius:8px">
                    <div style="font-size:1.2rem;font-weight:900;color:#FFD700">${stats.goalsPerMatch}</div>
                    <div style="font-size:0.65rem;color:var(--text-dim)">${lang === 'en' ? 'Goals/Match' : 'Gol/Partita'}</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--bg3);border-radius:8px">
                    <div style="font-size:1.2rem;font-weight:900;color:#3b82f6">${stats.assistsPerMatch}</div>
                    <div style="font-size:0.65rem;color:var(--text-dim)">${lang === 'en' ? 'Assist/Match' : 'Assist/Partita'}</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--bg3);border-radius:8px">
                    <div style="font-size:1.2rem;font-weight:900;color:#8b5cf6">${stats.avgVoto}</div>
                    <div style="font-size:0.65rem;color:var(--text-dim)">${lang === 'en' ? 'Avg Rating' : 'Media Voto'}</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--bg3);border-radius:8px">
                    <div style="font-size:1.2rem;font-weight:900;color:#f59e0b">${stats.total}</div>
                    <div style="font-size:0.65rem;color:var(--text-dim)">${lang === 'en' ? 'Total Matches' : 'Partite Totali'}</div>
                </div>
            </div>
        </div>`;
}
...
async function selectCareer(id) {
    ...
    setTimeout(() => {
        if (window.GS_Tutorial && !GS_Tutorial.isDone()) GS_Tutorial.start();
    }, 1200);
}
...
function ccGoStep(n) {
    ...
    if (old) { old.classList.remove('active'); old.classList.add('exit'); setTimeout(() => old.classList.remove('exit'), 300); }
    _ccCurrentStep = n;
...
function ccNextStep(from) {
    ...
    if (from === 2) {
        ...
        const isRPMMode = document.getElementById('btn-mode-rpm')?.classList.contains('active');
        if (isAIMode && !_aiPromptSaved) {
            const errEl = document.getElementById('create-error');
            if (errEl) errEl.textContent = _t('ai_avatar_err_nogen');
            return;
        }
        if (isRPMMode && !_rpmAvatarUrl) {
            const errEl = document.getElementById('create-error');
            if (errEl) errEl.textContent = '⚠️ Crea prima il tuo avatar 3D con Ready Player Me!';
            return;
        }
    }
    ccGoStep(from + 1);
...
function ccSelectNation(key) {
    ...
    ccBuildNationList(document.getElementById('cc-nation-search')?.value || '');
}
...
function setAvatarMode(mode) {
    ...
    document.getElementById('btn-mode-rpm')?.classList.toggle('active', isRPM);
    // Toggle panels
    const defPanel = document.getElementById('avatar-default-panel');
    const aiPanel  = document.getElementById('avatar-ai-panel');
    const rpmPanel = document.getElementById('avatar-rpm-panel');
    if (defPanel) defPanel.style.display = isDef  ? 'block' : 'none';
    if (aiPanel)  aiPanel.style.display  = isAI   ? 'block' : 'none';
    if (rpmPanel) rpmPanel.style.display = isRPM  ? 'block' : 'none';
    // RPM mode: rimuovi max-height dal contenitore controls per non tagliare l'iframe
    const controls = document.querySelector('.cc-appearance-controls');
    if (controls) {
        controls.style.maxHeight = isRPM ? 'none' : '';
        controls.style.overflow  = isRPM ? 'visible' : '';
    }
    // Sesso: nascosto in AI e RPM
    const genderRow = document.getElementById('gender-row');
    if (genderRow) genderRow.style.display = (isAI || isRPM) ? 'none' : '';
    ...
    } else if (isRPM && _rpmAvatarUrl) {
        _showRPMPreview(_rpmAvatarUrl);
    } else if (!isDef) {
        const prev = document.getElementById('avatar-preview');
        if (prev) {
            prev.innerHTML = `<div style="color:var(--text-dim);font-size:0.75rem;text-align:center;padding:16px;line-height:1.5">${isRPM ? '🧍 Crea il tuo avatar 3D' : _t('ai_preview_hint')}</div>`;
        }
    }
}
...
async function doCreateCareer() {
    ...
    const res = await api('auth.php', {
        ...
        skin_hair:  isAIMode ? 'ai' : (isRPMMode ? 'rpm' : selectedHairStyle),
        skin_color: isAIMode ? 'ai' : (isRPMMode ? 'rpm' : selectedSkin),
        eye_color:  isAIMode ? 'ai' : (isRPMMode ? 'rpm' : selectedEye),
        hair_color: isAIMode ? 'ai' : (isRPMMode ? 'rpm' : selectedHairColor),
        ai_prompt:  isAIMode ? _aiPromptSaved : null,
        ai_avatar:  isRPMMode ? _rpmAvatarUrl : null,
...
function renderDashboard() {
    ...
    // Add radar chart container if not exists
    const statCard = document.querySelector('.stat-card');
    if (statCard && !document.getElementById('player-radar-chart')) {
        const radarWrap = document.createElement('div');
        radarWrap.id = 'player-radar-chart';
        radarWrap.className = 'radar-chart-container';
        radarWrap.style.cssText = 'margin-top:16px;padding:4px;height:240px;overflow:visible';
        statCard.appendChild(radarWrap);
    }
    ...
    // Render radar chart if player data available
    if (currentPlayer && window.GS_Charts) {
        const radarContainer = document.getElementById('player-radar-chart');
        if (radarContainer) {
            radarContainer.innerHTML = '';
            const p = currentPlayer;
            GS_Charts.radarChart(radarContainer, {
                [_t('stat_tiro')]: p.tiro,
                [_t('stat_velocita')]: p.velocita,
                [_t('stat_dribbling')]: p.dribbling,
                [_t('stat_fisico')]: p.fisico,
                [_t('stat_mentalita')]: p.mentalita
            }, { id: 'gs-radar', size: 220 });
        }
    }
}
...
document.getElementById('play-month-btn')?.addEventListener('click', async () => {
    ...
    // Crowd reactions based on results
    if (window.GS_Crowd) {
        const lm = res.lega_msgs || [];
        const hasWin = lm.some(m => m.esito === 'V');
        const allLoss = lm.length > 0 && lm.every(m => m.esito === 'S');
        if (res.champions_win) { GS_Crowd.triggerCelebration(6000); }
        else if (res.pallone_doro?.pos === 1) { GS_Crowd.triggerCelebration(5000); }
        else if (res.promozione?.includes('PROMOZIONE')) { GS_Crowd.triggerCelebration(4000); }
        else if (res.promozione?.includes('RETROCESSIONE')) { GS_Crowd.triggerBoos(); }
        else if (allLoss) { GS_Crowd.triggerBoos(); }
        else if (hasWin) { GS_Crowd.triggerCelebration(4000); }
    }
        ...
        div.innerHTML = `
            ...
            <div>
                ...
                <div class="struttura-bonuses">
                    ${s.bonus_allenamento   ? `<span class="struttura-bonus">+${s.bonus_allenamento} ${_t('strutture_bonus_training')}</span>` : ''}
                    ${s.bonus_crescita      ? `<span class="struttura-bonus">+${s.bonus_crescita}% ${_t('strutture_bonus_growth')}</span>` : ''}
                    ${s.riduzione_infortuni ? `<span class="struttura-bonus">-${s.riduzione_infortuni}% ${_t('strutture_bonus_injury')}</span>` : ''}
                </div>
            </div>
...
async function loadDashboardNotizie() {
    ...
    // Show loading placeholder while translating
    if (currentLang === 'en') {
        el.innerHTML = '<p style="color:var(--text-dim);font-size:0.82rem;padding:8px 0">Translating news...</p>';
    }
...
async function loadNotizie() {
    ...
    if (currentLang === 'en') {
        el.innerHTML = '<p style="color:var(--text-dim);padding:20px;text-align:center">⏳ Translating news...</p>';
    }
...
async function loadAgente() {
    ...
    entries.forEach(([lvStr, info]) => {
        ...
        <div class="agente-card ${isOwned?'owned':''} ${isNext&&popOk&&!isOwned?'next':''}">
            ...
            <div class="ag-stats">
                <span>💰 +${Math.round(parseFloat(info.bonus_stipendio)*100)}% ${_t('ag_salary_bonus')}</span>
                <span>📉 -${info.bonus_ovr_sconto}% ${_t('ag_ovr_bonus')}</span>
            </div>
...
async function renderChampions(player) {
    ...
    if (bracket.length) {
        html += `<div class="champ-section-title" style="margin-top:28px">${_t('knockout_stage')}</div>`;
...
function _renderBracket(bracket, player) {
    ...
    if (poPairs.length)
        phases.push({ icon:'⚡', label:_t('bracket_playoff'), sub:`${poPairs.length} match`, pairs: poPairs, rank: 0 });
    ...
    if (ottPairsDisplay.length)
        phases.push({ icon:'🔵', label:_t('bracket_ottavi'), sub:`${ottPairsDisplay.length} match`, pairs: ottPairsDisplay, rank: 1 });
    ...
    const finPairsBuilt = semPairs.length
        ? semPairs   // le due semifinaliste sono già la coppia finale
        : pairUp(finAll);
    ...
    if (quaFinal.length)  phases.push({ icon:'⚔️', label:_t('bracket_quarti_sub'),  sub:_t('bracket_quarti_sub'), pairs: quaFinal,  rank: 2 });
    // Nel torneo (20 squadre): dopo i Quarti rimangono 2 squadre → la 'Semifinale' PHP è la FINALE
    if (finFinal.length || vicAll.length)
        phases.push({ icon:'🏆', label:_t('bracket_finale_sub'), sub:_t('bracket_finale_sub'), pairs: finFinal, vincitore: vicAll[0] || null, rank: 3 });
    if (!phases.length)
        return `<p style="color:var(--text-dim);padding:20px;text-align:center">${_t('bracket_no_data_full')}</p>`;
    ...
    const html = phases.map((phase, phIdx) => {
        ...
        if (phase.vincitore) {
            ...
            champ = `<div class="bk-champion">
                ...
                <div class="bk-champion-label">${_t('match_win')} UCL</div>
...
function getStrutturaName(lvl) {
    ...
    return _t('fac_' + idx) || (currentLang === 'en' ? 'None' : 'Nessuna');
}
...
// ── Ready Player Me ───────────────────────────────────────────────────────
let _rpmAvatarUrl  = null;
let _rpmGlbUrl     = null;
let _rpmListenerOn = false;

const RPM_IFRAME_URL = 'https://readyplayer.me/avatar?frameApi&clearCache&bodyType=halfbody';

// ── DEV MODE: mock RPM quando l'iframe non è raggiungibile (es. GitHub Codespaces) ──
// Metti DEV_RPM_MOCK = true per bypassare RPM e usare un avatar di test.
// In produzione lascia sempre false.
const DEV_RPM_MOCK = true;
const DEV_RPM_MOCK_GLB = 'https://models.readyplayer.me/638df693d72bffc6fa17943c.glb';
const DEV_RPM_MOCK_PNG = 'https://api.multiavatar.com/dev_test_player.png';

function _activateDevMockAvatar() {
    _rpmGlbUrl    = DEV_RPM_MOCK_GLB;
    _rpmAvatarUrl = DEV_RPM_MOCK_PNG;
    const prevW = document.getElementById('rpm-preview-wrap');
    const img   = document.getElementById('rpm-preview-img');
    const btn   = document.getElementById('rpm-open-btn');
    if (prevW) prevW.style.display = 'block';
    if (img)   img.src = DEV_RPM_MOCK_PNG;
    if (btn)   btn.style.display = 'none';
    _showRPMPreview(DEV_RPM_MOCK_PNG);
    _setRPMStatus('✅ [DEV] Avatar mock attivo — in produzione verrà usato il vero RPM.');
    const wizard = document.querySelector('.cc-wizard');
    if (wizard) wizard.style.maxHeight = '';
}

function openRPMCreator() {
    // DEV MODE: bypassa l'iframe e usa avatar mock
    if (DEV_RPM_MOCK) {
        _activateDevMockAvatar();
        return;
    }
    const wrap   = document.getElementById('rpm-iframe-wrap');
    const iframe = document.getElementById('rpm-iframe');
    const btn    = document.getElementById('rpm-open-btn');
    const prevW  = document.getElementById('rpm-preview-wrap');
    if (!wrap || !iframe) return;
    if (!_rpmListenerOn) { window.addEventListener('message', _onRPMMessage); _rpmListenerOn = true; }
    iframe.src = RPM_IFRAME_URL;
    wrap.style.display  = 'block';
    if (btn)   btn.style.display   = 'none';
    if (prevW) prevW.style.display = 'none';
    // Espandi il wizard per contenere l'iframe
    const wizard = document.querySelector('.cc-wizard');
    if (wizard) wizard.style.maxHeight = '95vh';
    _setRPMStatus('Personalizza il tuo avatar, poi clicca "Next" in fondo...');
}
...
function closeRPMCreator() {
    ...
    if (btn)    btn.style.display    = _rpmAvatarUrl ? 'none' : 'block';
    if (iframe) iframe.src           = '';
    // Ripristina altezza wizard
    const wizard = document.querySelector('.cc-wizard');
    if (wizard) wizard.style.maxHeight = '';
}

function _onRPMMessage(e) {
    if (!e.origin.includes('readyplayer.me')) return;
    if (!e.data || typeof e.data !== 'string') return;
    if (!e.data.includes('readyplayer.me') && !e.data.includes('.glb')) return;
    const glbUrl = e.data.split('?')[0];
    if (!glbUrl.endsWith('.glb')) return;
    _rpmGlbUrl    = glbUrl;
    const pngUrl  = glbUrl.replace('.glb', '.png') + '?scene=halfbody-portrait-v1&quality=1&width=256&height=256';
    _rpmAvatarUrl = pngUrl;
    closeRPMCreator();
    _showRPMPreview(pngUrl);
    _setRPMStatus('✅ Avatar pronto!');
    const prevW = document.getElementById('rpm-preview-wrap');
    const img   = document.getElementById('rpm-preview-img');
    if (prevW) prevW.style.display = 'block';
...
function _showRPMPreview(url) {
    const prev = document.getElementById('avatar-preview');
    if (prev) prev.innerHTML = '<img src="' + url + '" style="width:130px;height:130px;object-fit:cover;border-radius:50%;" alt="RPM">';
}

function _setRPMStatus(txt) {
    const el = document.getElementById('rpm-status-text');
