...
async function renderChampions(player) {
    ...
    requestAnimationFrame(() => {
        ...
        // Stagger per le squadre nel bracket (animazione CSS gestita nativamente via animation-delay)
        wrap.querySelectorAll('.bk-team').forEach((el, idx) => {
...
function _renderBracket(bracket, player) {
    ...
    // ── Team card ──────────────────────────────────────────────────────
    const teamCard = (t, isPending = false) => {
        if (!t) return '';
        const isMe   = t.team_id == player.team_id;
        ...
        const flag   = _buildFlag(t.nazione_nome || '');
        let rowCls = 'bk-team';
        if (winner)    rowCls += ' bk-winner';
        else if (isMe) rowCls += ' bk-me';
        else if (elim) rowCls += ' bk-elim';
        else if (isPending) rowCls += ' bk-pending';
        return `<div class="${rowCls}">
            <span class="bk-flag">${flag}</span>
            <span class="bk-name">${t.team_nome}</span>
            <span class="bk-right">
                <span class="bk-ovr">${t.ovr}</span>
                ${winner ? '<span class="bk-icon">🏆</span>' : ''}
                ${elim   ? '<span class="bk-icon bk-x">✕</span>' : ''}
                ${isMe   ? '<span class="bk-badge-me">TU</span>' : ''}
            </span>
        </div>`;
    };

    // ── Match box: mostra sempre entrambe le squadre, mai TBD ──────────
    const matchBox = (t1, t2, label = '', t1Pending = false, t2Pending = false) => {
        const top    = t1 ? teamCard(t1, t1Pending) : '<div class="bk-team bk-unknown"><span class="bk-name bk-unk-txt">Da definire</span></div>';
        const bottom = t2 ? teamCard(t2, t2Pending) : '<div class="bk-team bk-unknown"><span class="bk-name bk-unk-txt">Da definire</span></div>';
        return `<div class="bk-match">
            ${label ? `<div class="bk-label">${label}</div>` : ''}
            ${top}
    ...
    // Squadre prime classificate (vanno direttamente agli ottavi)
    const ottaviPrimi = {};
    ottavi.forEach(t => {
        if (t.gruppo && parseInt(t.posizione_gruppo) === 1)
            ottaviPrimi[t.gruppo] = t;
    });

    // Vincitore playoff (già risolto = in ottavi con pos 2 o 3)
    // Non risolto = ancora in playoff/playoff_3
    ...
    const playoffWinner = (g2, g3) => {
        ...
        if (resolved) return { team: resolved, pending: false };
        const pending = byGruppo(playoff2, g2) || byGruppo(playoff3, g3);
        return pending ? { team: pending, pending: true } : { team: null, pending: false };
    };
    ...
    if (playoffPairs.length)
        sections.push({ id:'playoff', title:'Playoff', icon:'⚡', matches: playoffPairs.map(([a,b,l]) => matchBox(a,b,l)) });
    ...
    const ottaviMatches = ottaviDef.map(([g1, g2, g3]) => {
        const primo = ottaviPrimi[g1] || null;
        const { team: sfid, pending } = playoffWinner(g2, g3);
        const label = `1° Gr.${g1} vs Vin. ${g2}2/${g3}3`;
        return matchBox(primo, sfid, label, false, pending);
    }).filter(Boolean);
    if (ottaviMatches.length)
        sections.push({ id:'ottavi', title:'Ottavi di Finale', icon:'🔵', matches: ottaviMatches });

    // ── QUARTI ─────────────────────────────────────────────────────────
    const quartiP = chunkPairs(quarti);
    if (quartiP.length)
        sections.push({ id:'quarti', title:'Quarti di Finale', icon:'⚔️', matches: quartiP.map(([a,b]) => matchBox(a,b)) });

    // ── SEMIFINALE ─────────────────────────────────────────────────────
    const semiP = chunkPairs(semi);
    if (semiP.length)
        sections.push({ id:'semi', title:'Semifinale', icon:'🥈', matches: semiP.map(([a,b]) => matchBox(a,b)) });
    ...
    if (finaleP.length || vincitore.length) {
        ...
        if (vincitore.length) {
            m.push(`<div class="bk-champion">
                ${teamCard(vincitore[0])}
                <div class="bk-champion-label">🏆 Campione Champions Cup</div>
            </div>`);
        }
        sections.push({ id:'finale', title:'Finale', icon:'🥇', matches: m });
    }

    if (!sections.length) return '<p style="color:var(--text-dim);padding:16px">Nessun dato disponibile.</p>';

    // ── RENDER ─────────────────────────────────────────────────────────
    const cols = sections.map((s, i) => `
        <div class="bk-col" data-fase="${s.id}">
            <div class="bk-col-head">
                <span class="bk-col-icon">${s.icon}</span>
                <span class="bk-col-title">${s.title}</span>
            </div>
            <div class="bk-col-body">
                ${s.matches.join('')}
            </div>
        </div>
        ${i < sections.length - 1 ? '<div class="bk-arrow">›</div>' : ''}
    `).join('');

    return `<div class="bk-bracket">${cols}</div>`;
}
