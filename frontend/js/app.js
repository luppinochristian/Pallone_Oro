...
function _renderBracket(bracket, player) {
    ...
    const elim  = t => t && (t.eliminato == 1 || t.eliminato === '1');
    const won   = t => t && t.fase === 'vincitore';

    const row = (t) => {
        if (!t) return `<div class="br-row br-tbd"><span class="br-name">Da definire</span></div>`;
        const me = t.team_id == myId;
        let cls  = 'br-row' + (won(t) ? ' br-won' : me ? ' br-me' : elim(t) ? ' br-out' : '');
        return `<div class="${cls}">
            <span class="br-flag">${flag(t)}</span>
            <span class="br-name">${t.team_nome}${me ? ' <span class="br-you">TU</span>' : ''}</span>
            <span class="br-right">
                <span class="br-ovr">${t.ovr}</span>
                ${elim(t) ? '<span class="br-x">✕</span>' : ''}
                ${won(t)  ? '<span class="br-trophy">🏆</span>' : ''}
            </span>
        </div>`;
    };

    const match = (a, b) => `<div class="br-match">${row(a)}<div class="br-div"></div>${row(b)}</div>`;
    const pairs = arr => { const p=[]; for(let i=0;i<arr.length;i+=2) p.push([arr[i],arr[i+1]||null]); return p; };
    const byG   = (arr, g) => arr.find(t => t.gruppo === g) || null;

    const po2 = (byFase['playoff']   ||[]).filter(t => parseInt(t.posizione_gruppo) === 2);
    const po3 = (byFase['playoff_3'] ||[]).filter(t => parseInt(t.posizione_gruppo) === 3);
    const ott = byFase['ottavi']     || [];
    const qua = byFase['quarti']     || [];
    const sem = byFase['semifinale'] || [];
    const fin = byFase['finale']     || [];
    const vic = byFase['vincitore']  || [];

    const primi = {};
    ott.filter(t => parseInt(t.posizione_gruppo) === 1).forEach(t => { primi[t.gruppo] = t; });
    const ottNP = ott.filter(t => parseInt(t.posizione_gruppo) !== 1);
    const poW   = (g2, g3) =>
        ottNP.find(t => (t.gruppo===g2 && parseInt(t.posizione_gruppo)===2) ||
                        (t.gruppo===g3 && parseInt(t.posizione_gruppo)===3))
        || byG(po2,g2) || byG(po3,g3) || null;

    const cols = [];

    const poPairs = [[byG(po2,'A'),byG(po3,'C')],[byG(po2,'B'),byG(po3,'D')],
                     [byG(po2,'C'),byG(po3,'A')],[byG(po2,'D'),byG(po3,'B')]].filter(([a,b])=>a||b);
    if (poPairs.length)
        cols.push({ icon:'⚡', label:'Playoff',
            body: poPairs.map(([a,b])=>match(a,b)).join('') });

    const ottPairs = [[primi['A'],poW('B','D')],[primi['B'],poW('A','C')],
                      [primi['C'],poW('D','B')],[primi['D'],poW('C','A')]].filter(([a,b])=>a||b);
    if (ottPairs.length)
        cols.push({ icon:'🔵', label:'Ottavi',
            body: ottPairs.map(([a,b])=>match(a,b)).join('') });

    if (qua.length) cols.push({ icon:'⚔️', label:'Quarti',    body: pairs(qua).map(([a,b])=>match(a,b)).join('') });
    if (sem.length) cols.push({ icon:'🥈', label:'Semifinale', body: pairs(sem).map(([a,b])=>match(a,b)).join('') });

    if (fin.length || vic.length) {
        let body = pairs(fin).map(([a,b])=>match(a,b)).join('');
        if (vic.length) body += `<div class="br-champ">${row(vic[0])}<div class="br-champ-lbl">🏆 Campione</div></div>`;
        cols.push({ icon:'🥇', label:'Finale', body });
    }

    if (!cols.length) return `<p style="color:var(--text-dim);padding:16px;text-align:center">Nessun dato disponibile.</p>`;

    return `<div class="br-wrap">${cols.map((c,i) =>
        `<div class="br-col">
            <div class="br-head"><span>${c.icon}</span>${c.label}</div>
            <div class="br-body">${c.body}</div>
        </div>${i < cols.length-1 ? '<div class="br-arr">›</div>' : ''}`
    ).join('')}</div>`;
}
