...
async function doCreateCareer() {
    ...
    const piede_forte = parseInt(document.getElementById('c-piede-forte')?.value || '3');
    const piede_debole = parseInt(document.getElementById('c-piede-debole')?.value || '2');
    ...
    const res = await api('auth.php', {
        ...
        hair_color: selectedHairColor, piede_forte, piede_debole
    }, 'POST');
    if (res.error) { if(errEl) errEl.textContent = res.error; return; }
...
function renderDashboard() {
    ...
    if (pfEl) pfEl.textContent = '⭐'.repeat(pf) + '☆'.repeat(5 - pf) + ` (${pf}/5)`;
    if (pdEl) pdEl.textContent = '⭐'.repeat(pd) + '☆'.repeat(5 - pd) + ` (${pd}/5)`;
...
async function loadSkillTree() {
    ...
    if (pfV) pfV.textContent = '⭐'.repeat(pf) + '☆'.repeat(5-pf) + ` (${pf}/5)`;
    if (pdV) pdV.textContent = '⭐'.repeat(pd) + '☆'.repeat(5-pd) + ` (${pd}/5)`;
