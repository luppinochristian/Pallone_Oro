...
function showResults(res) {
    ...
    if (legaMsgs.length) {
        ...
        legaMsgs.forEach((m, i) => {
            ...
            const hasStats = m.player_gol !== undefined && m.player_gol >= 0;
            html += `<div class="res-match-card" style="border-top:3px solid ${color}">
                ...
                <div class="res-match-stats">
                    <span>⚽ ${hasStats ? m.player_gol : '-'} gol</span>
                    <span>🎯 ${hasStats ? m.player_assist : '-'} assist</span>
                    <span>📊 ${hasStats ? m.player_voto : '-'}</span>
                </div>
            </div>`;
        });
        html += `</div></div>`;
...
async function loadTransfer() {
    ...
    const [leghe, p] = await Promise.all([
        api('player.php', { action: 'leghe', career_id: currentCareerId }, 'GET'),
        api('player.php', { action: 'get', career_id: currentCareerId }, 'GET')
    ]);

    if (!p || p.error) {
        el.innerHTML = `<div style="text-align:center;padding:48px;color:var(--red)">Errore caricamento giocatore.</div>`;
        return;
    }
    if (!leghe || leghe.error || !leghe.length) {
        el.innerHTML = `<div style="text-align:center;padding:48px;color:var(--red)">Errore caricamento leghe.</div>`;
        return;
    }
    ...
    if (!selectedNazioneId && p.lega_id) {
        ...
        if (myLega) selectedNazioneId = String(myLega.nazione_id);
    }
    const nazioniMap = {};
    leghe.forEach(l => {
        const nid = String(l.nazione_id);
    ...
    Object.values(nazioniMap).forEach(n => {
        const isActive  = String(selectedNazioneId) == String(n.id);
    ...
    html += '</div>';
    const nazione = nazioniMap[String(selectedNazioneId)];
    if (nazione) {
        ...
        if (!teams || !Array.isArray(teams)) {
            html += '<div style="padding:20px;color:var(--text-dim)">Errore caricamento squadre.</div>';
        } else {
...
async function selectNazione(nazioneId) { selectedNazioneId = String(nazioneId); selectedLegaId = null; await loadTransfer(); }
