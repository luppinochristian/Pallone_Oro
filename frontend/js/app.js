...
const TRANSLATIONS = {
    it: {
        ...
        bracket_quarti_sub:'Semifinali',
        bracket_finale_sub:'Finale',
    ...
    en: {
        ...
        ai_describe_sub_html:'Write your player\'s physical features in English — hair, eyes, skin tone, expression. The AI (Pollinations) will generate a portrait.',
        ...
        bracket_quarti_sub:'Semi-finals',
        bracket_finale_sub:'Final',
...
function setLanguage(lang) {
    ...
    if (currentPlayer) {
        ...
        renderGame();
        renderDashboard();
        // Ritraduce le notizie nella dashboard se presenti
        _retranslateNewsInView();
...
// Ritraduce le notizie già caricate in _rawNewsData senza rifetching
async function _retranslateNewsInView() {
    if (!_rawNewsData || !_rawNewsData.length) return;
    const dashEl = document.getElementById('dash-notizie-lista');
    const tipoIcon = { positivo:'🟢', negativo:'🔴', mercato:'💼', agente:'🤝', obiettivo:'🎯', info:'📋' };

    // Dashboard news (ultime 5)
    if (dashEl) {
        if (currentLang === 'en') {
            dashEl.innerHTML = '<p style="color:var(--text-dim);font-size:0.82rem;padding:8px 0">Translating news...</p>';
        }
        const notizie = await _translateNewsItems(_rawNewsData.slice(0, 5), currentLang);
        dashEl.innerHTML = notizie.map(n => `
...
function _applyTranslations() {
    ...
    if (nationSearch) {
        nationSearch.placeholder = currentLang === 'en' ? '🔍 Search nation...' : '🔍 Cerca nazione...';
    }
}
...
function buildHairSVG(style, hc, s) {
    ...
    // Base scalp cap — covers the top portion of the head under all hair styles
    // This prevents any "background hair" artifacts from showing through gaps
    const baseCap = `<ellipse cx="${cx}" cy="${s*0.28}" rx="${s*0.245}" ry="${s*0.155}" fill="${hc}" opacity="1"/>`;
    switch (style) {
        case 'short':
            // Copertura top+tempie, niente che scende oltre le orecchie
            return baseCap + `<path d="M${s*0.26} ${s*0.32}
        ...
        case 'medium':
            ...
            return baseCap + `<path d="M${s*0.24} ${s*0.36}
        ...
        case 'long':
            ...
            return baseCap + `<path d="M${s*0.26} ${s*0.34}
        ...
        case 'curly':
            ...
            return baseCap + `<path d="M${s*0.28} ${s*0.34}
        ...
        case 'afro':
            ...
            return baseCap + `<ellipse cx="${cx}"     cy="${s*0.20}" rx="${s*0.30}" ry="${s*0.18}" fill="${hc}"/>
        ...
        case 'mohawk':
            ...
            return baseCap + `<path d="M${s*0.42} ${s*0.34}
...
document.addEventListener('DOMContentLoaded', async () => {
    // Nascondi immediatamente nav e menu prima dell'auth check (evita FOUC)
...
function showPage(page) {
    ...
    // Safety: always hide nav on auth-related pages
    if (page === 'auth') {
        const mn = document.getElementById('main-nav');
        if (mn) mn.style.display = 'none';
        const ew = document.getElementById('exit-dropdown-wrap');
