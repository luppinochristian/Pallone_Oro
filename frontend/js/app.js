...
function renderDashboard() {
    ...
    // Badge forma / infortunio
    const formaBadgeEl = document.getElementById('forma-badge');
    if (formaBadgeEl) {
        const infortuni = parseInt(p.infortuni_mesi ?? 0);
        if (infortuni > 0) {
            formaBadgeEl.style.display = 'inline-block';
            formaBadgeEl.innerHTML = `<span class="infortunato-badge">🩹 Infortunato — ${infortuni} mes${infortuni === 1 ? 'e' : 'i'} rimanenti</span>`;
        } else {
            // Calcola forma da morale + energia
            const formaScore = (parseInt(p.morale||50) + parseInt(p.energia||50)) / 2;
            let formaCls, formaIcon, formaLabel;
            if (formaScore >= 80)      { formaCls = 'forma-top';     formaIcon = '🔥'; formaLabel = 'In forma smagliante'; }
            else if (formaScore >= 60) { formaCls = 'forma-buona';   formaIcon = '💪'; formaLabel = 'Buona forma'; }
            else if (formaScore >= 40) { formaCls = 'forma-normale'; formaIcon = '😐'; formaLabel = 'Forma nella norma'; }
            else                       { formaCls = 'forma-scarsa';  formaIcon = '❄️'; formaLabel = 'Forma scarsa'; }
            formaBadgeEl.style.display = 'inline-block';
            formaBadgeEl.innerHTML = `<span class="forma-badge ${formaCls}">${formaIcon} ${formaLabel}</span>`;
        }
    }
    ...
    // Gol e assist stagionali (dall'anno corrente del log)
...
function renderGame() {
    ...
    // Azioni bloccate se infortunato
    const infortuni = parseInt(p.infortuni_mesi ?? 0);
    const AZIONI_BLOCCATE_INFORTUNI = ['allenamento_tiro','allenamento_velocita','dribbling',
                                        'allenamento_fisico','allenamento_speciale'];
    document.querySelectorAll('.action-btn').forEach(b => {
        b.classList.remove('selected', 'action-blocked');
        const id = b.dataset.action;
        if (infortuni > 0 && AZIONI_BLOCCATE_INFORTUNI.includes(id)) {
    ...
    // Banner infortunio sopra la griglia azioni
    const actionsPanel = document.querySelector('.actions-panel');
    const existingBanner = actionsPanel?.querySelector('.infortuni-game-banner');
    if (existingBanner) existingBanner.remove();
    if (infortuni > 0 && actionsPanel) {
        const banner = document.createElement('div');
        banner.className = 'infortuni-game-banner';
        banner.innerHTML = `🩹 Sei infortunato — <strong>${infortuni} mes${infortuni === 1 ? 'e' : 'i'} rimanenti</strong>. Puoi solo riposare, allenarti mentalmente o fare attività social.`;
...
function toggleAction(id, el) {
    if (el.classList.contains('action-blocked')) {
        toast('Non puoi allenarti fisicamente mentre sei infortunato!', 'error');
        return;
    }
...
async function toggleCareerYear(anno) {
    ...
    const getMeseNome = (m) => ['','Set','Ott','Nov','Dic','Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago'][m] || `M${m}`;
