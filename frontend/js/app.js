...
async function generateAIAvatar() {
    ...
    if (!userPrompt) {
        if (statusEl) statusEl.textContent = 'Scrivi prima una descrizione!';
        return;
    }
    if (statusEl) statusEl.textContent = '⏳ Generazione in corso (5-15s)...';
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Generando...'; }
    // Mostra spinner nell'anteprima
    const prev = document.getElementById('avatar-preview');
    if (prev) prev.innerHTML = `<div class="ai-spinner"></div>`;

    // Prompt arricchito per ottenere ritratto da giocatore di calcio
    const fullPrompt = encodeURIComponent(
        `football player portrait, ${userPrompt}, cartoon style, flat colors, clean background, square format`
    );
    const imgUrl = `https://image.pollinations.ai/prompt/${fullPrompt}?width=256&height=256&nologo=true&seed=${Math.floor(Math.random()*99999)}`;

    try {
        // Carica l'immagine e convertila in base64 per il salvataggio
        const response = await fetch(imgUrl);
        if (!response.ok) throw new Error('Generazione fallita');
        const blob = await response.blob();
        const reader = new FileReader();
        reader.onload = () => {
            _aiAvatarUrl = reader.result;  // data:image/jpeg;base64,...
            _showAIPreview(_aiAvatarUrl);
            if (statusEl) statusEl.textContent = '✅ Immagine generata!';
            if (btn) { btn.disabled = false; btn.textContent = '🎨 Rigenera'; }
        };
        reader.readAsDataURL(blob);
    } catch(e) {
        if (statusEl) statusEl.textContent = '❌ Errore generazione. Riprova.';
        if (btn) { btn.disabled = false; btn.textContent = '🎨 Genera Immagine'; }
        if (prev) prev.innerHTML = `<div style="color:var(--red);font-size:0.75rem;text-align:center;padding:16px">Errore.<br>Riprova.</div>`;
    }
}

function _showAIPreview(dataUrl) {
    const prev = document.getElementById('avatar-preview');
    if (!prev) return;
    prev.innerHTML = `<img src="${dataUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"
        onerror="this.parentElement.innerHTML='<div style=color:var(--red);font-size:0.7rem;text-align:center;padding:12px>Errore immagine</div>'" alt="AI Avatar">`;
}
