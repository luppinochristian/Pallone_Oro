...
function _renderBracket(bracket, player) {
    ...
    const matchCard = (a, b, matchPhaseRank) => {
        ...
        // Chi ha vinto questo specifico match:
        // Vince chi ha rank STRETTAMENTE più alto (ha avanzato oltre questa fase).
        // Se stessi rank: chi è eliminato ha perso. Se entrambi stesso stato: ⏳ pending.
        ...
        // Altrimenti entrambi ⏳ (match non ancora giocato)

        // Ha perso IN QUESTA fase specifica = rank uguale alla fase E eliminato
        const aLost = (rankA === matchPhaseRank) && aElim;
        const bLost = (rankB === matchPhaseRank) && bElim;
    ...
    const primiAll = {};
    ottAll.filter(t => parseInt(t.posizione_gruppo) === 1).forEach(t => { primiAll[t.gruppo] = t; });
    // Vincitori del playoff (pos 2 o 3 che hanno raggiunto gli ottavi)
    ...
    const w = ottPairs.map(([a,b]) => winnerOf(a, b));  // 4 vincitori degli ottavi (o TBD)
