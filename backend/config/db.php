 ...
 *  - getDB(): connessione singleton al database SQLite
 *  - Creazione automatica di tutte le tabelle al primo avvio
 *    (accounts, players, teams, leghe, nazioni, notizie, ecc.)
 *  - Popolamento iniziale dei dati di gioco (squadre, leghe,
 *    nazioni, strutture, agenti, trofei)
 ...
 *
 * Il database è un file SQLite (db.sqlite) nella cartella /data.
 * Tutte le relazioni tra tabelle sono gestite tramite foreign key.
 * ============================================================
 */
// Mostra errori nel log ma non nell'output (evita HTML nel JSON)
...
function initSchema($pdo) {
    $pdo->exec("CREATE TABLE IF NOT EXISTS players (
        ...
        mese_corrente INTEGER DEFAULT 9,
        anno_corrente INTEGER DEFAULT 1,
        infortuni_mesi INTEGER DEFAULT 0,
        trasferimento_anno INTEGER DEFAULT 0,
        piede_forte INTEGER DEFAULT 3,
        piede_debole INTEGER DEFAULT 2,
        piede_forte_lato TEXT DEFAULT 'dx',
        piede_debole_lato TEXT DEFAULT 'sx',
        livello_skill INTEGER DEFAULT 2,
        ...
        ai_avatar TEXT DEFAULT NULL,
        ai_prompt TEXT DEFAULT NULL,
        champions_premio_pagato INTEGER DEFAULT 0,
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS log_mensile (
        ...
        voto REAL DEFAULT 6.0, evento_speciale TEXT, avv TEXT DEFAULT '',
        esito TEXT DEFAULT '',
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (player_id) REFERENCES players(id) ON DELETE CASCADE
    )");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS calendario (
        ...
        UNIQUE(lega_id, anno, home_id, away_id)
    )");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS champions_cup (
        ...
        posizione_gruppo INTEGER DEFAULT 0,
        punti_gruppo INTEGER DEFAULT 0,
        vittorie_gruppo INTEGER DEFAULT 0,
        pareggi_gruppo INTEGER DEFAULT 0,
        sconfitte_gruppo INTEGER DEFAULT 0,
        gol_fatti_gruppo INTEGER DEFAULT 0,
        gol_subiti_gruppo INTEGER DEFAULT 0,
        partite_gruppo INTEGER DEFAULT 0,
        UNIQUE(team_id, anno),
        FOREIGN KEY (team_id) REFERENCES teams(id)
    )");
    ...
    // Sistema account (v8) - CREATE IF NOT EXISTS funziona senza errori su DB esistente
    ...
    // Nuove colonne players

    // ── Performance indexes ──────────────────────────────────────────
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_players_account ON players(account_id)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_calendario_lega_anno_mese ON calendario(lega_id, anno, mese)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_calendario_giocata ON calendario(giocata)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_champions_team_anno ON champions_cup(team_id, anno)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_classifica_lega_anno ON classifica(lega_id, anno)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_log_player_anno ON log_mensile(player_id, anno)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_notizie_player ON notizie(player_id)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_obiettivi_player_anno ON obiettivi(player_id, anno)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_stagioni_player ON stagioni(player_id)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_tokens_token ON account_tokens(token)");
    ...
    try { $pdo->exec("ALTER TABLE players ADD COLUMN piede_forte INTEGER DEFAULT 3"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN piede_debole INTEGER DEFAULT 2"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN livello_skill INTEGER DEFAULT 2"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN piede_forte_lato TEXT DEFAULT 'dx'"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN piede_debole_lato TEXT DEFAULT 'sx'"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN trasferimento_anno INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN infortuni_mesi INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN champions_premio_pagato INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN tiro INTEGER DEFAULT 50"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN dribbling INTEGER DEFAULT 50"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN mentalita INTEGER DEFAULT 50"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN energia INTEGER DEFAULT 100"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN soldi REAL DEFAULT 10000"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN team_id INTEGER DEFAULT NULL"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN anno_corrente INTEGER DEFAULT 1"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN assist_carriera INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN trofei INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN gruppo TEXT DEFAULT NULL"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN punti_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN gol_fatti_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN gol_subiti_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN partite_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN vittorie_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN pareggi_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN sconfitte_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN posizione_gruppo INTEGER DEFAULT 0"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN team_id INTEGER DEFAULT NULL"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE champions_cup ADD COLUMN eliminato INTEGER DEFAULT 0"); } catch(Exception $e) {}
    ...
    try { $pdo->exec("ALTER TABLE log_mensile ADD COLUMN avv TEXT DEFAULT ''"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE log_mensile ADD COLUMN esito TEXT DEFAULT ''"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE players ADD COLUMN champions_premio_pagato INTEGER DEFAULT 0"); } catch(Exception $e) {}
    // Bilingual news columns (migration for existing DBs)
    try { $pdo->exec("ALTER TABLE notizie ADD COLUMN titolo_en TEXT DEFAULT NULL"); } catch(Exception $e) {}
    try { $pdo->exec("ALTER TABLE notizie ADD COLUMN testo_en TEXT DEFAULT NULL"); } catch(Exception $e) {}
}
...
// Cache del body JSON per evitare doppia lettura di php://input
...
function getLang() {
    // 1. Header X-Lang (mandato dal frontend)
    foreach ($_SERVER as $k => $v) {
        if (strpos($k, 'HTTP_') === 0) {
            $key = str_replace('_', '-', substr($k, 5));
            if ($key === 'X-LANG') return ($v === 'en') ? 'en' : 'it';
        }
    }
    // 2. Query string ?lang=en (richieste GET)
    if (!empty($_GET['lang'])) return ($_GET['lang'] === 'en') ? 'en' : 'it';
    // 3. Body JSON (richieste POST)
    $body = _getCachedBody();
    if (!empty($body['lang'])) return ($body['lang'] === 'en') ? 'en' : 'it';
    // 4. Fallback italiano
    return 'it';
}

function t(string $it, string $en): string {
    return getLang() === 'en' ? $en : $it;
}
