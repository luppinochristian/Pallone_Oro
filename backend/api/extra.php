...
function getNotizie($player_id) {
    ...
    // Unica query: in EN filtra solo notizie con versione inglese disponibile
    $enFilter = ($lang === 'en') ? "AND titolo_en IS NOT NULL AND titolo_en != ''" : '';
    $stmt = $db->prepare("SELECT * FROM notizie WHERE player_id=? {$enFilter} ORDER BY id DESC LIMIT 50");
    $stmt->execute([$player_id]);
    $rows = $stmt->fetchAll();

    // Restituisce la versione nella lingua corretta
    ...
    // Conteggio non letti con la stessa logica
    $unread = $db->prepare("SELECT COUNT(*) as c FROM notizie WHERE player_id=? AND letto=0 {$enFilter}");
