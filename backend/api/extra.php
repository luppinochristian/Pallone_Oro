...
function getNotizie($player_id) {
    ...
    // When in English, only fetch news that have an English version available
    // (old news without titolo_en are pre-bilingual and would show in Italian)
    if ($lang === 'en') {
        $stmt = $db->prepare("SELECT * FROM notizie WHERE player_id=? AND titolo_en IS NOT NULL AND titolo_en != '' ORDER BY id DESC LIMIT 50");
    } else {
    ...
    $notizie = array_map(function($n) use ($lang) {
        if ($lang === 'en' && !empty($n['titolo_en']) && !empty($n['testo_en'])) {
            $n['titolo'] = $n['titolo_en'];
            $n['testo']  = $n['testo_en'];
        }
        unset($n['titolo_en'], $n['testo_en']);
        return $n;
    }, $rows);

    // Unread count: same filter in EN mode
    if ($lang === 'en') {
        $unread = $db->prepare("SELECT COUNT(*) as c FROM notizie WHERE player_id=? AND letto=0 AND titolo_en IS NOT NULL AND titolo_en != ''");
    } else {
