...
function getNotizie($player_id) {
    ...
    // For each language, filter only news that have a translated version available
    $langFilter = '';
    $langTitoloCol = 'titolo';
    $langTestoCol  = 'testo';
    if ($lang === 'en') {
        $langFilter    = "AND titolo_en IS NOT NULL AND titolo_en != ''";
        $langTitoloCol = 'titolo_en';
        $langTestoCol  = 'testo_en';
    } elseif ($lang === 'de') {
        $langFilter    = "AND titolo_de IS NOT NULL AND titolo_de != ''";
        $langTitoloCol = 'titolo_de';
        $langTestoCol  = 'testo_de';
    } elseif ($lang === 'es') {
        $langFilter    = "AND titolo_es IS NOT NULL AND titolo_es != ''";
        $langTitoloCol = 'titolo_es';
        $langTestoCol  = 'testo_es';
    }

    // For non-IT, also include news that have at least an EN fallback if no specific lang version
    if ($lang !== 'it' && $langFilter) {
        // Show translated news OR fall back to EN-available news for older entries
        $fallbackFilter = $lang === 'en' ? $langFilter : "(($langFilter) OR (titolo_en IS NOT NULL AND titolo_en != ''))";
        $stmt = $db->prepare("SELECT * FROM notizie WHERE player_id=? AND $fallbackFilter ORDER BY id DESC LIMIT 50");
    } else {
    ...
    // Serve the correct language column, with fallback chain: requested → EN → IT
    $notizie = array_map(function($n) use ($lang, $langTitoloCol, $langTestoCol) {
        if ($lang !== 'it') {
            // Try primary lang column first, then EN fallback, then keep IT
    ...
    // Unread count with same filter logic
    if ($lang !== 'it' && isset($fallbackFilter)) {
        $unread = $db->prepare("SELECT COUNT(*) as c FROM notizie WHERE player_id=? AND letto=0 AND $fallbackFilter");
    } else {
