...
function getChampions() {
    ...
    $stmt = $db->prepare("
        ...
        WHERE cc.anno = ?
        ORDER BY cc.gruppo, cc.punti_gruppo DESC, (cc.gol_fatti_gruppo - cc.gol_subiti_gruppo) DESC
    ");
    $stmt->execute([$anno]);
    $rows = $stmt->fetchAll();
    ...
    foreach ($gironi as &$g) {
        usort($g, fn($a,$b) => $b['punti_gruppo'] <=> $a['punti_gruppo'] ?: ($b['gol_fatti_gruppo']-$b['gol_subiti_gruppo']) <=> ($a['gol_fatti_gruppo']-$a['gol_subiti_gruppo']));
    }
