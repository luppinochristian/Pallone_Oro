...
function simulaChampions($db, $p) {
    ...
    // Giocatore non in Champions o eliminato: simula comunque i gironi/fasi NPC
    if (!$record || $record['eliminato']) {
        _simulaChampionsNPCOnly($db, $anno, $mese, $team_id);
        return null;
    }

    // Fase a gironi: nov-dic
...
// Simula Champions interamente per NPC quando il giocatore non partecipa
function _simulaChampionsNPCOnly($db, $anno, $mese, $skip_team_id) {
    if (in_array($mese, [11, 12])) {
        // Simula giornate di gruppo per tutti i gruppi
        ...
        _simulaFaseNPC($db, $anno, 'playoff', 'ottavi', $skip_team_id);
    } elseif ($mese == 2) {
        _simulaFaseNPC($db, $anno, 'ottavi', 'ottavi_ritorno', $skip_team_id);
    } elseif ($mese == 3) {
        _simulaFaseNPC($db, $anno, 'ottavi_ritorno', 'quarti', $skip_team_id);
    } elseif ($mese == 4) {
...
// Simula un'intera giornata di gruppo per tutte le squadre NPC di un gruppo
function _simulaGiornataGruppoNPC($db, $anno, $gruppo, $mese, $skip_team_id) {
    ...
    $coppie = ($mese == 11) ? [[0,1],[2,3]] : [[0,2],[1,3]];
    ...
    // A dicembre: chiudi il girone per le NPC
    if ($mese == 12) {
        _chiudiGironeSoloNPC($db, $anno, $gruppo, $skip_team_id);
    }
}

// Chiudi girone per NPC senza giocatore
function _chiudiGironeSoloNPC($db, $anno, $gruppo, $skip_team_id) {
    ...
    foreach ($classifica as $i => $r) {
        if ($i === 0) $db->prepare("UPDATE champions_cup SET fase='ottavi' WHERE team_id=? AND anno=?")->execute([$r['team_id'],$anno]);
        elseif ($i === 1) $db->prepare("UPDATE champions_cup SET fase='playoff' WHERE team_id=? AND anno=?")->execute([$r['team_id'],$anno]);
        else $db->prepare("UPDATE champions_cup SET eliminato=1 WHERE team_id=? AND anno=?")->execute([$r['team_id'],$anno]);
    }
}
...
function simulaGiornataGruppo($db, $p, $record, $anno, $mese) {
    ...
    // Tutte le squadre del gruppo (giocatore incluso)
    ...
    // Avversari del giocatore
    $avversari = array_values(array_filter($tutte_squadre, fn($t) => $t['team_id'] != $team_id));

    // Offset mensile: novembre → partite 0-1, dicembre → partite 2-3
    $offset = ($mese == 11) ? 0 : 2;
    $partite_da_giocare = min(2, max(0, count($avversari) - $offset));
    if ($partite_da_giocare === 0) $partite_da_giocare = min(2, count($avversari));

    $gol_fatti = 0; $gol_subiti = 0; $punti = 0; $partite_giocate = 0;
    $vittorie = 0; $pareggi = 0; $sconfitte = 0;
    $msgs_partite = [];

    // Partite del giocatore — aggiorna giocatore E avversario per ogni match
    ...
    // Aggiorna stats giocatore con V/P/S corretti
    ...
    // Round-robin NPC: coppie diverse per novembre e dicembre
    // Escludi gli avversari già aggiornati dalle partite del giocatore
    $avversari_ids_giocati = [];
    for ($i = 0; $i < $partite_da_giocare; $i++) {
        $avversari_ids_giocati[] = $avversari[($offset + $i) % count($avversari)]['team_id'];
    }
    $npc = array_values(array_filter($tutte_squadre, fn($t) => $t['team_id'] != $team_id));
    $coppie = ($mese == 11) ? [[0,1],[2,3]] : [[0,2],[1,3]];
    foreach ($coppie as [$ia, $ib]) {
        if (!isset($npc[$ia]) || !isset($npc[$ib])) continue;
        // Salta se almeno uno ha già giocato contro il giocatore questo mese
...
function simulaFaseChampions($db, $p, $record, $fase_corrente, $fase_successiva, $prob_base, $label_fase) {
    ...
    // Avanza/elimina anche tutte le NPC nella stessa fase
    ...
    if ($avanza) {
        ...
        $labels = ['playoff'=>'Playoff','ottavi'=>'Ottavi di Finale','ottavi_ritorno'=>'Ottavi (Ritorno)','quarti'=>'Quarti di Finale','semifinale'=>'Semifinale','finale'=>'Finale'];
...
// Avanza o elimina tutte le NPC che si trovano in una determinata fase
...
function concludiChampions($db, $p, $player_id) {
    ...
    // Simula finale per le NPC indipendentemente dal giocatore
