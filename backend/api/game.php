    ...
    foreach ($partiteMese as $partita) {
        ...
        if ($home_id == $team_id || $away_id == $team_id) {
            ...
            if ($rand <= $prob) {
                ...
                $casa_label = $isHome ? '🏠' : '✈️';
                $msgs[] = ['esito'=>'V','label'=>$casa_label,'giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>3,'isHome'=>$isHome,'player_gol'=>$match_partita['gol'],'player_assist'=>$match_partita['assist'],'player_voto'=>$match_partita['voto']];
            } elseif ($rand <= $prob + 15) {
                $esito = 'P'; $punti = 1;
                $gf = rand(0,2); $gs = $gf;
                $msgs[] = ['esito'=>'P','label'=>'🤝','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>1,'isHome'=>$isHome,'player_gol'=>$match_partita['gol'],'player_assist'=>$match_partita['assist'],'player_voto'=>$match_partita['voto']];
            } else {
                $esito = 'S'; $punti = 0;
                $gf = rand(0,2); $gs = rand(1,4);
                $msgs[] = ['esito'=>'S','label'=>'💔','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>0,'isHome'=>$isHome,'player_gol'=>$match_partita['gol'],'player_assist'=>$match_partita['assist'],'player_voto'=>$match_partita['voto']];
            }
