...
function _chiudiGironeSoloNPC($db, $anno, $gruppo, $skip_team_id) {
    $stmt = $db->prepare("SELECT cc.team_id FROM champions_cup cc WHERE cc.anno=? AND cc.gruppo=? AND cc.team_id!=? ORDER BY cc.punti_gruppo DESC, (cc.gol_fatti_gruppo-cc.gol_subiti_gruppo) DESC, cc.gol_fatti_gruppo DESC");
...
function qualificaChampions($db, $anno) {
    ...
    foreach ($leghe as $lega) {
        ...
        $anno_class = $anno - 1;
        $stmt = $db->prepare("SELECT c.team_id, t.ovr FROM classifica c JOIN teams t ON c.team_id=t.id WHERE c.lega_id=? AND c.anno=? ORDER BY c.punti DESC, (c.gol_fatti-c.gol_subiti) DESC, c.gol_fatti DESC LIMIT 4");
...
function chiudiGirone($db, $p, $record, $anno, $gruppo) {
    $team_id = $p['team_id'];
    $stmt = $db->prepare("SELECT cc.team_id, cc.punti_gruppo, cc.gol_fatti_gruppo, cc.gol_subiti_gruppo FROM champions_cup cc WHERE cc.anno=? AND cc.gruppo=? ORDER BY cc.punti_gruppo DESC, (cc.gol_fatti_gruppo-cc.gol_subiti_gruppo) DESC, cc.gol_fatti_gruppo DESC");
    $stmt->execute([$anno, $gruppo]);
...
function concludiChampions($db, $p, $player_id) {
    ...
    // Fallback: se il giocatore è ancora in finale (non processato a maggio per qualsiasi motivo)
...
function calcPremiCampionato($db, $p, $player_id) {
    ...
    $stmt = $db->prepare("
        ...
        ORDER BY c.punti DESC, diff DESC, c.gol_fatti DESC
    ");
...
function calcPalloneDoro($db, $player_id, $p, $overall, $gol_carriera, $assist_carriera) {
    $anno = $p['anno_corrente'];
    $stmt = $db->prepare("SELECT SUM(gol) as gol,SUM(assist) as assist,AVG(voto) as voto FROM log_mensile WHERE player_id=? AND anno=? AND avv!='' AND avv!='__riepilogo'");
