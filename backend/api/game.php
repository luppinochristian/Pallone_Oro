...
function playMonth($player_id, $data) {
    ...
    // Inietta gol/assist/voto del giocatore nella sua partita (la prima con player_gol=-1)
    $lega_msgs = $lega_result['msgs'] ?? [];
    foreach ($lega_msgs as &$lm) {
        if (isset($lm['player_gol']) && $lm['player_gol'] === -1) {
            $lm['player_gol']    = $match['gol'];
            $lm['player_assist'] = $match['assist'];
            $lm['player_voto']   = $match['voto'];
            break;
        }
    }
    unset($lm);
    // lega_msgs inviati separatamente nel JSON, non in risultati testuali
    ...
    foreach ($partiteMese as $partita) {
        ...
        if ($home_id == $team_id || $away_id == $team_id) {
            ...
            if ($rand <= $prob) {
                ...
                $casa_label = $isHome ? '🏠' : '✈️';
                $msgs[] = ['esito'=>'V','label'=>$casa_label,'giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>3,'isHome'=>$isHome,'player_gol'=>-1,'player_assist'=>-1,'player_voto'=>-1];
            } elseif ($rand <= $prob + 15) {
                $esito = 'P'; $punti = 1;
                $gf = rand(0,2); $gs = $gf;
                $msgs[] = ['esito'=>'P','label'=>'🤝','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>1,'isHome'=>$isHome,'player_gol'=>-1,'player_assist'=>-1,'player_voto'=>-1];
            } else {
                $esito = 'S'; $punti = 0;
                $gf = rand(0,2); $gs = rand(1,4);
                $msgs[] = ['esito'=>'S','label'=>'💔','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>0,'isHome'=>$isHome,'player_gol'=>-1,'player_assist'=>-1,'player_voto'=>-1];
            }
