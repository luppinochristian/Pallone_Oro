...
function playMonth($player_id, $data) {
    ...
    foreach (($lega_result['msgs'] ?? []) as $m) $risultati[] = $m;
    ...
    foreach ($partiteMese as $partita) {
        ...
        if ($home_id == $team_id || $away_id == $team_id) {
            ...
            if ($rand <= $prob) {
                ...
                $casa_label = $isHome ? '🏠' : '✈️';
                $msgs[] = "{$casa_label} G{$partita['giornata']}: vinci {$gf}-{$gs} vs {$avv_nome} (+3 pt)";
            } elseif ($rand <= $prob + 15) {
                $esito = 'P'; $punti = 1;
                $gf = rand(0,2); $gs = $gf;
                $msgs[] = "🤝 G{$partita['giornata']}: pareggio {$gf}-{$gs} vs {$avv_nome} (+1 pt)";
            } else {
                $esito = 'S'; $punti = 0;
                $gf = rand(0,2); $gs = rand(1,4);
                $msgs[] = "💔 G{$partita['giornata']}: sconfitta {$gf}-{$gs} vs {$avv_nome}";
            }
