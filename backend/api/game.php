...
function simulaChampions($db, $p) {
    ...
    // Mese 10 (ottobre): qualifica le top-4 di ogni prima divisione (se non già fatto)
    if ($mese == 10) {
    ...
    // Fase a gironi: novembre-dicembre (11,12)
    if (in_array($mese, [11, 12]) && $record['fase'] == 'gironi') {
        return simulaFaseChampions($db, $p, $record, 'gironi', 'quarti', 1, 55);
    }
    // Quarti: gennaio-febbraio (1,2)
    if (in_array($mese, [1, 2]) && $record['fase'] == 'quarti') {
        return simulaFaseChampions($db, $p, $record, 'quarti', 'semifinale', 3, 60);
    }
    // Semifinale: marzo-aprile (3,4)
    if (in_array($mese, [3, 4]) && $record['fase'] == 'semifinale') {
        return simulaFaseChampions($db, $p, $record, 'semifinale', 'finale', 5, 65);
    }
...
function changeTeam($player_id, $data) {
    ...
    // Il primo mese di carriera (settembre anno 1) è bloccato
    $finestra_aperta = in_array($mese, [1, 2, 7, 8, 9]);
