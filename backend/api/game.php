...
function playMonth($player_id, $data) {
    ...
    // Distribuisce gol/assist del giocatore tra tutte le sue partite del mese
    $lega_msgs = $lega_result['msgs'] ?? [];
    $player_idxs = [];
    foreach ($lega_msgs as $idx => $lm) {
        if (isset($lm['player_gol']) && $lm['player_gol'] === -1) $player_idxs[] = $idx;
    }
    $num_pm = count($player_idxs);
    if ($num_pm > 0) {
        $gol_rimasti    = intval($match['gol']);
        $assist_rimasti = intval($match['assist']);
        foreach ($player_idxs as $i => $idx) {
            $is_last = ($i === $num_pm - 1);
            $g = $is_last ? $gol_rimasti    : ($gol_rimasti > 0    ? rand(0, $gol_rimasti)    : 0);
            $a = $is_last ? $assist_rimasti : ($assist_rimasti > 0 ? rand(0, $assist_rimasti) : 0);
            if (!$is_last) { $gol_rimasti -= $g; $assist_rimasti -= $a; }
            $lega_msgs[$idx]['player_gol']    = $g;
            $lega_msgs[$idx]['player_assist'] = $a;
            $lega_msgs[$idx]['player_voto']   = $match['voto'];
        }
    }
...
function simulaGiornataLega($db, $p, $match) {
    ...
    foreach ($partiteMese as $partita) {
        ...
        if ($home_id == $team_id || $away_id == $team_id) {
            ...
            if ($rand <= $prob) {
                ...
                $casa_label = $isHome ? '🏠' : '✈️';
                $msgs[] = ['esito'=>'V','label'=>$casa_label,'giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>3,'isHome'=>$isHome];
            } elseif ($rand <= $prob + 15) {
                $esito = 'P'; $punti = 1;
                $gf = rand(0,2); $gs = $gf;
                $msgs[] = ['esito'=>'P','label'=>'🤝','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>1,'isHome'=>$isHome];
            } else {
                $esito = 'S'; $punti = 0;
                $gf = rand(0,2); $gs = rand(1,4);
                $msgs[] = ['esito'=>'S','label'=>'💔','giornata'=>$partita['giornata'],'avv'=>$avv_nome,'gf'=>$gf,'gs'=>$gs,'punti'=>0,'isHome'=>$isHome];
            }
    ...
    return ['msgs'=>$msgs,'bonus_soldi'=>0,'trofeo'=>0];
}
...
function generaNotizieDinamiche($db, $player_id, $p, $match, $lega_result, $player_skills = []) {
    ...
    $legaMsg = implode(' | ', $lega_result['msgs'] ?? []);
