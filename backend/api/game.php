...
function playMonth($player_id, $data) {
    ...
    foreach ($azioni as $azione) {
        ...
        if (in_array($azione, $azioni_fisiche) && $azione !== 'allenamento_speciale') {
            ...
            if ($rischio_en > 0 && rand(1, 100) <= $rischio_en) {
                ...
                $risultati[] = t("🚨 INFORTUNIO per stanchezza! Energia troppo bassa ({$en_display}%). Fuori 1 mese!","🚨 INJURY from fatigue! Energy too low ({$en_display}%). Out for 1 month!");
        ...
        switch ($azione) {
            case 'allenamento_tiro':
                ...
                $risultati[] = t("🎯 Tiro migliorato +{$g}","🎯 Shooting improved +{$g}"); break;
            case 'allenamento_velocita':
                $g = rand(1,3) + $bonus['bonus_crescita'];
                $sc['velocita'] += $g; $sc['energia'] -= rand(10,20);
                $risultati[] = t("⚡ Velocità migliorata +{$g}","⚡ Speed improved +{$g}"); break;
            case 'dribbling':
                $g = rand(1,3) + $bonus['bonus_crescita'];
                $sc['dribbling'] += $g; $sc['energia'] -= rand(10,20);
                $risultati[] = t("🏃 Dribbling migliorato +{$g}","🏃 Dribbling improved +{$g}"); break;
            case 'allenamento_fisico':
                $g = rand(1,3) + $bonus['bonus_crescita'];
                $sc['fisico'] += $g; $sc['energia'] -= rand(10,20);
                $risultati[] = t("💪 Fisico migliorato +{$g}","💪 Physical improved +{$g}"); break;
            case 'allenamento_mentalita':
                $g = rand(1,3) + $bonus['bonus_crescita'];
                $sc['mentalita'] += $g; $sc['energia'] -= rand(8,15);
                $sc['morale'] += rand(3,7); // allenare la testa migliora anche l'umore
                $risultati[] = t("🧠 Mentalità migliorata +{$g}","🧠 Mental improved +{$g}"); break;
            case 'riposo':
                $r = rand(20,40);
                $sc['energia'] += $r; $sc['morale'] += rand(5,10);
                $risultati[] = t("😴 Riposo: +{$r} energia","😴 Rest: +{$r} energy"); break;
            case 'social':
                $pop = rand(2,8) + intval($p['team_stelle'] ?? 1);
                $sc['popolarita'] += $pop; $sc['morale'] += rand(3,8);
                $risultati[] = t("📱 Social: +{$pop} popolarità","📱 Social: +{$pop} popularity"); break;
            case 'allenamento_speciale':
                ...
                if ($rischio > 75) {
                    ...
                    $risultati[] = t("🚨 INFORTUNIO! Sarai fuori per {$durata_infortunio} " . ($durata_infortunio === 1 ? "mese" : "mesi") . "!","🚨 INJURY! Out for {$durata_infortunio} month" . ($durata_infortunio === 1 ? "" : "s") . "!");
                } else {
    ...
    if ($p['struttura_livello'] >= 7) {
        $sp = rand(5000,20000);
        $match['stipendio'] += $sp;
        $risultati[] = t("🤝 Sponsor Academy: +€","🤝 Academy Sponsor: +€").number_format($sp);
    }
...
function getAdvancedMatchNews($nome, $team, $avv, $pgol, $passist, $pvoto, $esito, $giornata, $gf, $gs, $casa, $skills = []) {
    $items = [];
    $scarto = abs($gf - $gs);
    $isHome = $casa === _tn('in casa','at home');
...
function getExtraNewsPool($nome, $team, $avv, $pgol, $passist, $pvoto, $esito, $overall, $pop, $stelle, $anno, $mese) {
    ...
    $gf_label = $pgol > 0
        ? "{$pgol} " . _tn('gol','goals')
        : ($passist > 0 ? "{$passist} assist" : _tn('una prestazione solida','a solid performance'));
...
function generaNotizia($db, $player_id, $anno, $mese, $titolo, $testo, $tipo='info', $titolo_en=null, $testo_en=null) {
    $db->prepare("INSERT INTO notizie (player_id,anno,mese,titolo,testo,titolo_en,testo_en,tipo) VALUES(?,?,?,?,?,?,?,?)")
       ->execute([$player_id, $anno, $mese, $titolo, $testo, $titolo_en, $testo_en, $tipo]);
}
...
function _tn(string $it, string $en): string { return _gsLang() === 'en' ? $en : $it; }
function _tna(array $it, array $en): array   { return _gsLang() === 'en' ? $en : $it; }
...
function _buildNotizie($p, $match, $lega_result, $player_skills = []): array {
    ...
    if (!empty($partite) && rand(1,100) <= 75) {
        ...
        $pm_casa  = ($prime_partita['isHome'] ?? false) ? _tn('in casa','at home') : _tn('in trasferta','away');
...
function generaNotizieDinamiche($db, $player_id, $p, $match, $lega_result, $player_skills = []) {
    ...
    $count = min(count($notizie_it), count($notizie_en));
    for ($i = 0; $i < $count; $i++) {
        $it = $notizie_it[$i];
        $en = $notizie_en[$i];
