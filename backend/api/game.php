...
function _simulaGiornataGruppoNPC($db, $anno, $gruppo, $mese, $skip_team_id) {
    ...
    // Stesse coppie mensili usate in simulaGiornataGruppo (per coerenza del calendario)
    // Quando il giocatore non è in questo gruppo, simula TUTTE le 3 partite della giornata
    $npc_full_schedule = [
        9  => [[0,1],[2,3]],
        10 => [[0,2],[1,3]],
        11 => [[0,3],[1,2]],
    ...
    $coppie = $npc_full_schedule[$mese] ?? [[0,1]];
...
function simulaGiornataGruppo($db, $p, $record, $anno, $mese) {
    ...
    // Leggi tutte le squadre del gruppo ordinate per team_id (ordine stabile)
    ...
    $n_avv = count($avversari); // 4 in un gruppo da 5

    // Rotazione avversari per mese — garantisce ogni avversario incontrato una volta
    // Settembre: avv[0], avv[1]  | Ottobre: avv[2], avv[3]
    // Novembre:  avv[1], avv[2]  | Dicembre: avv[0], avv[3]  (ritorno cross)
    $schedule = [
        9  => [0, 1],
        10 => [2, 3],
        11 => [1, 2],
    ...
    $idx_list = $schedule[$mese] ?? [0, 1 % $n_avv];
    ...
    foreach ($idx_list as $idx) {
        ...
        $msgs_partite[] = "Gr.{$gruppo} {$gf}-{$gs} vs {$avv['ovr']}ovr";
    ...
    // NPC round-robin — la coppia NPC che non ha già giocato col giocatore questo mese
    // Settembre: giocatore vs [0,1] → NPC: [2 vs 3]
    // Ottobre:   giocatore vs [2,3] → NPC: [0 vs 1]
    // Novembre:  giocatore vs [1,2] → NPC: [0 vs 3]
    // Dicembre:  giocatore vs [0,3] → NPC: [1 vs 2]
    $npc_schedule = [
        9  => [[2,3]],
        10 => [[0,1]],
        11 => [[0,3]],
    ...
    $coppie_npc = $npc_schedule[$mese] ?? [[0,1]];
    foreach ($coppie_npc as [$ia, $ib]) {
    ...
    return "{$esito} Champions Gr.{$gruppo}: " . implode(', ', array_map(fn($m) => explode(' vs ',$m)[0], $msgs_partite)) . " ({$punti}pt)";
}
...
function _simulaPlayoffNPC($db, $anno, $skip_team_id) {
    ...
    $terze = $db->prepare("SELECT cc.team_id, cc.gruppo, t.ovr FROM champions_cup cc JOIN teams t ON cc.team_id=t.id WHERE cc.anno=? AND cc.fase='playoff_3' AND cc.eliminato=0 AND cc.team_id!=? ORDER BY cc.gruppo");
    $terze->execute([$anno, $skip_team_id]);
