...
function _simulaGiornataGruppoNPC($db, $anno, $gruppo, $mese, $skip_team_id) {
    // Stesso calendario del gruppo con giocatore ma tutte e 5 le squadre sono NPC.
    // S0..S4 ordinate per team_id. Stessa logica: 3 partite per mese (2 giornate condensate).
    // Set(9):  S0-S1, S0-S2, S3-S4  (S0 gioca 2 volte, S3-S4 una, S2 riposa 1°)
    // Usiamo lo stesso schema "fixed" dove S0 è il "perno" (come P nel gruppo del giocatore)
    // Set(9):  S0 vs S1, S0 vs S2, S3 vs S4
    // Ott(10): S0 vs S3, S0 vs S4, S1 vs S2
    // Nov(11): S0 vs S1, S0 vs S3, S2 vs S4
    // Dic(12): S0 vs S2, S0 vs S4, S1 vs S3
    ...
    $coppie = $full_schedule[$mese] ?? [[0,1],[2,3]];
...
function simulaGiornataGruppo($db, $p, $record, $anno, $mese) {
    ...
    // Calendario 8 partite: P gioca 2 partite/mese contro avversari diversi (andata+ritorno)
    // Ogni NPC affronta P esattamente 2 volte, + 1 partita NPC vs NPC al mese
    // Set(9):  P vs N0, P vs N1, N2 vs N3
    // Ott(10): P vs N2, P vs N3, N0 vs N1
    // Nov(11): P vs N0, P vs N2, N1 vs N3  ← ritorno
    // Dic(12): P vs N1, P vs N3, N0 vs N2  ← ritorno
    $player_schedule = [
        9  => [0, 1],
        10 => [2, 3],
        11 => [0, 2],
    ...
    // --- 1 Partita NPC vs NPC ---
    [$ia, $ib] = $npc_schedule[$mese] ?? [2, 3];
