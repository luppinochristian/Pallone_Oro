...
function _simulaGiornataGruppoNPC($db, $anno, $gruppo, $mese, $skip_team_id) {
    // Quando il giocatore non è in questo gruppo: tutte e 5 le squadre sono NPC.
    // Stesso calendario Berger: ogni giornata 2 partite, 1 squadra riposa.
    // Squadre ordinate per team_id: [S0, S1, S2, S3, S4]
    // Set(9):  S0 vs S1, S2 vs S3  [S4 riposa]
    // Ott(10): S0 vs S2, S1 vs S4  [S3 riposa]  ← S3 riposa al posto di skip
    // Nov(11): S0 vs S3, S1 vs S2  [S4 riposa]  wait — usiamo stesso schema senza fisso
    // Schema bilanciato 5 squadre / 4 giornate (2 partite ciascuna):
    // G1: (0vs1),(2vs3) — S4 riposa
    // G2: (0vs2),(3vs4) — S1 riposa
    // G3: (0vs3),(1vs4) — S2 riposa
    // G4: (0vs4),(1vs2) — S3 riposa
...
function simulaGiornataGruppo($db, $p, $record, $anno, $mese) {
    ...
    // Leggi tutte le squadre ordinate per team_id (ordine stabile)
    $stmt = $db->prepare("SELECT cc.team_id, t.ovr FROM champions_cup cc JOIN teams t ON cc.team_id=t.id WHERE cc.anno=? AND cc.gruppo=? ORDER BY cc.team_id");
    $stmt->execute([$anno, $gruppo]);
    $tutte_squadre = $stmt->fetchAll();
    if (count($tutte_squadre) < 2) return null;

    // Con 5 squadre e 4 giornate: ogni giornata il giocatore gioca 1 partita
    // e 2 NPC giocano tra loro, con 1 NPC che riposa (diverso ogni mese).
    // Calendario Berger-like (giocatore fisso):
    //   Set (9):  P vs N0, N1 vs N2  [N3 riposa]
    //   Ott (10): P vs N1, N2 vs N3  [N0 riposa]
    //   Nov (11): P vs N2, N0 vs N3  [N1 riposa]
    //   Dic (12): P vs N3, N0 vs N1  [N2 riposa]
    // NPC indices sono gli avversari [0..3] nell'ordine team_id
    $avversari = array_values(array_filter($tutte_squadre, fn($t) => $t['team_id'] != $team_id));
    $n_avv = count($avversari);

    $player_schedule = [9=>0, 10=>1, 11=>2, 12=>3];
    $npc_schedule    = [9=>[1,2], 10=>[2,3], 11=>[0,3], 12=>[0,1]];

    $avv_idx = $player_schedule[$mese] ?? 0;
    if (!isset($avversari[$avv_idx % $n_avv])) return null;

    // --- Partita del giocatore ---
    $avv = $avversari[$avv_idx % $n_avv];
    ...
    // Aggiorna giocatore
    $db->prepare("UPDATE champions_cup SET punti_gruppo=punti_gruppo+?,vittorie_gruppo=vittorie_gruppo+?,pareggi_gruppo=pareggi_gruppo+?,sconfitte_gruppo=sconfitte_gruppo+?,gol_fatti_gruppo=gol_fatti_gruppo+?,gol_subiti_gruppo=gol_subiti_gruppo+?,partite_gruppo=partite_gruppo+1 WHERE team_id=? AND anno=?")
       ->execute([$pt,$vittoria,$pareggio,$sconfitta,$gf,$gs,$team_id,$anno]);
    // Aggiorna avversario NPC
    $db->prepare("UPDATE champions_cup SET punti_gruppo=punti_gruppo+?,vittorie_gruppo=vittorie_gruppo+?,pareggi_gruppo=pareggi_gruppo+?,sconfitte_gruppo=sconfitte_gruppo+?,gol_fatti_gruppo=gol_fatti_gruppo+?,gol_subiti_gruppo=gol_subiti_gruppo+?,partite_gruppo=partite_gruppo+1 WHERE team_id=? AND anno=?")
       ->execute([$pt_avv,$v_avv,$pa_avv,$s_avv,$gs,$gf,$avv['team_id'],$anno]);

    // --- Partita tra le due NPC del mese ---
    $npc_pair = $npc_schedule[$mese] ?? [1,2];
    [$ia, $ib] = $npc_pair;
    if (isset($avversari[$ia % $n_avv]) && isset($avversari[$ib % $n_avv])) {
    ...
    $avv_nome = $avv['ovr'] . 'ovr';
    $esito = $pt >= 3 ? "🏆" : ($pt >= 1 ? "🤝" : "💔");
    return "{$esito} Champions Gr.{$gruppo}: {$gf}-{$gs} ({$pt}pt)";
}
...
function simulaFaseChampions($db, $p, $record, $fase_corrente, $fase_successiva, $prob_base, $label_fase) {
    ...
    if ($fase_corrente === 'playoff' || $fase_corrente === 'playoff_3') {
        ...
        // Se il giocatore è terzo, il suo avversario è la seconda di un altro gruppo
        if ($pos === 3) {
            $inv = ['A'=>['opp_gruppo'=>'C','opp_pos'=>2],
                    'B'=>['opp_gruppo'=>'D','opp_pos'=>2],
                    'C'=>['opp_gruppo'=>'A','opp_pos'=>2],
                    'D'=>['opp_gruppo'=>'B','opp_pos'=>2]];
            $info = $inv[$gruppo] ?? null;
        } else {
        ...
        if ($info) {
            ...
            $stmt->execute([$anno, $info['opp_gruppo'], $info['opp_pos'] ?? $info['opp_pos']]);
            $avv = $stmt->fetch();
