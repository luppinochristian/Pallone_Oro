    ...
    foreach ($partiteMese as $partita) {
        ...
        if ($home_id == $team_id || $away_id == $team_id) {
            ...
            if ($rand <= $prob) {
                ...
                $casa_label = $isHome ? '🏠' : '✈️';
                // Gol/assist giocatore non possono superare i gol della squadra
                ...
                $esito = 'P'; $punti = 1;
                $gf = rand(0,2); $gs = $gf;
                // Pareggio: gol giocatore non supera i gol della squadra
                $pg = min($match_partita['gol'], $gf);
                $pa_p = min($match_partita['assist'], max(0, $gf - $pg));
                ...
                // Sconfitta: gol giocatore non supera i gol della squadra (spesso 0)
                $pg = min($match_partita['gol'], $gf);
                $pa_p = min($match_partita['assist'], max(0, $gf - $pg));
