...
function getAgentBonus($db, $player_id) {
    ...
    $ag = $stmt->fetch();
    if (!$ag || !$ag['livello']) return ['bonus_stipendio'=>0,'bonus_offerte'=>0];
    $agenti = [
        1=>['bonus_stipendio'=>0.10,'bonus_offerte'=>1],
        2=>['bonus_stipendio'=>0.20,'bonus_offerte'=>2],
        3=>['bonus_stipendio'=>0.35,'bonus_offerte'=>3],
        4=>['bonus_stipendio'=>0.50,'bonus_offerte'=>5],
    ];
    return $agenti[$ag['livello']] ?? ['bonus_stipendio'=>0,'bonus_offerte'=>0];
}
...
function changeTeam($player_id, $data) {
    ...
    if ($p['overall']<$min_overall && $team['stelle']>1) {echo json_encode(['error'=>"Overall insufficiente. Serve almeno {$min_overall}"]);return;}
