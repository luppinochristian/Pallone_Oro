...
if (!defined('AGENTI')) {
    define('AGENTI', [
        ...
        4 => ['nome'=>'Alexandre Dupont',  'costo'=>250000, 'costo_up'=>500000, 'bonus_stipendio'=>0.50,'bonus_ovr_sconto'=>10.0, 'pop_richiesta'=>75, 'descr'=>'Super-agente. +50% stipendio, -10% OVR. Solo per star: 75 popolarità.'],
        5 => ['nome'=>'Giovanni El-Fares',  'costo'=>600000, 'costo_up'=>0,      'bonus_stipendio'=>0.75,'bonus_ovr_sconto'=>15.0, 'pop_richiesta'=>90, 'descr'=>'Leggenda degli agenti. +75% stipendio, -15% OVR. Riservato alle icone: 90 popolarità.'],
    ]);
}
...
function assumiAgente($player_id, $data) {
    ...
    $stmt->execute([$player_id]);
    $player = $stmt->fetch();
    if (!$player) { echo json_encode(['error'=>'Giocatore non trovato']); return; }
