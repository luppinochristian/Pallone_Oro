...
if (!$player_id) {
    echo json_encode(['error' => t('Non autenticato','Not authenticated')]); exit;
}
if ($player_id < 0) {
    echo json_encode(['error' => t('Seleziona una carriera','Please select a career')]); exit;
}
...
function getPlayer($id) {
    ...
    else echo json_encode(['error' => t('Giocatore non trovato','Player not found')]);
}
...
function updateAppearance($player_id) {
    ...
    $allowed_hair = ['short_black','short_brown','short_blonde','short_red','long_black','long_brown','long_blonde','afro_black','afro_brown','bald','curly_black','curly_brown',
                     'short','medium','long','curly','afro','mohawk','bun'];
    ...
    if ($updates) {
        ...
        echo json_encode(['error' => t('Nessun campo valido da aggiornare','No valid fields to update')]);
    }
}
