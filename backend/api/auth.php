...
function doRegister($data) {
    ...
    try {
        // Squadra iniziale: seconda divisione casuale, metà inferiore classifica (posizioni 10-18)
        $leghe_seconde = $db->query("SELECT id FROM leghe WHERE livello=2")->fetchAll();
        $lega_random = $leghe_seconde[array_rand($leghe_seconde)];
        $stmt_sq = $db->prepare("SELECT id FROM teams WHERE lega_id=? ORDER BY ovr ASC LIMIT 9");
        $stmt_sq->execute([$lega_random['id']]);
        $squadre_basse = $stmt_sq->fetchAll();
        $team_iniziale = !empty($squadre_basse) ? $squadre_basse[array_rand($squadre_basse)]['id'] : 1;

        $stmt = $db->prepare("INSERT INTO players 
            (username, password, player_name, gender, nationality, age, overall, tiro, velocita, dribbling, fisico, mentalita, team_id) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->execute([
            $username,
            ...
            $age,
            $overall,
            rand(45,56), rand(45,56), rand(45,56), rand(45,56), rand(45,56),
            $team_iniziale
        ]);
