...
function getCareerList($db,$account_id){
    $stmt=$db->prepare("SELECT p.id,p.career_name,p.player_name,p.gender,p.age,p.overall,p.mese_corrente,p.anno_corrente,p.palloni_doro,p.trofei,p.gol_carriera,p.skin_hair,p.skin_color,p.eye_color,p.hair_color,t.nome as team_nome,l.nome as lega_nome FROM players p LEFT JOIN teams t ON p.team_id=t.id LEFT JOIN leghe l ON t.lega_id=l.id WHERE p.account_id=? ORDER BY p.created_at DESC");
...
function createCareer($data){
    ...
    $overall=rand(50,56);
    $db->prepare("INSERT INTO players (account_id,career_name,player_name,gender,nationality,age,overall,tiro,velocita,dribbling,fisico,mentalita,team_id,skin_hair,skin_color,eye_color,hair_color,piede_forte,piede_debole,livello_skill,piede_forte_lato,piede_debole_lato,mese_corrente) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")->execute([$account_id,$career_name,$player_name,$gender,$nationality,$age,$overall,rand(45,56),rand(45,56),rand(45,56),rand(45,56),rand(45,56),$team_id,$skin_hair,$skin_color,$eye_color,$hair_color,$piede_forte,$piede_debole,2,$piede_forte_lato,$piede_debole_lato,9]);
