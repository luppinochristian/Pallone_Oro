...
function createCareer($data){
    ...
    $piede_forte=min(5,max(1,intval($data['piede_forte']??3)));
    $piede_debole=min(5,max(1,intval($data['piede_debole']??2)));
    ...
    $overall=rand(50,56);
    $db->prepare("INSERT INTO players (account_id,career_name,player_name,gender,nationality,age,overall,tiro,velocita,dribbling,fisico,mentalita,team_id,skin_hair,skin_color,eye_color,piede_forte,piede_debole,livello_skill) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")->execute([$account_id,$career_name,$player_name,$gender,$nationality,$age,$overall,rand(45,56),rand(45,56),rand(45,56),rand(45,56),rand(45,56),$team_id,$skin_hair,$skin_color,$eye_color,$piede_forte,$piede_debole,2]);
