...
function getCareerList($db,$account_id){
    $stmt=$db->prepare("SELECT p.id,p.career_name,p.player_name,p.gender,p.age,p.overall,p.mese_corrente,p.anno_corrente,p.palloni_doro,p.trofei,p.gol_carriera,p.skin_hair,p.skin_color,p.eye_color,p.hair_color,p.ai_avatar,t.nome as team_nome,l.nome as lega_nome FROM players p LEFT JOIN teams t ON p.team_id=t.id LEFT JOIN leghe l ON t.lega_id=l.id WHERE p.account_id=? ORDER BY p.created_at DESC");
...
function createCareer($data){
    ...
    $ai_avatar=$data['ai_avatar']??null;  // base64 dell'immagine AI (null = modalità default)
    ...
    $overall=rand(50,56);
    $db->prepare("INSERT INTO players (account_id,career_name,player_name,gender,nationality,age,overall,tiro,velocita,dribbling,fisico,mentalita,team_id,skin_hair,skin_color,eye_color,hair_color,ai_avatar,piede_forte,piede_debole,livello_skill,piede_forte_lato,piede_debole_lato,mese_corrente) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")->execute([$account_id,$career_name,$player_name,$gender,$nationality,$age,$overall,rand(45,56),rand(45,56),rand(45,56),rand(45,56),rand(45,56),$team_id,$skin_hair,$skin_color,$eye_color,$hair_color,$ai_avatar,$piede_forte,$piede_debole,2,$piede_forte_lato,$piede_debole_lato,9]);
